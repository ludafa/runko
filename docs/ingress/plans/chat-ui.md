...
> 相关：[产品文档](../features/chat-ui.md) · [技术方案](../tech/chat-ui.md)
...
1. **两处动了数据流的交互增量**（超出「只改呈现」的边界，见[技术方案 §5](../tech/chat-ui.md)）：插话的「待注入」乐观回显、待发消息的「插进本轮」。两者都是改版过程中暴露的反馈缺口——前者点了没反应，后者只能删了重发。**D8 保留**。
...
4. **ai-elements 的官方 CLI 与 shadcn CLI 在本机都跑不通**（`@modelcontextprotocol/sdk` 要 `zod/v4`、解析到 v3）。改为照 CLI 的逻辑手工从 registry 取件，见[技术方案 §2](../tech/chat-ui.md)。
5. **上游是按 radix 写的，本项目是 base-ui**（`components.json` 的 `base-vega` 风格）。`asChild`→`render`、`data-[state=open]`→`data-[open]`/`data-[panel-open]`、HoverCard 延迟 props 都要就地适配，见[技术方案 §3](../tech/chat-ui.md)。
...
### 人眼验收（对照[产品文档](../features/chat-ui.md)「成功标准」）
