...
related: ["ingress/features/chat-webapp.md", "ingress/tech/chat-webapp.md", "architecture/tech/agent-kernel.md"]
---
# Chat Webapp（施工进展）

> 相关：[产品视角](../features/chat-webapp.md) · [技术方案](../tech/chat-webapp.md)
> 依赖：[core-sdk](../../logic/engine/features/core-sdk.md) · [sandbox](../../host/contract/features/sandbox.md)
> 被增强：[turn-checkpoint](../../logic/orchestration/features/turn-checkpoint.md) · [single-ledger](../../logic/orchestration/features/single-ledger.md) · [compaction](../../logic/engine/features/compaction.md)

> **立项 banner**：已立项开工——2026-07-12 用户 `/goal` 直接立项。
> 原型来源：[sandbox 端到端示例](../../host/contract/features/sandbox.md)（沙盒 E2E 示例 = 本应用的核心能力原型）。
...
- **P12-0 脚手架（主线程）**：seed 拷入 apps/（client→web 更名、去 docs/.claude/.git）、包名改 `@runko-chat/*`、根 workspace 并入 + 根脚本 filter 收窄、overrides/allowBuilds 合入、`pnpm install` + seed 基线可跑（bootstrap：db migrate + openapi + kubb 生成）。
...
- **P13-1 transcript 减量**（durable/ephemeral 分层，✅ 已交付）。
- **P13-5 单一数据账本**被定为**最高优先级**：UIMessage 为唯一真相，验证实验方案见 [single-ledger](../../logic/orchestration/features/single-ledger.md)，通过后 core 迁移正式立项——**已落地**（P13-5-1..6，见 [plans/single-ledger](../../logic/orchestration/plans/single-ledger.md) 与 [tech/single-ledger](../../logic/orchestration/tech/single-ledger.md)）。
- **P13-2 turn checkpoint 与 P13-2b 沙盒保活顺延**（方案见 [turn-checkpoint](../../logic/orchestration/features/turn-checkpoint.md)，若单账本落地其对齐语义将简化）。
- **P13-3 中断哨兵、P13-4 留存治理**待讨论。
...
- **2026-09-19 服务端接入挂起（✅ 自动化全绿，真机待跑）**：① 新增 `CHAT_SUSPEND_MEMORY_WINDOW`（毫秒，缺省 5 分钟，`0` = 一等人就挂起），删掉早就没人读的 `CHAT_APPROVAL_TIMEOUT_MS` / `CHAT_ASK_USER_TIMEOUT_MS`；② 在场心跳**刻意不**喂给框架的 `reportPresence`——沙盒只为等人续命 5 分钟，窗口被推过去之后「允许」会跑在睡着的沙盒上，理由见[技术方案 §6.3](../tech/chat-webapp.md)；③ 会话 DTO 加 `pendingDecisions`（裁决表未结清行数，一次分组查询），会话列表据此标「等你」。顺手把 OpenAPI 与生成的前端 client 重新生成了一遍（之前改文档路径时漏了），presence 路由的 summary 改用仓库根路径，免得生成器把相对链接抄到另一层目录里就断。新增 node-server 4 条、web 1 条用例。
- **2026-09-19 挂起之后卡片还能答（✅ 自动化全绿，真机待跑）**：框架上了[挂起与恢复](../../logic/orchestration/features/suspend-resume.md)之后，chat 服务端等人满 240 秒不再自动拒绝，而是挂起。前端原来的失效规则「卡片那一轮已收尾 ⇒ 已失效」会把挂起的卡片画成「已失效」，人回来答不了；挂起时发消息也会一直转圈。**修法全在 web 侧**，服务端与 wire 没动：从账本推出「还在等人」的调用、这些卡片不算失效、答完重开直播流、恢复那一轮开头没有 `start` 的 chunk 拿原消息副本当种子接着物化、在等人时发消息走排队。设计见[技术方案 §6.2](../tech/chat-webapp.md)，拆单与验证见[挂起与恢复 · 施工进展 S5](../../logic/orchestration/plans/suspend-resume.md)。新增 22 条用例，14 个变异全部被抓住。**同批订正**：功能手册与技术方案里「等满 4 分钟自动拒绝」「ask-user 超时返回提示文案」两处说法已过时，改成挂起。
...
- **2026-07-27 混合帧回放的顺序修复：崩溃轮跑到新轮后面去了（✅）**：用户实测截图——最新完成的那一轮显示在**上面**，22:14 那个被服务重启中断的旧轮显示在**下面**。根因在 `materialize.ts` 的 `MessageLedger`：渲染顺序由 `order` 数组决定，而 `order` 记的是**「upsert 到达的顺序」而不是 wire 顺序**。两种帧的物化时机差着一个微任务——`MessageFrame` 走 `upsert` **同步**落位，`ChunkEnvelope` 要经 `readUIMessageStream()` **异步**吐出消息才 upsert。于是回放一段「先是崩溃轮的 chunk 行、后是新轮的 message 行」的历史时，同步那批先把 `order` 占满，异步物化的旧轮消息只能排到末尾。**这个洞一直存在，只是没机会暴露**：崩溃的轮从前总是账本里的最后一轮（崩溃即终止），后面没有别的轮，排到末尾看不出问题；[优雅关闭](../../logic/orchestration/features/graceful-shutdown.md)让崩溃轮之后还能继续对话，它就浮出来了。**修法**：新增 `ensureOrder(id)`，在**同步**可见的 `start` chunk（它已经带着 `messageId`）那一刻就把位子占下，异步物化的 upsert 只填内容、不再改顺序；`snapshot()` 跳过「占了位但还没物化」的格子（刻意不塞空消息占坑——那会闪一个空气泡）。steer 消息那条路同样在 `start` 时占位。**验证**：web 276 全绿，新增用例同样做了**反向验证**（临时移除 `start` 处的占位 → 该用例变红 → 恢复 → 绿）。
...
- **P13-5（✅ 已落地，2026-07-15 定案项）**：UIMessage 单账本取代整套 wire 事件模型——`agent_events` 改 `kind`(message/chunk) + `runko` 标量 header 取代 `nimbo_state_json`；审批改走 ai 原生 `tool-approval-request`/`tool-approval-response` chunk + `onReview` 人审通道 + `tool-ask-user` 部件；人工裁决收敛为纯两值（「改参数」口子 2026-07-15 删除）；工具名一律 kebab-case。历史 wire 模型详见[技术方案 附录 A](../tech/chat-webapp.md)。
- **2026-07-17 表/列专业化更名（✅）**：`chat_sessions` → `conversations`、`agent_events` → `conversation_events`（子表按父实体命名；解开 "session" 三重超载——better-auth 登录态 / 对话线程 / SDK agent 会话，terms.md 主术语同步迁为「conversation（会话/对话）」）；`runko_*` 列 → `agent_session_*`、`telemetry_events.session_id` → `agent_session_id`（**库表命名不带产品名，产品更名不迁库**）；删除 `agent_events.type` 冗余列（payload 判别镜像、无代码读取）。迁移 `drizzle/0004_rename_conversations.sql`（SQLite RENAME 原地保数据）；API 路径 `/api/chat/sessions*` → `/api/chat/conversations*`，kubb 客户端与 openapi 重生成；代码标识符全链同步（server/web，含 TanStack 路由 `$conversationId`）。历史文档段落保留旧名如实记录。
- **2026-07-18 会话级授权 `allow-session`（✅）**：审批卡片加第三个按钮「会话内都允许」——用户批准一次后，本会话内**完全相同的调用**（同工具 + 入参指纹）不再弹卡片。全部落 chat 层、core 不动（分类器是回调、够不着 core 的 `review-once` once 记忆）：新增 `conversation-grants.ts`（按 (会话, 工具, 入参指纹) 记内存、稳定序列化保键序无关、进程重启即失效）；`onApproval` 分类前先查 `hasConversationGrant` 命中即 `allow`；wire `behavior` 加 `allow-session`，`resolveReview` 拿 `pendingReviews` 项里的 `toolName`/`input` 记账。刻意**按具体调用**而非按工具名——授权 `rm -rf build` 不放行别的危险 bash。测试：server `conversation-grants` 单测 + routes 端到端（allow-session 后第二轮相同命令无 tool-approval-request）；web 卡片第三按钮。server 217 / web 160 全绿。terms.md「会话级授权」词条、tech/features chat-webapp §6/审批段同步。
- **2026-07-18 会话级授权改持久化 + 记授权用户（✅）**：把授权从「内存耗材」改为**存 DB**——新增 `conversation_grants` 子表（PK `(conversation_id, user_id, grant_key)`，FK→conversations `ON DELETE CASCADE`，迁移 `drizzle/0005`）；`conversation-grants.ts` 从模块级内存 Map 改为薄 DB 访问（`grant`=INSERT OR IGNORE / `has`=SELECT / `clear`=DELETE，`onApproval` 每 gate 调用查一条带 PK 的 SELECT，本地 SQLite sub-ms）。**动机**：会话/工作区（快照+分支）本就跨重启持久，授权唯独易失、重启后重新询问只是摩擦；且粒度已窄到「精确命令+单会话」，持久化不显著扩大安全面。顺带化解了上一版「内存表无界增长」（现在是随会话增长的持久数据、会话删除即级联清，与 `conversation_events` 同）。**记 `user_id`（审批人）+ 按本轮发起者查**：为将来一个 conversation 多用户时「每人管自己的授权」铺数据（多用户卡片可见性/可点性属未来渲染层，不动此表）。测试：`conversation-grants` 单测改用真测试库 + seed，新增「按用户隔离」用例；server 219 全绿。terms.md 词条、tech chat-webapp §6 同步。

1. **v1 内存态 runner**：进程重启丢失内存态 turn 与内存态 pending 审批/提问（沙盒仍在跑但 runko loop 停）——DB 事件保留至崩溃点，重连回放发现无进行中轮即静默收尾；web 端「已失效」兜底。这是 v1 明示取舍，不在本期修。
