...
> 相关：[产品与使用手册](../features/telemetry.md) · [施工进展](../plans/chat-observability.md)（遥测属「chat 可观测性」拆单）· 依赖 [tech/chat-webapp](./chat-webapp.md)（§11 可观测性总览、注入落位）· [tech/single-ledger](../../logic/orchestration/tech/single-ledger.md)（账本侧计时数据 `data-tool-timing` 与消息 metadata 的定义）
