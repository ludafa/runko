...
**认出旧库就罢工**：库文件名与环境变量都不变（`DATABASE_PATH`，缺省 `data.db`）。第一次真的打开这个文件时，若发现里面有旧版（drizzle）的迁移记录表 `__drizzle_migrations`，**直接抛错**，错误信息里写清「这是旧版数据，新版表结构不兼容；确认不要了就删掉这个文件再起」。**不自动删、也不自动迁**：删数据这件事只能由人来做。

不加这道闸的后果是静默的：建表语句是「表不存在才建」，旧库里同名的表照样在，于是新代码在旧表上跑，缺哪一列要到某次查询才炸。

旧数据不搬（[施工 · 待确认](../plans/unified-demo.md#待确认) #1 已定）。真要留着的话，附录 B 里写了导入脚本要做哪几件事。
...
| 读的地方 | 读什么 | 换成 |
|---|---|---|
| `routes/chat.ts` 列表、详情 | 排队消息 | `runtime.listQueue(id)` |
| 同上 | 有没有轮在跑 | `runtime.getActivity(id).active` |
| 同上 | 几处等你答 | 一次分组查询数 `agent_decisions` 里未结清的行 |
| `push/notifier.ts` | 队列里还有没有下一条 | 数一下 `agent_queue` 的行数 |

**只有两处直接查框架的表**，都在 `agent/runko-tables.ts` 里，都只是数个数：

- 会话列表要显示「有几处在等你答」。走接口的话有几个会话就要问几次，一句分组 SQL 顶掉那一串往返。
- 推送那边要判断「队列里还有没有下一条」。它够不着框架的运行时（`push/` 不认识 `@runko/agent`），只能自己数。

其余一律走接口。**只数个数是安全的**：表结构是 persist-kysely 的公开契约（见它的 `schema.sql`），但绕开接口就绕开了它的语义（比如列里 `null` 与「键不存在」的翻译），数个数不涉及这些翻译。

排队消息与「有没有轮在跑」按会话各问一次框架（列表页有几个会话就问几次）。这两样都要经过框架的语义——队列那一列是 JSON，两种库的存法还不一样；「有没有轮在跑」还要先看本进程的登记册。为了少几次往返自己重写一遍，不划算。
...
- **旧库导入脚本**（已定不做）：如果哪天要保留 `data.db` 里的历史，可以写一个一次性脚本：账本只搬消息行，保留序号，时间乘 1000；裁决表一对一搬；队列拆成一条一行；租约水位设成旧表的最大序号，防止新序号撞上旧序号。better-auth 的表还要做日期和字段名的转换。估计 150–250 行。是否要做见[施工 · 待确认](../plans/unified-demo.md#待确认)。
