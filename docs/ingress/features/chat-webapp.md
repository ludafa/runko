...
5. **人在回路（[human-in-the-loop](../../terms.md)）**：
   ...
   - **一直不理会怎样**：卡片会一直等你，**不会被自动拒绝**。服务端先在内存里等 5 分钟（可配）；过了这个时间，这一轮就**[挂起](../../terms.md)**——已经做过的事都存好，服务端不再占着机器，但卡片**仍然可以点**。你几小时后回来答都行：答完，agent 从停下的地方接着跑，执行的就是当初请你批准的那条命令。沙盒如果已经休眠，会先唤醒，所以要多等几秒到几十秒，界面上显示「正在准备…」。提问卡片同理。
   - **挂起时页面上看到什么**：会话列表里这个会话标着「等你」；进去之后时间线上方有一行「有 N 处在等你答复」；那一轮的末尾写着「等待你的答复，这一轮已挂起」。答完之后，那行尾注变成一行灰字「在这里挂起过，答复之后已接着跑」。
   - **挂起时又发了一条消息**：它会进待发队列，等你把卡片答完、那一轮接着跑完之后再发出去。不想答了，就点「拒绝」。
   - **卡片什么时候变「已失效」**：这一轮一旦结束（跑完、你按了停止、服务重启中断），还挂着的卡片立刻显示成**已失效**并收起按钮——那时服务端已经不再等任何裁决了，点了也没用。**挂起不算结束**，挂起的卡片还能点。
...
3. **页面盯着不会延长等待**：[在场](../../terms.md)心跳只用来少发推送，不会让这一轮在内存里多等——沙盒只为等人续命 5 分钟，再等下去沙盒会先睡着，这时点「允许」反而跑不了。所以就算你一直开着页面，满 5 分钟也照样挂起；挂起不影响你随后作答。
