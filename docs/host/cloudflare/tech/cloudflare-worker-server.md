---
title: "Cloudflare Worker Server（技术方案）"
slug: cloudflare-worker-server
view: 技术
layer: 宿主层
module: 沙盒
...
> 依赖：[sandbox 技术方案](../../contract/tech/sandbox.md)（§6 [网关形态](../../../terms.md)与协议、§7 workerd 实测）· [core-sdk](../../../logic/engine/features/core-sdk.md)
> 相关产品：[沙盒 provider](../../contract/features/sandbox-provider.md)（Node 版服务端的 provider 选择，Cloudflare 仍是其非目标）
...
`getSandbox` 依赖每请求的 `env` binding，**拿不到模块级单例**。这正是 [`apps/node-server`](../../../ingress/tech/chat-webapp.md) 上 Workers 时必须改造的点之一（它的 `sandboxManager` 是模块级实例化的）。本示例的 `gatewayFor`/`workspaceFor` 都在请求处理函数内调用。
