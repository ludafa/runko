...
断线续传要求：先拿到「我断线期间错过的」，再接上「现在正在产生的」，而这两步之间**不能有缝**——有缝就会丢掉恰好落在缝里的那些内容。

落地形式是**「`subscribe` 同步返回」**：它返回时订阅已经挂上了，调用方据此写出「先挂订阅、再取[进行中草稿](../../../terms.md)快照」这段中间没有 `await` 的代码（Node 单线程，同步段内没有别的东西能插进来）。

> **所以 Redis 那档用裸 pub/sub 就够，不需要 Streams。** 「错过的那一段」由[账本](../../../terms.md)回放补（订阅方连上时带 `after=<看到的最后一个 seq>`），流分发只管「现在正在产生的」——这正是约束 ① 说的分工。让 Streams 也存一份历史，等于把账本的职责复制一遍，两份历史迟早打架。
> 实现见 [`@runko/stream-redis`](../../../../packages/stream-redis/README.md)：本地登记簿同步挂上，Redis 的 `SUBSCRIBE` 在后台补；订阅慢一拍漏的那几帧由回放补。

**③ 实现要说清自己能不能跨进程（`crossInstance`）。**

这是一个可选的只读标志：`true` = 别的进程发的帧这里也收得到（Redis 那档），缺省 `false` = 只管本进程（内置那档）。

框架只拿它决定一件事：**一轮跑在别的副本上时，本副本的订阅要不要继续等**。

| | 订阅方在非持有者副本上连上来 |
|---|---|
| `false` | 发两帧权威快照就收线——等也等不到。[接入层](../../../terms.md)要么把请求转给持有者，要么让客户端重连 |
| `true` | 留着等。内容马上从别的副本广播过来；那一轮收尾时的 `activity: false` 同样会到，收线时机与本地一致 |

漏了这个标志的症状很隐蔽：配了 Redis 的多副本部署里，连到非持有者副本只收到两帧快照流就关了，正在跑的那一轮一个字也看不到，而且不报错（2026-09-22 在[集群实验环境](../../node/features/cluster-lab.md)里实测到）。
