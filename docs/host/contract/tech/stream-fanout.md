...
**`true` 这一档框架自带两道兜底**，因为「收尾那一帧一定会到」并不成立：持有者被 `kill -9`（没人接管）时没有任何进程会发它，订阅慢一拍也可能把它漏掉。两道兜底是——① 回放期间收到的收尾广播，即便帧本身被丢弃，它带的信息也会留下（否则正好落在「远端已广播、尚未释放归属」那段窗口里时，订阅方会去等一帧永远不会再来的帧）；② 跟远端时定期复查一次归属，归属没了就自己补一帧 `activity: false` 收线。实现见 `packages/agent/src/runtime.ts` 的 `subscribe`。
