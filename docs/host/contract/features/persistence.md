...
>
> **状态：领域接口已定稿**（2026-08-16 随 `@runko/agent` 交付，见 `packages/agent/src/persistence.ts`）。
> 官方实现包首发 `@runko/persist-sql`，施工见[施工进展](../plans/persistence.md)。
...
拿 runko 建产品的人，应用里本来就有自己的数据库、自己的 ORM、自己的领域模型。如果 runko 硬绑一种存储，他要么被迫多养一个数据库，要么放弃自己那套 schema 管理。
...
| 你的情况 | 装哪个 | 你要给它什么 | 有了吗 |
|---|---|---|---|
| 只是本地跑跑、不要求重启后还在 | **什么都不用装** | 内置内存实现，零配置 | ✅ 随 `@runko/agent` |
| 用 SQLite，只有一个驱动 | `@runko/persist-sqlite` | 一个 `better-sqlite3` 实例 | ✅ |
| 用 PostgreSQL，只有一个驱动 | `@runko/persist-postgres` | 一个 `pg.Pool` | ✅ |
| 用 MySQL，只有一个驱动 | `@runko/persist-mysql` | 一个 `mysql2` 连接池 | ✅ |
| 用 **MongoDB** | `@runko/persist-mongo` | 一个 MongoDB `Db` | ✅ |
| 已经在用 **Kysely** | `@runko/persist-kysely` | 你已有的 Kysely 实例 | ✅ |
| 已经在用 drizzle | `@runko/persist-drizzle` | 你已有的 drizzle 实例 | ⬜ 未开工 |
| 已经在用 Prisma | `@runko/persist-prisma` | 你已有的 Prisma client | ⬜ 未开工 |
| 跑在 Cloudflare Durable Object 上 | `@runko/durable-object` | 什么都不用给（用 `ctx.storage.sql`） | ⬜ 未开工 |
| **你的 schema 跟哪个都对不上** | **什么都不用装，自己实现三个接口** | —— | ✅ 头等路径，见下 |

### 一种库一个包

**每种数据库是独立的包，各自只 peer-dep 自己那个驱动。** 装 `@runko/persist-postgres`
...
直接装核心**——runko 的四张表和你自己的表就在同一个实例、同一套迁移之下。
...
`@runko/persist-mongo` **不是薄壳**——Kysely 是 SQL 查询构建器，Mongo 用不上，所以它
直接实现三个领域接口。
...
> **2026-08-23 推翻了原来的「方言不是包」。** 原方案是一个 `@runko/persist-sql` 同时支持
...
- **迁移是你自己跑的。** 官方实现自带迁移脚本，但**迁移不是接口的一部分**——runko 不假设你的存储「有迁移这回事」。
- **runko 不碰你的用户表。** 它只认一个不透明的 `ownerId` 字符串，不建外键、不管这个 id 从哪来。你的用户体系是你的。

## 4. 范围与非目标

**不做的**：

- **不提供 ORM，也不替你管连接池**——你给实例，runko 用。
...
- **SQLite / PostgreSQL / MySQL 之外的方言**。这三种官方支持；其余走「自己实现接口」那条路，或者自己配一个 Kysely dialect 再用 `@runko/persist-kysely`。
...
别装薄壳，直接装核心，把你自己的实例给它——runko 的四张表和你的表就在同一个实例、
同一套迁移之下：
...
固定叫 `agent_ledger` / `agent_decisions` / `agent_queue`，**不提供前缀开关**。
...
不想用它、想让 runko 的表进你自己的迁移体系也行：表结构就是 `RunkoDatabase` 那三个
...
2. **宿主能把这几张表装进自己的库**：chat 应用（`apps/node-server`）与自己的业务表共用一个 Kysely 实例，从空库开始建表 → 跑几轮 → 重启进程 → 历史还在、队列还在，SQLite 与 Postgres 各跑一遍。
3. 不装任何持久化包时，`@runko/agent` 仍能直接跑起来（内置内存实现不受影响）。
