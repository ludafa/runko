...
> 相关：[技术方案](../tech/sandbox.md) · [施工进展](../plans/sandbox.md) · 依赖 [core-sdk 功能](../../../logic/engine/features/core-sdk.md)（本功能是 core 的 `NimboFS`/`NimboExec` 接口的一组官方适配器实现）
>
...
nimbo 官方提供三个开箱即用的适配器包：`@nimbo/sandbox-e2b`、`@nimbo/sandbox-vercel`、`@nimbo/sandbox-cloudflare`。任何满足能力要求的沙盒，也都可以照契约自己写适配器接入（见[技术方案](../tech/sandbox.md) §检查清单）。
