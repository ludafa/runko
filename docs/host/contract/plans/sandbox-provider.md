...
> 依赖：[chat-webapp](../../../ingress/features/chat-webapp.md)（`sandbox-manager.ts`、`routes/chat.ts`、DB schema）· [sandbox](../features/sandbox.md)（两个适配器包）
