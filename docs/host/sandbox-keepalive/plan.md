...
> 产品文档 [docs/host/sandbox-keepalive/feature.md](./feature.md)，技术方案 [docs/host/sandbox-keepalive/tech.md](./tech.md)。
...
- `docs/host/sandbox-keepalive/feature.md`、`docs/host/sandbox-keepalive/tech.md`、本文件。

- `docs/agent/turn-checkpoint/tech.md` §5 顶部加了「本节已被取代」说明（写清推翻了什么、保留了什么）。
...
**⚠️ 与计划的偏差**：查剩余用的是 **`sandbox.expiresAt`**（`get expiresAt(): Date | undefined`），**不是计划里写的 `sandbox.timeout`**。核 `@vercel/sandbox@2.5.0` 的 d.ts 发现后者的官方注释是 "The **default** timeout of this sandbox"——建盒时配的时长，不是剩余量。原始出处 [turn-checkpoint](../../agent/turn-checkpoint/tech.md) §5 那一处不准，技术方案 §1 已加订正。
...
**结论回填**：跑完把实际结果写回本节，与 [docs/core/verification/plan.md](../../core/verification/plan.md) 的既有格式对齐。
...
- **P13-2b 的"保活不进 nimbo SDK / 适配器"被推翻**（原文见 [turn-checkpoint](../../agent/turn-checkpoint/tech.md) §5）。理由见技术方案 §2.1：知识归属、补足语义只有适配器做得干净、"归宿主"指的是所有权而非执行。**保留** P13-2b 的补足语义与单轮上限设计。
