...
> 产品文档见 [docs/host/sandbox-keepalive/feature.md](./feature.md)，施工进展见 [docs/host/sandbox-keepalive/plan.md](./plan.md)。
...
[turn-checkpoint](../../agent/turn-checkpoint/tech.md) §5（P13-2b，方案定稿未施工）原本规定保活"封在 chat 应用的 `SandboxClient` 实现里，**不进 nimbo SDK / 适配器**"，理由是 [BYO 实例](../../terms.md)原则里"超时延长点名归宿主"。
...
5. **E2B 有硬上限**：Pro 24 小时、Hobby 1 小时。`idleTimeoutMs` 超了 `setTimeout` 会报错。
6. **只保证沙盒还活着**，不保证沙盒里的命令做了什么。
7. **不做自愈**。续期失败只记录，不重连不重放。理由见 [sandbox-provider](../sandbox-provider/tech.md) §5.1。
...
施工中与本文档原稿的三处偏差（闸门落点、`expiresAt` vs `timeout`、判定线取 3/4）已就地订正在上面对应小节，来龙去脉见 [施工进展](./plan.md)的变更记录。

## 10. 相关文档

- [docs/host/sandbox-keepalive/feature.md](./feature.md)：产品文档。
- [docs/host/sandbox-keepalive/plan.md](./plan.md)：施工进展。
- [docs/host/sandbox/tech.md](../sandbox/tech.md) §4.6：沙盒生命周期与会话恢复的既有立场。
- [docs/host/sandbox-provider/tech.md](../sandbox-provider/tech.md) §5.1：这个问题的第一次分析（缓存失效与保活心跳）。
- [docs/agent/turn-checkpoint/tech.md](../../agent/turn-checkpoint/tech.md) §5：P13-2b 保活方案，本文档取代其定位部分、保留其补足语义与单轮上限。
- [docs/core/core-sdk/tech.md](../../core/core-sdk/tech.md)：core 的接口分层。
