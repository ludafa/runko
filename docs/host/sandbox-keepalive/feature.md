...
> 技术方案见 [docs/host/sandbox-keepalive/tech.md](./tech.md)，施工进展见 [docs/host/sandbox-keepalive/plan.md](./plan.md)。
...
真机实测的数字（来自 [docs/host/sandbox-provider/tech.md](../sandbox-provider/tech.md) §5.1）：
...
- **不重连、不重建。** 沙盒真的没了要怎么办，是宿主的策略——重建意味着重新拉代码、装 skill、切分支，SDK 不知道这些。见 [docs/host/sandbox/tech.md](../sandbox/tech.md) §4.6。
- **不自动重试失败的命令。** 续期失败只记录，不做任何补救。理由见 [docs/host/sandbox-provider/tech.md](../sandbox-provider/tech.md) §5.1 关于"workspace 自愈代理"的分析：命令跑到一半断线后重放会重复执行，`git push` 这类操作会出事。
...
- [docs/host/sandbox-keepalive/tech.md](./tech.md)：技术方案。
- [docs/host/sandbox-keepalive/plan.md](./plan.md)：施工进展。
- [docs/host/sandbox/feature.md](../sandbox/feature.md)：沙盒适配契约总纲。
- [docs/host/sandbox-provider/feature.md](../sandbox-provider/feature.md)：chat 应用的多 provider 支持，§5.1 是这个问题的第一次分析。
- [docs/agent/turn-checkpoint/feature.md](../../agent/turn-checkpoint/feature.md)：P13-2b 保活方案的原始出处，本文档取代其中的保活部分。
