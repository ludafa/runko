...
**其二：SSE 那条直播流不再转发给持有者**。转发中间件原本挂了六条路径，`…/stream` 在有 Redis 时要摘掉——留着的话，一个本可以就地答的订阅会被白转一跳，持有者崩了还会跟着 503。

| 请求 | 单副本 / 无 Redis | 有 Redis |
|---|---|---|
| `GET …/stream`（SSE） | 转给持有者 | **不转**，就地订阅 |
| `GET …/ws`（WebSocket） | **从来不转**——升级握手没法转发 | 不转，就地订阅 |
| 发消息、停止、队列删、审批、提问 | 转给持有者 | 一样转 |

**WebSocket 那条从来就不在转发清单里**：协议升级发生在 HTTP 服务器那一层，没法像普通请求那样把响应原样转回来。所以**多副本下想用 WebSocket 看直播，就必须配 Redis**——没有广播，连到非持有者副本就只看得到已经落盘的部分。
