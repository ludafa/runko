...
> 依赖：[chat-webapp](../../app/chat-webapp/feature.md)（`sandbox-manager.ts`、`routes/chat.ts`、DB schema）· [sandbox](../sandbox/feature.md)（两个适配器包）

> **立项 banner**：2026-07-19 用户提出「在 web 集成 E2B 沙盒，让用户自选 e2b/vercel」→ 调研 + 三决策定案 → 立项。
> **调研结论与决策**见本页「变更记录」；核心差异对照见[技术方案 §1](./tech.md)。
