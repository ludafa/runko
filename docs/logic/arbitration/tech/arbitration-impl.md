...
### 4.4 同名重启：自己上一辈子的租约立刻算陈旧

**陈旧判据有两支**：心跳超过[接管阈值](../../../terms.md)没续，**或者**这条租约挂在我自己的名字下、而它最后一次心跳早于我这个进程启动的时刻。

第二支解决的是重启：进程崩了马上重新起来，上一辈子留下的租约心跳还很新鲜，按第一支要干等一个接管阈值（缺省 60 秒）那条会话才能再用。而一个**刚起来的**进程不可能已经在跑任何一轮，所以挂在自己名下、比自己还老的租约，一定是上一辈子留下的——启动扫描直接收拾掉。

三处必须用同一条判据：`listStale`、`clearStale`、`acquire`。`recover()` 的顺序是 `listStale → clearStale → acquire`，判据不一致时后一步会清掉前一步刚拿到的令牌。

**前提是每个进程的 `holder` 唯一**（k8s 里同一个 pod 重启后名字不变，正合适）。万一两个活着的进程配成了同一个名字，后起来的那个会把先来的那一轮抢过去；先来的手里那个租期标识已经作废、写不进账本，所以坏的是那一轮，不是数据——这正是 §4.2 里「令牌管不写坏」的分工。

内存版没有这一支：它的归属表跟进程同生共死，看不到上一辈子的任何东西。
...
- **崩溃仍然不恢复**：进程被强杀走老路（补一条「已停止」），因为它没停在干净边界上。
  这条标记由两处补：进程启动时的 `recover()`，以及**多副本下先接手的那个副本**——租约版抢占时
  若顶掉了过期持有者，`acquire` 会带上 `takeover`，轮编排据此先替上一轮收尾。见
  [多副本部署 · 技术方案 §9](../../../host/node/tech/multi-replica.md)。
