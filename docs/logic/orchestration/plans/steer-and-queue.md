...
> 相关：[功能](../features/steer-and-queue.md)，[技术方案](../tech/steer-and-queue.md)。
> 依赖/延续：[chat 聊天 webapp](../../../ingress/features/chat-webapp.md)（`routes/chat.ts` / `turn-runner.ts` / `use-chat-messages.ts` 是本期主战场）· [UIMessage 单账本](../features/single-ledger.md)（[账本](../../../terms.md)不变，队列刻意在账本之外）。
