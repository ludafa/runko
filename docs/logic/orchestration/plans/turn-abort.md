...
- **第一批（A0–A7，跑到中途按停止）**：代码完成、自动化全绿（2026-07-25）；真机端到端 2026-08-23 大部分已跑（A7 那格）。
...
| A7 | 真机端到端 | 🟡 **大部分已跑**（2026-08-23，随[轮编排运行时](./agent-runtime.md)的复跑一并覆盖）——7 条里 1/3/4/5/6 有实证、2 部分覆盖，7 仍待跑（要界面）。见下「验证方案」 |
...
| B6 | 真机端到端（成功标准 8–12） | 🟡 **服务端侧已跑**（2026-08-30）——8/9/10/11 有实证，界面侧那几半与 12 待跑。见下「B6 真机实测结果」。标准 8 的 ack 值已过期，见「B6 的成功标准 8 已过期」 |
...
### B6 真机实测结果（2026-08-30，服务端侧）

会话 `e66d2b3b`（**E2B 沙盒处于 `sleeping`，所以每条消息都要先唤醒——装配窗口天然是几秒，好掐点**）。web 那一侧当时没起，所以界面观感项没跑。

| 标准 | 结论 | 实测到的 |
|---|---|---|
| **8** 发完立刻停 | ✅ 服务端侧 | `POST .../messages` **89ms** 就返回（`mode:"started"`），停止发在 ack 之后 **64ms**——铁定在装配窗口里。这一轮**从没启动**：账本只多了一行孤立的 user 消息（`seq=45`，没有配对的 assistant），收尾 `status:'interrupted'`，[起轮标记](../../../terms.md)清空。「刷新页面后一致」那半要界面，未跑 |
| **9** 冷启动沙盒上做同样的事 | ✅ 服务端侧 | 正是本次的场景。停止发出到真正落成 `interrupted` 隔了 **7.5 秒**——等装配走到检查点才收尾，**没有假装已停**。对照组：一轮已经在跑时按停止只要 **39ms**（A7 第 1 条）。界面上「停止中」那半要界面，未跑 |
| **10** 立刻再发一条 | ✅ | 窗口内第二条 → `mode:"queued"`、HTTP 202，没有起第二轮。「第一轮跑完自动接上」由[轮编排复跑](./agent-runtime.md)第 2 组另行验过（交棒 1.06 秒） |
| **11** 立刻插话 | ✅ | 窗口内 `intent:"steer"` → `mode:"queued"`、**HTTP 202 而不是 409**，转排队正确。「不留悬空回显」那半要界面，未跑 |
| **12** 装配失败后仍能起轮 | ⬜ 待跑 | 要故意配错沙盒凭据 |

三条请求在 **83 毫秒**内打完（`20:57:52.312 / .358 / .395`），全部落在同一个装配窗口内。

### B6 的成功标准 8 已过期（2026-08-23 发现）

标准 8 原文要的是「装配窗口里按停止 → 202 `mode:'aborted'`」。**这个 ack 值在[轮编排运行时](./agent-runtime.md)落地之后已经不可能出现了**：

- 框架的 `EnqueueResult` 只有 `started` / `queued` / `steered` / `rejected` 四种（`packages/agent/src/types.ts`），没有 `aborted`；
- 根因是语义变了——[起轮占位](../../../terms.md)现在是装配的**第一件事**，所以 `POST .../messages` 一登记就返回 `started`，不再等装配走完；老实现是 ack 卡在装配里，才有机会把它改判成 `aborted`。

**但 `StartTurnAckSchema` 与 OpenAPI 描述里那一档还留着**（`apps/node-server/src/schemas/chat.ts`、`routes/chat.ts` 的 202 描述），是一段死契约。

行为本身没退化：装配窗口里按停止，这一轮照样停、「已停止」照样走[直播流](../../../terms.md)送达——变的只是那条 ack 说什么。所以 B6 要么按新语义重述标准 8（ack 是 `started`，判定看流上的收尾帧），要么先把死契约摘掉。**两条都还没做。**
...
**真机实测结果（2026-08-23，真 E2B + 真模型，会话 `e66d2b3b`）**——随[轮编排运行时的复跑清单](./agent-runtime.md)一并跑的，逐条对上面七项：

| # | 结论 | 实测到的 |
|---|---|---|
| 1 | ✅ | 长轮（`sleep 120`）跑到 3 秒时按停止：按下 20:20:51.744 → `status:'interrupted'` 20:20:51.783，**39 毫秒**；末尾「已停止」 |
| 2 | 🟡 部分 | 轮进行中刷新页面验过（历史一致、进行中内容重现）；**「停止之后再刷新」这一条没单独跑** |
| 3 | ✅ | 排 2 条（X/Y）再按停止 → 待发区清空、两条都没被执行、此后无自动起轮 |
| 4 | ✅ | `rm -rf` 进审批卡片 → 按停止：按下 20:21:46.270 → `interrupted` 20:21:46.307，**37 毫秒**，远小于 240 秒；裁决表结清并写下理由 |
| 5 | ✅ | 停止后紧接着发下一条，新一轮正常起（`turnActive=true`），没有锁死 |
| 6 | ✅ | 连点停止 5 次（2026-08-30）：第 1 次 200 生效并清空队列，后 4 次干净返回 **409「no turn in progress」**，无报错、无重复副作用 |
| 7 | ⬜ 待跑 | 另开标签页。同时开两条流验过（协议层等价），但没验界面观感 |
