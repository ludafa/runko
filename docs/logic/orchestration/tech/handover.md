...
| 闸门放行转发来的请求 | **去掉** | 交接几百毫秒就完成，之后没有请求需要转给旧节点（G10）。去掉后验证环境才能复现 F1 |
| 配了 Redis 广播时立刻断直播，否则等轮收尾后再断 | **改成**：每份对话交接完成后就发[请重连帧](../../../terms.md)、断这份对话的直播（§8） | 不管有没有 Redis，交接完成后新持有者都能播 |
| [等待窗口](../../../terms.md)（`finishWindowMs`） | **过渡保留**，按阶段交权落地后**直接删除**（不经过废弃期，`@runko/agent` 随之 major） | 它就是附录 A 第 2 条否决的做法（G8、G9、G11）。在本方案落地之前，它仍比「立刻中止」好，所以不急着删 |
