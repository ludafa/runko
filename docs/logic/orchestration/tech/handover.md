...
> 依赖/延续：[优雅关闭与崩溃恢复](./graceful-shutdown.md)（本方案取代它「关闭时一律中止」的做法）· [挂起与恢复](./suspend-resume.md)（工具收尾借用它的恢复路径）· [归属仲裁机制](../../arbitration/tech/arbitration-impl.md)（租约多一种「预留」状态）· [多副本部署](../../../host/node/tech/multi-replica.md)（本方案建在它之上）· [唯一 demo](../../../ingress/tech/unified-demo.md)（node-server 要先走完它才具备多副本的前提）。
> 术语一律以 [术语表](../../../terms.md) 为准。本文新增的词：[指定交接](../../../terms.md)、[节点登记表](../../../terms.md)、[发布序号](../../../terms.md)、[交接预留](../../../terms.md)、[工具收尾](../../../terms.md)、[已交权](../../../terms.md)、[请重连帧](../../../terms.md)、[定时回捞](../../../terms.md)、[待接手](../../../terms.md)、[下线中](../../../terms.md)。
...
- 任何「节点状态表」都是写进去之后别人才读得到，**一定有时差**。同一批下线的节点可能在几毫秒内先后收到 SIGTERM，彼此看到的对方状态还是「正常」。
...
| 应用层转发的示范、跨容器故障验证 | `apps/persist-demo` | ✅（将并入 node-server） |
| 崩溃后补「已停止」（启动扫描 + 接管时） | `runtime.recover()` · `runtime/queue.ts` | ✅ |
| 恢复轮：用 `Settlement` 结清一个悬空调用再接着跑 | core 的 `settleAndRun` | ✅ |

### 3.2 缺口（2026-09-25 读代码 + 临时测试确认）

| # | 缺口 | 怎么确认的 | 本方案里怎么消掉 |
|---|---|---|---|
| G1 | 关闭时**正在干活的轮被立刻中止**，用户看到「中断」 | 读代码（`shutdown()` 对非等人轮一律 `abortTurn`） | §6 按阶段交权 |
| G2 | 关闭中止后，**待发队列里的消息没人推**：新进程 `recover()` 扫不到，浏览器重连也不触发；直到用户再发一条，而且新的那条会**插到前面先跑** | 临时测试 P1 复现 | §7 指定交接 + §10 兜底 |
| G3 | 关闭那几秒里**答了审批，答案落库、接口返回成功，但没人起恢复轮** | 临时测试 P2 复现 | 同上 |
| G4 | 关闭中止时，**已告知「插进去了」但还没交给模型的插话丢失**（挂起时有转进队列的逻辑，中止时没有） | 临时测试 P3 复现 | §6：交权不再中止；中止路径另补 |
| G5 | web 判断「服务重启」的文案常量与 `@runko/agent` 的 `ABORT_REASON_SHUTDOWN` 对不上，界面显示成「已停止」 | 读代码（`turn-marker.tsx` 逐字比较） | 施工 H0 |
| G6 | node-server 只在 `server.close()` 的回调里退出；还有长连接开着时**进程可能一直退不掉**（`node --watch` 会无限等） | 读代码推断，未实测 | §5 退出条件 |
| G7 | **node-server 今天不能做任何形式的多进程并存**：手写的仲裁把库里所有「正在跑」标记都当孤儿，新进程启动会打断旧进程还活着的轮 | 读代码（`agent/persistence.ts` 的 `listStale`） | 先走完[唯一 demo](../../../ingress/plans/unified-demo.md) U1–U5 |
| G8 | 文档与代码对「交权」说法不一：术语表写「跑完当前这一轮再放手」，代码是「立刻中止」 | 读文档 + 代码 | 本方案重新定义交权，旧说法一并改掉 |
...
|---|---|---|---|---|---|
| 正常服务 | — | 开 | — | 开 | — |
| **交接** | SIGTERM | **关**（F1） | 保留 | 开 | 标「下线中」→ 对每份对话：按阶段落账 → 预留 → 请接手 → 踢长连接 |
...
- 接手节点抢到归属后看到账本末尾悬空，**只等结果，不调模型**。这段时间里用户发的新消息进待发队列（已有行为）。
- 工具跑完，旧节点把结果写进那一行（**只在它还空着时写得进**），再通过出站通知接手节点。
...
| `state` | `ready` / `leaving`（[下线中](../../../terms.md)） |
...
这一步是一次条件更新（CAS）：`WHERE conversation_id = ? AND lease_token = <我的令牌>`，把持有者与令牌清掉、写上 `reservedFor` 与 `reservedUntil`。对应到[归属仲裁的接口](../../arbitration/tech/arbitration-impl.md)：`Grant` 多一个方法「放手并预留给某节点」，`acquire` 在预留期内只放行被预留的那个节点。**轮编排仍然不认识租期标识**（那份文档 §2 的约束一），它只知道「把归属交给 X」。
...
- 旧节点在**这份对话交接完成之后**，在已有的直播流上发一帧[请重连帧](../../../terms.md)，然后自己关掉这条连接（F6）。
- 浏览器收到它：**不走**现在的 1、2、4、8、16 秒退避，而是立刻重连，失败就每 200～300 毫秒再试，最多试若干秒，期间界面不提示。真正的断线仍然走原来的退避。
- 重连经入口落到接手节点（或落到任何节点再被转发过去），拿到的是回放 + 当前草稿，**整体替换**浏览器手上的旧草稿——于是模型输出段被扔掉的那半段字自然消失。
...
- **[定时回捞](../../../terms.md)**：每个节点定时（比如每 10 秒）找「没人持有、也没有有效预留，但有活没干完」的对话：待发队列不空、裁决表里答了但没恢复、收尾记录里有结果但没结清。它只碰**预留已过期或根本没有预留**的对话，所以不会抢指定交接的活。它同时补上了 G2、G3 这类「有活但没人推」的老洞。

## 11. 数据模型

```mermaid
erDiagram
    AGENT_NODES ||--o{ AGENT_LEASES : "持有 / 被预留"
    ...
    AGENT_LEASES {
        ...
        bigint heartbeat_at
        bigint acquired_at
        string reserved_for "新增，可空：预留给哪个节点"
        bigint reserved_until "新增，可空：预留截止时刻"
        bool awaiting_takeover "新增：没挑到接手节点，等新进程扫"
    }
    AGENT_TOOL_TAILS {
        string conversation_id PK
...
`AGENT_TOOL_TAILS` 可以单独建表，也可以复用裁决表加一种类型——见[施工 · 待确认](../plans/handover.md#待确认)。
...
sequenceDiagram
    autonumber
    participant A as 旧节点 A（序号 41，下线中）
    participant R as 节点登记表
...
| 2 | 跑完当前这一轮再放手（旧的「交权」定义） | agent 一轮常常好几分钟，宽限期要设得很长，发布变慢；而且这段时间里旧节点入站已关，这份对话的请求全部失败（F1） |
...
2026-09-25 在 `@runko/agent` 上写了三条临时测试（跑完即删，未提交），用共享的 `memoryPersistence()` 模拟「旧进程关闭、新进程起来，共用一个库」：
