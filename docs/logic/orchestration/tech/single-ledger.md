---
title: "单一数据账本（single-ledger）— 技术方案"
slug: single-ledger
view: 技术
layer: 逻辑层
module: 轮编排
...
> **依赖**：本方案改造 [chat-webapp](../../../ingress/tech/chat-webapp.md) 的持久化层。下游的 [上下文压缩（compaction）](../../engine/tech/compaction.md) 在本账本上再加第三种条目。
...
> **`data-tool-timing` 语义定案**（chat 可观测性，2026-07-16；同日按用户反馈修订加入 `executionStartedAt`）：每个工具调用一条独立 id（`id = toolCallId`），物化方式照 `data-plan-update` 先例（同 id 覆盖），差别只是 id 不唯一。三个时刻对应调用的三段生命周期——loop 对同一步的多个调用默认**串行**结算，仅当整批全部 `Tool.readOnly`（纯读工具，见 [tech/core-sdk](../../engine/tech/core-sdk.md) §4.1 与 [tech/builtin-tools](../../engine/tech/builtin-tools.md)）时并行（`runOneStep` 的 settle 分支）；串行批的排队真实存在，并行批的 `executionStartedAt` 几乎同刻：
>
...
> **目标态会变：纯内存路由只覆盖「等人的前几分钟」。** 架构定案后等人走**混合模式**——先在内存里等一段（默认沿用[审批保活预算](../../../terms.md) 5 分钟），**等不到就[挂起](../../../terms.md)**：这一轮落盘退出、释放归属，人几小时后回来在**任意节点**恢复。这要求裁决**落到一张人工裁决表**里，而不是只挂在某个进程的 promise 上。
>
> 配套两条定案：
>
> - **裁决表是纯审计表**——会话此刻是什么状况**从账本推**，不存状态字段（[账本即事实](../../../architecture/tech/agent-kernel.md)）。裁决表只回答「这次调用批没批、谁批的、什么时候」。
> - **恢复时必须用账本里原封不动的参数执行**。走「告诉模型你被批准了、你再发一次」那条路，模型可能发出参数不同的命令——用户批准的和实际跑的不是同一条，审批闸门形同虚设。
>
> 完整设计见 [架构总纲 · 技术方案 §3 · §4](../../../architecture/tech/agent-kernel.md)。
...
> 本节是现行实现（`@runko/agent` 的 `consumeStream` / `finalize`），上面 §6 的图是 chat 应用单进程时代的写法，留作对照。

**规矩**：一轮的**收尾帧**（`message-metadata`，前端据它把这一轮标成完成 / 失败 / 已停止）**必须排在这一轮的成品消息帧之后**。core 在生成器的最后一帧交出收尾帧，agent **先扣住不发**，等 `finalize()` 把成品消息写进账本、广播完，再发它。

**为什么**：前端一收到收尾帧就把这一轮当成结束，输入框可以发下一条。收尾帧要是先到，中间有一段「前端以为结束了、服务端还在写库」的空档（时序见附录 A）：

- 用户在空档里发下一条，前端为新一轮换连接，还没送到的成品消息帧就收不到了，上一轮的回复在前端账上一直是「半截」的。
- 写库失败时，用户已经看到「完成了」，刷新后回复却没了。

```mermaid
sequenceDiagram
    autonumber
    participant C as core（session.stream）
    participant A as agent（consumeStream）
    participant D as 账本
    ...
    C-->>A: 收尾帧（message-metadata）
    A->>A: 扣住，不发
    C-->>A: 生成器结束
    A->>D: finalize：逐条写入成品消息
    alt 全部写进去了
        A-->>B: 成品消息帧（带 seq）
        A-->>B: 扣住的收尾帧
    else 没写全（库出错、被拒、归属丢了）
        A->>A: 记日志（具体原因只在这里）
        A-->>B: 写进去的那几条的成品消息帧
        A-->>B: 改发「失败」收尾帧：internal_error
        A->>D: 归属还在的话，尽量补一条失败标记（写不进也不再抛）
    end
```

**落库失败报什么**：状态 `failed`，错误码 `internal_error`（系统异常），`message` 是一句不含细节的通用话。存储层到底出了什么错**只进日志**，不给用户看——用户能做的只有重试，细节对他没用。

**先发帧、再补标记**：库出错时补标记可能要等到连接超时，用户不该跟着干等。**归属已经丢了就不补**：比如节点连不上库、自我围栏停了手，这份对话已经不归它管，再碰库只会把收尾卡住；接手的节点会补「已中断」。

**失败标记写不进去怎么办**：多半是库连不上，这时记日志、不再抛——收尾剩下的几步（广播「没有轮在跑了」、放手）照常跑完，不然这份对话会被锁死。代价是这一轮在账本里只剩起轮那条用户消息、没有下文：还连着的用户看到了「系统异常」，刷新之后只看到自己那句话、没有回答，接着发消息可以继续。

**扣住期间断线重连的订阅者**：`finalize` 一开头就清空[进行中草稿](../../../terms.md)，重连回放拿到的是「已经写进账本的」加上随后广播的成品消息帧和收尾帧，不会看到半截。
...
## 附录 A · 收尾帧先于成品消息时的空档（已修）

这是 §6.1「先存好，再说完成」要解决的问题，留作说明。两个轮本身不会重叠，重叠的是「第 1 轮的成品消息帧还在路上」和「第 2 轮的连接已经开了」这一小段空档。

**修之前，第 1 轮收尾时服务端按这个顺序发帧：**

1. 模型生成的文字，一段段推给前端（增量帧）。
2. **收尾帧**（`message-metadata`）：告诉前端「这一轮完成了」。前端在这一步就切成空闲，输入框可以发下一条。
3. 服务端把助手回复**写进账本**，再推**成品消息帧**（`MessageFrame`，带 seq）。写库要花时间，这一帧晚一点才到。
4. 「没有轮在跑了」，关流。

前端的规则是：**没收到成品消息帧的消息，都算半截回复。**第 2 步和第 3 步之间，第 1 轮的回复其实已经完整了，但在前端的账上还是半截的。
...
sequenceDiagram
  ...
  participant S as 服务端（跑第 1 轮）

  Note over F,S: 流① 开着，属于第 1 轮
  S-->>F: 文字增量帧 …（流①）
  S-->>F: 收尾帧：这一轮完成了（流①）
  F->>F: 切成空闲，输入框可用
  Note over F: 第 1 轮回复还没收到成品消息帧，<br/>在前端账上算半截回复

  rect rgba(255, 170, 60, 0.18)
    Note over U,S: 空档：前端以为结束了，服务端还在写库
    U->>F: 马上发第 2 条
    F->>S: POST /messages（普通请求）
    S-->>F: 202，第 2 轮已起
    F-xS: 开流② 之前，先掐掉流①
    S->>S: 把第 1 轮回复写进账本
    S--xF: 成品消息帧：流① 已断，送不到
  ...
  Note over F,S: 流② 开着，属于第 2 轮

  alt 每次开新流都丢半截回复
    F->>F: 第 1 轮回复被当成半截回复删掉，页面上消失
  else 只在真断线重连时丢
    F->>F: 第 1 轮回复保留在页面上
  end
```

- 第 7 步（掐掉流①）和第 8、9 步（写库、发成品消息帧）谁先谁后不一定，看写库和网络哪个快。只要成品消息帧晚于掐断流①，就会落进这个空档。
- 开新流时前端会**先掐掉旧的流**（`use-chat-messages.ts` 的 `startTail()` 开头就这么做），防止两条流同时往页面里写。

**两种开新流，处理不一样：**

| 开新流的原因 | 丢不丢半截回复 | 为什么 |
|---|---|---|
| 真断线重连（网断了，或节点下线让你重连） | **丢** | 半截回复没存过，新节点会从头重写（时序见[交权 · 技术方案 §8](./handover.md)） |
| 开新一轮（上一轮刚结束你就发了下一条） | **不丢** | 手上的「半截」多半是已经完整、只是成品消息帧还在路上的回复；丢了它，页面上上一轮的回复就没了 |

**还有一点没核实**：流② 连上后会从某个 seq 开始补发历史，第 1 轮的回复能不能借此补回来，要看前端记下的 seq 走到了哪里；测试里的结果是没补回来。

**修之后**：服务端改成先写账本、发成品消息帧、再发收尾帧（§6.1），前端收到「完成了」时回复已经落地，这段空档从根上没了；前端「只在真断线重连时丢」那条规则留作兜底。
