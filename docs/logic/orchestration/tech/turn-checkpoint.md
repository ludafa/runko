---
title: "Turn Checkpoint 与沙盒保活（技术方案）"
...
> 相关：[功能](../features/turn-checkpoint.md)，[施工进展](../plans/turn-checkpoint.md)。
> 依赖/延续：[chat 聊天 webapp](../../../ingress/features/chat-webapp.md)（turn 收尾在 `turn-runner/`、沙盒生命周期在 `sandbox-manager.ts`）· [UIMessage 单账本](../features/single-ledger.md)（[账本](../../../terms.md) `seq` 即快照对齐锚）· [tech/sandbox](../../../host/contract/tech/sandbox.md)（BYO 原则与「要基线管理就用沙盒里的 git」）。
>
...
`sandbox.recreated` 是 wire 事件，落账本、可回放：`{ recovery: 'wip' | 'branch' | 'fresh', checkpointTurn?, stateTurn }`。把 [chat webapp](../../../ingress/features/chat-webapp.md) §4.1 的「UI 如实提示」从文档承诺变成机制。web 时间线渲染为一条简单系统条目。
...
3. 未 push 的 commit **历史**不可恢复（内容可恢复，见 [features 非目标](../features/turn-checkpoint.md#4-范围与非目标)）。
