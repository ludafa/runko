...
> **依赖**：本功能改造 [chat-webapp](../../../ingress/features/chat-webapp.md) 的持久化层（把它原来的双份落盘换成单账本）。下游的 [上下文压缩（compaction）](../../engine/features/compaction.md) 在本功能的账本上再加第三种条目。
...
- **显示「完成」时已经存好了**：一轮在界面上变成完成，意味着它的回复已经写进账本，刷新页面一定还在。保存失败时界面显示「系统异常」，不会先说完成、刷新后回复却没了；具体出了什么错只进服务端日志。
