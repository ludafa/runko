---
title: "原生搜索（native search）· 施工进展"
slug: native-search
view: 施工
layer: 逻辑层
module: 执行引擎
packages: ["@runko/core", "@runko/sandbox-vercel"]
tags: ["原生搜索", "grep", "glob", "快路径", "性能"]
related: ["architecture/tech/agent-kernel.md"]
---
# 原生搜索（native search）· 施工进展

> 相关：[builtin-tools 技术方案](../tech/builtin-tools.md) §3.7/§3.8/§7（工具双路径自适应、已知限制）· [sandbox 技术方案](../../../host/contract/tech/sandbox.md) §3/§4/§5.1（`RunkoFS` 可选方法契约、适配器逐家映射）· [core-sdk 技术方案](../tech/core-sdk.md) §4.4（`RunkoFS` 契约段落）· 术语见 [docs/terms.md](../../../terms.md)「原生搜索」
> 依赖：[sandbox 施工进展](../../../host/contract/plans/sandbox.md)（P10 三接入包；本功能只改 `@runko/sandbox-vercel`，不涉及 E2B/Cloudflare）

## 背景：定位结论

chat 应用的沙盒工作区是 `@runko/sandbox-vercel` 的远端 microVM；内置 `grep`/`glob` 工具目前是纯 JS 逐文件扫描——每次 `readdir`/`stat`/`readFile` 都是一次到沙盒的网络往返，且全部串行，还会扫进 `.git`。一次 `grep` 动辄上千次往返，实测慢几十秒到几分钟。治本方向 [tech/sandbox](../../../host/contract/tech/sandbox.md) §4 早有预案：扫描类操作应该一条命令在沙盒内解决——本功能把这条预案落地成 `grep`/`glob` 的自适应快路径。

## 设计定案摘要（主线程拍板，2026-07-16）

1. **RunkoFS 可选方法**（`packages/core/src/types.ts`）：新增 `searchFiles?(query: FileSearchQuery): Promise<FileSearchResult>` 与 `searchContent?(query: ContentSearchQuery): Promise<ContentSearchResult>`，配套 `SearchUnsupportedError`（`packages/core/src/search.ts`，独立文件以维持 `types.ts` 头注释「纯接口不含运行时实现」的纪律）。查询/结果类型全文见 [tech/sandbox](../../../host/contract/tech/sandbox.md) §3。**`MemoryFS`/`OverlayFS`/`DirFS` 一律不实现**——`OverlayFS` 罩着远端 base 时，base 的 native 搜索看不见 overlay 层的脏写，回退到走 `glob()`（会经过 overlay 合并视图）才是正确语义。
...
| # | 内容 | 归属 | 状态 |
|---|---|---|---|
| 1 | core：`RunkoFS.searchFiles?`/`searchContent?` + 查询/结果类型 + `SearchUnsupportedError`，从 `@runko/core` 导出 | coder | ✅ 已实现（`packages/core/src/{types,search,index}.ts`，与本工单同步施工） |
...
| 日期 | 阶段 | 变更 | 结论 |
|---|---|---|---|
| 2026-07-16 | 设计定案（主线程） | grep/glob 原生搜索快路径：`RunkoFS` 两个可选方法 + 工具层双路径自适应 + Vercel `node -e` 脚本载体 | — |
...
| 2026-07-16 | 线上事故修复（主线程） | chat 应用 grep 依旧十几秒：`apps/node-server` 的 `gateWorkspace`（chat-agent.ts）逐方法重建 workspace 时只转发了 `RunkoFS` 七个必选方法，可选的 `searchFiles`/`searchContent` 被剥掉，原生搜索在 chat 应用里从未生效（验收范围只到 packages，未覆盖这层集成包装）。已补转发 + 2 个回归用例锁定（server 194/194 绿） | 教训：`RunkoFS` 新增可选能力方法时，所有"显式逐方法转发"的包装层必须同步；`gateWorkspace` 注释已加警示 |
