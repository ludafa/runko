...
> 依赖：[single-ledger](../../orchestration/plans/single-ledger.md)（P14 依赖 **P13-5** 落地——账本必须先是「UIMessage 单账本」）
> 回填目标（P14-4）：[plans/core-sdk](./core-sdk.md) 变更记录 · [plans/verification](../../../misc/plans/verification.md) 用例 · [tech/chat-webapp](../../../ingress/tech/chat-webapp.md) 存储相关表述 · [terms.md](../../../terms.md) §七
...
- **2026-07-15**：从旧编号文档 `docs/logic/engine/tech/compaction.md` 迁入 features / tech / plans 三视角结构。原文「产品设计」入 [features](../features/compaction.md)，「技术方案」（§2.1–2.7）入 [tech](../tech/compaction.md)，验证项（§2.8）与施工计划（§3）入本文。历史 banner「设计待 review（2026-07-15）」保留于本文顶部。
