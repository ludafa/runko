...
related: ["logic/engine/plans/core-sdk.md", "logic/engine/tech/core-sdk.md", "architecture/tech/agent-kernel.md"]
---
# core-sdk（产品视角 · 使用手册）

> 相关：[技术方案](../tech/core-sdk.md) · [施工进展](../plans/core-sdk.md)
> 依赖：[builtin-tools](./builtin-tools.md)（内置工具面：文件八件套 + bash + load-skill + update-plan）· [sandbox](../../../host/contract/features/sandbox.md)（把 [工作区](../../../terms.md)落进真实云沙盒的执行环境）
> 被延续/演进：[single-ledger](../../orchestration/features/single-ledger.md)（P13-5 起 [账本](../../../terms.md)改为 [UIMessage](../../../terms.md) 单账本、[审批结果](../../../terms.md)三值化、内置工具名 kebab-case——本页描述的对外行为已按 P13-5 落地状态书写）

这一份是**产品/使用视角**：这个 SDK 给谁、解决什么问题、怎么照着用（对外 API 的产品视角、用户可见行为）、范围与非目标、成功标准。内部方案与接口细节看 [技术方案](../tech/core-sdk.md)。

## 1. 一句话定义

runko 是一个**可嵌入 Node.js 应用的轻量 [agent](../../../terms.md) SDK**：开发者用几行代码就能在自己的服务里跑起一个具备文件操作、命令执行、[skills](../../../terms.md) 能力的 agent [loop](../../../terms.md)，而无需依赖任何外部 CLI 二进制。
...
| `eve`（eve.dev） | API 设计优秀（runko 的 API 层次即参考它），但它是带 HTTP server 与 durable workflow 的**框架**（`eve dev`/`eve start`、按 session 走 HTTP+NDJSON），不是可嵌入进程内的库；文件操作在真实沙盒，无虚拟 FS。 |
...
4. **自定义执行环境**：宿主已有自己的沙盒/容器/远程执行器，只需把"命令执行"作为工具注入 runko，loop 与工具彻底解耦（把 [工作区](../../../terms.md)接到 [sandbox](../../../host/contract/features/sandbox.md) 的云沙盒上即是这条路的成品）。
...
```ts
import { defineAgent, createSession, RunkoFS } from "runko";
...
只装 `@runko/sdk`（门面包）即可跑通；`session.fs.diff()` 拿到 agent 在虚拟文件系统里做的全部改动，真实目录零改写。
...
- **VirtualFS / RunkoFS**：会话的[工作区](../../../terms.md)。可从内存对象、真实目录（零拷贝 overlay 挂载）、或自定义实现构建；agent 的所有文件读写默认落在这里；结束后可导出 diff / 写回磁盘。
- **Tools**（`defineTool`）：工具层，全部可注入可替换。内置文件工具（基于 VirtualFS，见 [builtin-tools](./builtin-tools.md)）与可选命令执行工具；宿主可注册自定义工具，支持 per-tool [审批策略](../../../terms.md) + session 级[审批分类器](../../../terms.md)。
...
session/turn 生命周期命名参考 eve，过程粒度对齐 codex-sdk 的 item 级事件（P13-5 起以 [UIMessage](../../../terms.md) 部件 + [chunk](../../../terms.md)承载，见 [single-ledger](../../orchestration/features/single-ledger.md)）：

- **`send()`**：Promise，返回完整 `TurnResult`（`finalResponse` + `usage`）。这一轮失败时 `send()` 抛 `RunkoSessionError`（`error.code` 携带失败原因）。
- **`stream()`**：AsyncGenerator，实时吐 **[chunk（流块）](../../../terms.md)**——就是 AI SDK 官方的 [UIMessage](../../../terms.md) 流协议词汇表（`text-start`/`text-delta`/`text-end`、`reasoning-*`、`tool-input-available`、`tool-output-available`/`-error`/`-denied`、`tool-approval-request`/`-response`、`start-step`/`finish-step` 等）**加上** runko 特有的 data 部件（`data-file-change`、`data-plan-update`、`data-error`、`data-tool-progress`）。**任何 AI SDK 兼容的客户端可直接消费**，不需要认识 runko 私有事件格式。生成器的最终返回值仍是 `TurnResult`。

> 历史：这套过程流最初对齐 codex-sdk 的 `item.started/updated/completed`（item 类型 `agent_message`/`reasoning`/`tool_call`/`file_change`/`error`）与 eve 的 `session/turn` 生命周期事件；P13-5 [single-ledger](../../orchestration/features/single-ledger.md) 迁移后，这些语义原样落进 UIMessage 的部件与 chunk 词汇表（旧的 runko 私有 `SessionEvent`/`SessionItem` 已退役），对外表达力不减，但换成了标准协议。

### 4.5 虚拟文件（关键差异化功能）

- `RunkoFS.fromMemory({ "src/index.ts": "..." })` —— 纯内存工作区。
- `RunkoFS.fromDirectory("./project")` —— 真实目录零拷贝 overlay 挂载（读穿透磁盘、写落内存层），跑完 `fs.diff()` / `fs.writeBack()`。
...
- **命令执行 = 内置 bash 工具 + 可注入的执行环境（`RunkoExec`）**，与"内置文件工具 + 可注入 RunkoFS"完全同构：注入了 exec 实现才出现 bash 工具，默认无命令执行面。runko 提供本地子进程参考实现（`localExec`，默认要求审批）、纯 TS 解释器 `@runko/mini-bash` 与全语法档 `@runko/just-bash`；宿主注入 Docker/远程/自研沙盒实现即可换掉执行环境（[sandbox](../../../host/contract/features/sandbox.md) 三家云沙盒即成品）——runko 不关心沙盒长什么样；同一对象实现 `RunkoFS & RunkoExec` 时文件与命令天然同环境（`workspace` 一次注入）。
- **审批（[人在回路](../../../terms.md)）分两层，结果三值化**（见 [single-ledger](../../orchestration/features/single-ledger.md) §6）：
...
- `send(input, { outputSchema })`（zod schema）在一轮正常收尾后再走一轮抽取，返回 `TurnResult & { structuredOutput: T }`。provider 不支持原生结构化输出时自动走"JSON + zod 校验 + 修正重试 ≤ 2 次"回退（对调用方透明）。抽取失败（耗尽预算）抛 `RunkoStructuredOutputError`——注意 turn 本身成功，失败的只是附加的折叠步骤。
...
- 工具注入、两层审批；内置 bash 工具 + `RunkoExec` 执行环境接口（`localExec` 参考实现、`mini-bash` 零依赖极简档、`just-bash` 全语法档）。
...
- 不做 OS 级/容器级沙盒实现（只定义工具接口，沙盒由宿主注入——见 [sandbox](../../../host/contract/features/sandbox.md) 的接入包，仍是宿主侧可选依赖）。
...
- 现成的 Claude skill 目录不改动即可被 runko 加载并生效。
- 换掉命令执行工具的实现（本地→自定义）不需要改任何 loop 相关代码。

> 施工结论：docs/logic/engine/features/core-sdk.md §6 四条成功标准在 v1 收官时**全部达成**（真实 DeepSeek 端到端验证，含结构化输出回退路径实证、官方 xlsx skill 零改动加载、真实目录零改写），详见 [施工进展](../plans/core-sdk.md)。
