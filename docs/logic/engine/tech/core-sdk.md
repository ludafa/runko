...
related: ["logic/engine/features/core-sdk.md", "logic/engine/plans/core-sdk.md", "architecture/tech/agent-kernel.md"]
---
# core-sdk（技术方案）

> 相关：[产品/使用手册](../features/core-sdk.md) · [施工进展](../plans/core-sdk.md)
> 依赖：[builtin-tools](./builtin-tools.md)（内置工具本体与规格）· [sandbox](../../../host/contract/tech/sandbox.md)（`RunkoFS & RunkoExec` 的云沙盒实现，含 §4.4 两个可选[原生搜索](../../../terms.md)方法的适配器契约与逐家映射，拆单见 [plans/native-search](../plans/native-search.md)）
> 被延续/演进：[single-ledger](../../orchestration/tech/single-ledger.md)（P13-5：[账本](../../../terms.md)改为 [UIMessage](../../../terms.md) 单账本、`session.stream()` 吐 ai [chunk](../../../terms.md)、[审批结果](../../../terms.md)三值化、内置工具名 kebab-case——本页 §4.2 事件模型、§4.5 审批链、§4.8 loop 细节均已按 P13-5 落地状态书写，机制全文见该功能文档）
...
- `@openai/codex-sdk@0.144.1`：spawn `codex exec --experimental-json`，stdin/stdout JSONL。`turn/item` 粒度的事件模型干净，**runko 的 item 级事件粒度沿用它**。无虚拟文件；会话落在 `~/.codex/sessions`。
...
- **`eve`（eve.dev）**：TypeScript agent 框架，**runko 对外 API 的层次参考它**——
...
- **Vercel AI SDK（`ai@7.0.19`，2026-07 稳定版）**：**runko 的模型层基于它**——
  - `streamText` 手动 loop 是官方支持的一等模式（cookbook "Manual Agent Loop"）：tools 声明时**省略 `execute`** → `finishReason === "tool-calls"` 时 tool call 原样返回，调用方自己执行、自己以 `role: "tool"` 消息回填、再进下一步——这正是 runko 审批链/自有工具运行时需要的控制点；
  ...
  - 依赖树干净：`ai` 只依赖自家 `@ai-sdk/provider(-utils)`/`gateway`，无二进制；zod 与 runko 共用；
  - 也有更高层的 `ToolLoopAgent`/`@ai-sdk/workflow`（含 `needsApproval` 审批），但那会把 loop 语义让渡给 AI SDK（见 §4.3 选型说明）。

**结论**：runko = eve 的 API 层次与人体工学 + codex-sdk 的 item 级事件粒度 + AI SDK 的模型层 + 自有的 VirtualFS 内核与 loop，以可嵌入库（而非 server 框架）的形态交付。
...
L0 原语层              LanguageModel(AI SDK) / RunkoFS / RunkoExec / Tool / ApprovalPolicy —— 可注入接口与内置实现
```

- 上层永远只依赖下层的接口；宿主可以停在任何一层使用（只用 L0 的 RunkoFS 做文件沙盒也是合法用法）。
- 与 eve 的对应关系：L1/L3 形态对齐 eve（迁移心智零成本）；L2 的差异在于 runko 是**进程内 session 对象**而非 HTTP 服务——eve 的 `POST /session` + NDJSON 流在 runko 里是 `session.stream()` 这个 AsyncGenerator。

包结构（pnpm monorepo，五包；依赖单向 sdk → {core, virtual-fs, mini-bash}，virtual-fs/mini-bash → core，just-bash → core 但不进 sdk）：

```
@runko/sdk          # 主包（门面）：re-export 全部，batteries-included，5 行上手只装它
@runko/core         # L0 接口 + L1 定义层 + L2 运行层 + AI SDK step runner + skills + load-skill/update-plan/bash 工具本体
@runko/virtual-fs   # MemoryFS/OverlayFS/DirFS/mime/diff/reference + 文件工具八件套
@runko/mini-bash    # RunkoExec 实现：纯 TS 解释器（cat/grep/find/tail/head/echo/cd/pwd + 管道/;/&&/||/2>&1），跑在任意 RunkoFS 上，只读、defaultApproval "allow"；cd 双层语义（脚本内链间生效 + 实例级跨调用持久化，req.cwd 显式时优先；管道内按 POSIX 子 shell 无效果）；文件重定向（>/>>/<）刻意不支持——写面走 write-file 工具（有 file_change 事件与 readState 登记，重定向写是旁路）
@runko/just-bash    # RunkoExec 实现（v1.1，全语法档）：vercel-labs/just-bash 适配器——完整 if/for/while/case/函数/变量/重定向，RunkoFS→IFileSystem 适配，defaultApproval "allow"，不进 sdk 依赖（见 §4.5b）
```
...
宿主 Node 应用
   ...
   ▼
┌────────────────────────── Session ───────────────────────────┐
│  状态: messages[](../../../engine/tech/RunkoUIMessage), fs, usage, approvals(会话级授权) │
│    send()/stream()                                            │
...
sequenceDiagram
    ...
    participant Tool as Tool.execute / RunkoFS

    Host->>Session: send(input) / stream(input)
    Session->>Session: await skillFilesMounted + messagesReady
    Session->>Session: turn += 1；把 input 组成 user RunkoUIMessage 追加进账本
    ...
    Session-->>Host: send() 聚合出 TurnResult（status=failed 时 throw RunkoSessionError）
```
...
const Skill: {
  ...
  fromFS(fs: RunkoFS, path: string): Promise<Skill>;   // 从 VirtualFS 加载
...
**内置工具名（P13-5 起 kebab-case，见 single-ledger）**：`BuiltinToolName` 九名可裁剪联合 = `read-file`/`write-file`/`edit-file`/`delete-file`/`move-file`/`list-dir`/`glob`/`grep`/`update-plan`；`load-skill`（由 `agent.skills` 隐式控制）、`bash`（由 `RunkoExec` 注入隐式控制）是条件内置，不在可裁剪列表里。`READ_ONLY_TOOLS = [read-file, list-dir, glob, grep]` 为只读审查预设。
...
interface SessionOptions {
  fs?: RunkoFS;                                // 默认空 MemoryFS（由 @runko/sdk 装配；core 未注入时为抛错占位）
  exec?: RunkoExec;                            // 注入即激活内置 bash 工具（见 4.5a）
  workspace?: RunkoFS & RunkoExec;             // 语法糖：同源工作区一次注入 fs + exec（与前两者互斥）
...
**事件模型（P13-5 起，见 [single-ledger](../../orchestration/tech/single-ledger.md)）**：`session.stream()` 产出 **RunkoChunk** —— ai 官方的 UIMessage 流协议词汇表（`UIMessageChunk`）加上 runko 特有的 data 部件，不再有 runko 自有的 `SessionEvent`/`SessionItem` 联合（已退役）。账本落盘的是 **RunkoUIMessage**（= `UIMessage<RunkoMessageMetadata, RunkoDataParts>`）；模型上下文每步经官方 `convertToModelMessages` 现场推导，不再单独存 `nimbo_state_json`：
...
// 加上下列 runko data 部件的 data-<name> chunk
type RunkoChunk = UIMessageChunk<RunkoMessageMetadata, RunkoDataParts>;

// 账本条目的消息形态：官方 UIMessage 带 runko 元数据与 data 部件
type RunkoUIMessage = UIMessage<RunkoMessageMetadata, RunkoDataParts>;

interface RunkoMessageMetadata {                    // 挂在消息上，不参与转 ModelMessage
  turn?: number;
...
type RunkoDataParts = {                             // 仅供界面；官方转换器转 ModelMessage 时自动丢弃
...
**集成方式：每个 assistant step 调一次 `streamText`，loop 归 runko**（官方 "Manual Agent Loop" 模式）：

1. runko `Tool` → AI SDK `tool({ description, inputSchema })`——**刻意省略 `execute`**。AI SDK 因此不执行任何工具：`finishReason === "tool-calls"` 时 tool call 原样返回；
2. 审批链、工具执行（含 `ctx.update()` 进度）、deny 回填全在 runko 的 ToolRuntime 完成，结果以 `role: "tool"` 的 `ModelMessage` 回填后进入下一步 `streamText`；
...
interface RunkoFS {
  ...
  // 适配器契约见 tech/sandbox §3/§4，工具侧双路径自适应见 tech/builtin-tools §3.7/§3.8 ----
...
**原生搜索接缝（可选，`searchFiles`/`searchContent`）**：不在七个必须方法之列——实现了 = 该底座能一次调用在内部完成整个搜索（典型如远端沙盒在沙盒里跑一条脚本，省掉逐文件网络往返），[内置 `grep`/`glob`](./builtin-tools.md) 会优先调用；未实现该方法、或调用时抛 `SearchUnsupportedError`（`@runko/core` 导出），两个工具都静默回退现有 JS 逐文件扫描，行为不受影响。`MemoryFS`/`OverlayFS` 故意不实现——`OverlayFS` 罩着远端 base 时，base 的 native 搜索看不见 overlay 层的脏写，回退到走 `glob()`（会经过 overlay 合并视图）才是正确语义。适配器契约、查询/结果类型全量定义、逐家映射见 [tech/sandbox](../../../host/contract/tech/sandbox.md) §3/§4/§5.1；拆单见 [plans/native-search](../plans/native-search.md)。
...
- **`MemoryFS`** —— 纯内存路径树。`RunkoFS.fromMemory(files: Record<string, string | Uint8Array>)`。
- **`OverlayFS`** —— 读穿透 + 写覆盖：`base`（只读层，通常是真实目录的 `DirFS` 只读视图）+ `overlay`（MemoryFS，承接所有写入与删除墓碑）。`RunkoFS.fromDirectory(dir, { ignore })` 返回 OverlayFS——大项目零拷贝挂载，写永不落真实磁盘。
...
- **语义澄清（P2-1 施工时确定，随实现回填）**：
  - 工厂为独立导出函数 `fromMemory`/`fromDirectory`（非 `RunkoFS.` 值命名空间——与 core 的 `RunkoFS` 类型名冲突，跨包 interface+namespace 合并不成立）；门面呈现归 P7（`@runko/sdk` 落地了 `RunkoFS.fromMemory`/`fromDirectory` 值命名空间）。
  ...
  - **RunkoFS 实现契约**：对不存在路径统一抛 `NotFoundError`（@runko/virtual-fs 导出）——OverlayFS 判断 base 层存在性依赖此约定；第三方 `RunkoFS` 作 base 时需遵守或自行包一层归一化。
...
#### 4.5a `RunkoExec`：命令执行的接口倒置（与 RunkoFS 同构）

命令执行**工具内置、环境注入**——与文件工具依赖 `RunkoFS` 完全同构：
...
- **与 VirtualFS 的一致性**（"bash 能看到 agent 刚改的文件吗"）按三种模式解决，责任划分各不相同：
  - **模式 A · 同源工作区（推荐真沙盒）**：同一对象实现 `RunkoFS & RunkoExec`，`workspace` 选项一次注入。**一致性在此模式下是结构性的**——文件数据只有一份（在沙盒里），RunkoFS 实现只是它的 API 视图，bash 是另一个访问口；不存在同步逻辑与竞态。[sandbox](../../../host/contract/tech/sandbox.md) 的 E2B/Vercel/Cloudflare 三家适配器即此形态。
  - **模式 B · `localExec({ materialize: true })`**：一致性由 **runko** 负责（执行前把 VirtualFS 物化到随机临时目录、执行后按 mtime 回收变更进 overlay）；明确标注是便利实现而非安全边界。
  - **模式 C · 完全解耦**：宿主注入独立 exec（如只读分析环境），自己保证语义——明知不一致而用之的逃生门。
- **模式 A 的三条衍生规则**（同源即意味着 bash 可旁路 runko 的文件工具）：
  1. bash 旁路造成的文件变更**不产生 `data-file-change` 部件**（runko 只从自己的文件工具派生事件；与 Claude Code/codex 行为一致）；
  2. readState 先读后改校验以 `stat().mtime`（或实现提供的 version）为判据——bash 改过的文件必须重读才能 `edit-file`；
  3. `diff()/writeBack()/snapshot()` 是 MemoryFS/OverlayFS 的附加能力而非 `RunkoFS` 接口必需——内置文件工具不依赖它们，沙盒实现可选支持。
- **实现契约（P6-1 施工回填）**：`exec()` 的全部失败路径（解析错误/未知命令/超时/abort/命令级错误）应以 **resolve 的 `ExecResult`**（非零 `exitCode` + stderr）返回而非 reject——bash 工具把非零退出码当正常 tool 结果回填模型，reject 只应发生在实现自身完全不可用的场景。`@runko/mini-bash` 退出码取 POSIX/GNU 惯例：解析错误 2、未知命令 127、超时 124、abort 130；grep 无匹配 1 / 文件缺失 2（三态），其余命令文件缺失 1。mini-bash 的路径解析对 `..` 越界静默 clamp 到根（解释器语义，非安全边界——安全边界在 FS 层，§4.4）。
- **`localExec` 的 cwd 语义（P7-3 施工回填）**：**模式 C**（非 materialize）下是**真实主机路径**，直传 `spawn()`，缺省 `process.cwd()`；**模式 B**（`materialize: true`）下是**相对 fs 根的虚拟路径**，映射进物化临时目录，缺省即虚拟根 `/`。`materialize: true` 需要 `RunkoFS` 引用，经构造参数 `localExec({ materialize: true, fs })` 注入（未传时构造期同步抛错）。
- 宿主也仍可无视这套机制，直接 `defineTool` 一个自己的命令工具——`RunkoExec` 只是让最常见的需求零工具代码。

#### 4.5b `@runko/just-bash`：全语法档 bash（v1.1，基于 vercel-labs/just-bash）

mini-bash 的语法面（六命令 + 四操作符）撑不住 Claude 系模型高频产出的 `if/for/while/case` 脚本。升级方案不是扩写 mini-bash 解释器，而是新增可选包 **`@runko/just-bash`**：把 [just-bash](https://github.com/vercel-labs/just-bash)（Vercel Labs，Apache-2.0，纯 TS，专为 AI agent 场景设计）适配为 `RunkoExec`——**分档并存**，mini-bash 保留为零依赖极简档。
...
**选型依据**（调研对照 `just-bash@3.1.0` 实际 d.ts）：完整控制流（if/elif/for 含 C 式/while/until/case/函数/local/变量与参数扩展/glob/重定向/管道/`&&`/`||`）；`defineCommand` 注册 JS 自定义命令；**`BashOptions.fs: IFileSystem` 可插拔**——接口形状与 RunkoFS 高度同源；`executionLimits`（maxLoopIterations/maxCommandCount 等）防 agent 死循环；网络默认禁用；纯 TS（sql.js/quickjs 为 wasm 可选件，零原生二进制）。备选 bashkit（Rust+napi）与 bash-parser（九年未维护）均否决。

**适配器（RunkoFS → IFileSystem）映射与降级策略**：

| IFileSystem 成员 | 适配 |
|---|---|
| readFile/readFileBuffer/writeFile/mkdir/readdir/rm/stat/exists | RunkoFS 七方法直译（string 经 TextDecoder；FsStat.mtime = new Date(stat.mtime)） |
...
| getAllPaths | `fs.glob("**")` + 从文件路径合成目录集合（RunkoFS.glob 只报文件，§4.4） |
...
**RunkoExec 表面**：`defaultApproval: "allow"`——重定向写面落在**同一个注入的 RunkoFS**（与文件工具同沙盒，bash 旁路写的 readState mtime 防线即模式 A 三规则），网络默认关闭，无进程无真实磁盘；`describe()` 声明全语法能力、无 symlink、无网络、**输出非流式**（just-bash 无增量回调，onOutput 在结束时一次性回报）；`timeoutMs` 由适配器包一层 AbortController（超时 124 对齐既有契约；**取消保底**：just-bash 的协作式取消救不了永不 resolve 的 fs 调用，适配器沿 mini-bash 的 `raceAbort` 独立赛跑并自定退出码，P9-1 实测）；`req.cwd` → per-exec 覆盖，缺省沿实例 cwd——**cwd 持久语义由适配器闭包实现**（P9-1 实测修正原断言：just-bash 的 `Bash.exec()` 每次调用无状态，`cd` 只反映在该次 `result.env.PWD`；适配器以闭包 `instanceCwd` + 读回 `env.PWD` 复刻 mini-bash P6-4 同款语义）。执行限额默认收紧（maxCommandCount/maxLoopIterations 2000、maxOutputSize 1MB，较上游默认约收一个数量级）并暴露为 `justBash(fs, { limits })`。

**已知限制（P9-1 施工回填）**：`IFileSystem.getAllPaths()` 为**同步**签名而 `RunkoFS.glob` 异步——适配器折中为"每次顶层 `exec()` 开始刷新一次同步缓存"：同一次 exec 内，脚本前面命令新写入的文件对后续 `**` 递归 glob 展开不可见（下次 exec 才可见）；单层 glob 走 readdir 不受影响。cp/mv 的目录递归经 `glob` 枚举，空子目录不参与复制（继承"glob 只报文件"，§4.4）。

**包关系**：`@runko/just-bash` 依赖 `@runko/core`（类型）+ `just-bash`（运行时）。**不进 `@runko/sdk` 依赖**——just-bash 依赖树含 wasm 大件（sql.js、quickjs-emscripten 等），强制打包违背门面轻量默认；需要全语法档的宿主显式 `pnpm add @runko/just-bash`。
...
- **语义澄清（P5-1 施工回填）**：
  - **挂载机制**：core 经 `RunkoFS.writeFile` 接口把附属文件写入 `/.skills/<name>/`——"OverlayFS 只读层"语义归 P7 门面装配，core 不假设具体 FS 实现。挂载在 `createSession` 时同步发起、首次 `stream()/send()` 起始处 await 完成（工具执行前必已挂载，且 createSession 保持同步签名）。skills 均无 `files` 时完全不触碰 fs；fs 未注入且有 skill 带 `files` 时，首次 send/stream 以"注入 fs"指导性错误 reject。
...
```ts
// @runko/core 的 ./load 子路径导出（sdk export * 自动透传）
function loadAgent(dir: string, opts?: { model?: LanguageModel }): Promise<AgentDefinition>;
function loadAgentFromFS(fs: RunkoFS, dir: string, opts?): Promise<AgentDefinition>;  // runko 独有：定义本身也可以是虚拟的
```
...
- model 解析：`agent.ts` 里可写 `model: "anthropic/claude-sonnet-5"` 字符串——这正是 AI SDK 的 gateway 格式（也是 eve 的格式），原样透传；不想走 gateway 的宿主在 `loadAgent(dir, { model })` 里直接给 `LanguageModel` 实例覆盖。runko 不自建模型注册表。
...
- **模型层即 AI SDK 生态**：任何 `LanguageModel`（30+ provider 包、openai-compatible、gateway 字符串、社区 provider）开箱可用；宿主已有的 AI SDK 中间件（`wrapLanguageModel`、缓存、observability）对 runko 透明生效。
...
| `ai` 成为 runko 的强依赖，宿主项目若锁定了不同大版本会有 peer 冲突 | 中 | `ai` 声明为 peerDependency（range `^7`）+ devDependency，跟随宿主版本 |
...
| 无 compaction 时长会话撞上下文墙 | 中 | v1 显式报错可预期；v2 做 compaction（[compaction](./compaction.md)） |
