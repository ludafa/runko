...
> 依赖：[builtin-tools](./builtin-tools.md)（内置工具本体与规格）· [sandbox](../../../host/contract/tech/sandbox.md)（`NimboFS & NimboExec` 的云沙盒实现，含 §4.4 两个可选[原生搜索](../../../terms.md)方法的适配器契约与逐家映射，拆单见 [plans/native-search](../plans/native-search.md)）
...
宿主 Node 应用
   ...
   ▼
┌────────────────────────── Session ───────────────────────────┐
│  状态: messages[](../../../engine/tech/NimboUIMessage), fs, usage, approvals(会话级授权) │
│    send()/stream()                                            │
...
**事件模型（P13-5 起，见 [single-ledger](../../orchestration/tech/single-ledger.md)）**：`session.stream()` 产出 **NimboChunk** —— ai 官方的 UIMessage 流协议词汇表（`UIMessageChunk`）加上 nimbo 特有的 data 部件，不再有 nimbo 自有的 `SessionEvent`/`SessionItem` 联合（已退役）。账本落盘的是 **NimboUIMessage**（= `UIMessage<NimboMessageMetadata, NimboDataParts>`）；模型上下文每步经官方 `convertToModelMessages` 现场推导，不再单独存 `nimbo_state_json`：
...
- **与 VirtualFS 的一致性**（"bash 能看到 agent 刚改的文件吗"）按三种模式解决，责任划分各不相同：
  - **模式 A · 同源工作区（推荐真沙盒）**：同一对象实现 `NimboFS & NimboExec`，`workspace` 选项一次注入。**一致性在此模式下是结构性的**——文件数据只有一份（在沙盒里），NimboFS 实现只是它的 API 视图，bash 是另一个访问口；不存在同步逻辑与竞态。[sandbox](../../../host/contract/tech/sandbox.md) 的 E2B/Vercel/Cloudflare 三家适配器即此形态。
...
- **abort 的生效点（[停止本轮](../../orchestration/tech/turn-abort.md)工单回填）**：`runTurn` 的 step 循环**开头**显式检查 `abortSignal.aborted`，命中即以 `interrupted`/`aborted` 优雅收尾——「绝不开始新的一步」是 nimbo 自己的确定性保证。一步*之内*的中止仍由 `abortSignal` 本身负责（模型流被掐断 / `ToolContext.abortSignal` 交给工具），两条路收尾形状一致。之所以需要这个显式检查：工具执行被中止时工具是**正常收尾**的（§4.5a「失败即 `ExecResult`」，不抛错），若只等 AI SDK 抛错，loop 会照常进入下一步、白打一次模型调用才停。
