...
> 依赖：[builtin-tools](./builtin-tools.md)（内置工具面：文件八件套 + bash + load-skill + update-plan）· [sandbox](./sandbox.md)（把 [工作区](../terms.md)落进真实云沙盒的执行环境）
> 被延续/演进：[single-ledger](./single-ledger.md)（P13-5 起 [账本](../terms.md)改为 [UIMessage](../terms.md) 单账本、[审批结果](../terms.md)三值化、内置工具名 kebab-case——本页描述的对外行为已按 P13-5 落地状态书写）
...
4. **自定义执行环境**：宿主已有自己的沙盒/容器/远程执行器，只需把"命令执行"作为工具注入 nimbo，loop 与工具彻底解耦（把 [工作区](../terms.md)接到 [sandbox](./sandbox.md) 的云沙盒上即是这条路的成品）。
...
session/turn 生命周期命名参考 eve，过程粒度对齐 codex-sdk 的 item 级事件（P13-5 起以 [UIMessage](../terms.md) 部件 + [chunk](../terms.md)承载，见 [single-ledger](./single-ledger.md)）：
...
> 历史：这套过程流最初对齐 codex-sdk 的 `item.started/updated/completed`（item 类型 `agent_message`/`reasoning`/`tool_call`/`file_change`/`error`）与 eve 的 `session/turn` 生命周期事件；P13-5 [single-ledger](./single-ledger.md) 迁移后，这些语义原样落进 UIMessage 的部件与 chunk 词汇表（旧的 nimbo 私有 `SessionEvent`/`SessionItem` 已退役），对外表达力不减，但换成了标准协议。
...
- **命令执行 = 内置 bash 工具 + 可注入的执行环境（`NimboExec`）**，与"内置文件工具 + 可注入 NimboFS"完全同构：注入了 exec 实现才出现 bash 工具，默认无命令执行面。nimbo 提供本地子进程参考实现（`localExec`，默认要求审批）、纯 TS 解释器 `@nimbo/mini-bash` 与全语法档 `@nimbo/just-bash`；宿主注入 Docker/远程/自研沙盒实现即可换掉执行环境（[sandbox](./sandbox.md) 三家云沙盒即成品）——nimbo 不关心沙盒长什么样；同一对象实现 `NimboFS & NimboExec` 时文件与命令天然同环境（`workspace` 一次注入）。
- **审批（[人在回路](../terms.md)）分两层，结果三值化**（见 [single-ledger](./single-ledger.md) §6）：
...
- 不做 OS 级/容器级沙盒实现（只定义工具接口，沙盒由宿主注入——见 [sandbox](./sandbox.md) 的接入包，仍是宿主侧可选依赖）。
...
> 施工结论：docs/features/core-sdk.md §6 四条成功标准在 v1 收官时**全部达成**（真实 DeepSeek 端到端验证，含结构化输出回退路径实证、官方 xlsx skill 零改动加载、真实目录零改写），详见 [施工进展](../plans/core-sdk.md)。
