...
> **依赖**：示例演示 [core-sdk](./core-sdk.md) 的各项能力（VirtualFS / NimboExec / skills / 结构化输出 / streaming）与 [sandbox](./sandbox.md) 三家云沙盒适配器；模型驱动段的验证语义与 [plans/verification](../plans/verification.md) 一脉相承。
...
- **不是什么**：
  ...
  - **`cloudflare-gateway/` 不属于示例集本体**：它是 11 号真机段需要的一个独立可部署 wrangler 子项目，有自己的 `package.json`/构建/类型检查，按其自身 README 单独处理。
