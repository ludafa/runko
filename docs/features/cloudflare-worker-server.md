...
> 相关功能：[沙盒](./sandbox.md)（[网关形态](../terms.md)）· [示例集](./examples.md)（11 号 Cloudflare 示例的真机段用它）
...
- **Cloudflare 仍不是 chat 的[沙盒 provider](../terms.md) 选项**（[沙盒 provider](./sandbox-provider.md) 只支持 `vercel`/`e2b`），本项目不改变这一点。
...
- 外部 Node 机器上的 `cloudflareWorkspace({url, token})` 能经 `/gateway/*` 完成同样的文件与 exec 操作——即[示例集](./examples.md) 11 号的真机段可跑。
