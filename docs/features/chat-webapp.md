...
6. **持久化：刷新 / 重连可回放，进程重启可恢复**：会话与全部账本数据落 SQLite。刷新页面、HMR、网络抖动都不会丢进行中这一轮的后续输出——重连后先 [回放](../terms.md)历史、再续接直播（断线可续，见「成功标准」）。
7. **沙盒生命周期对用户透明**：
...
4. **进程重启会中断进行中的一轮**：沙盒还在，但驱动这一轮的内存态停了；已落库的内容保留到中断点，重连回放发现无进行中轮即静默收尾。这是 v1 内存态 runner 的已知取舍。
