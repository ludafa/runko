...
> 依赖 / 增强：[chat-webapp](./chat-webapp.md)（把它固定用 Vercel 沙盒的假设放开成可选）· [sandbox](./sandbox.md)（`@nimbo/sandbox-vercel` / `@nimbo/sandbox-e2b` 两个[沙盒适配器](../terms.md)）
...
在既有 [chat-webapp API](./chat-webapp.md) 上的**增量**（非破坏）：
...
3. **休眠丢未提交工作的兜底档同样存在**：两家的唤醒都优先走快照/resume（含未提交改动）；快照过期/沙盒被回收则重 clone + checkout 已 push 分支，这一档下未提交工作会丢失——与 [chat-webapp 已知取舍](./chat-webapp.md)同源，[turn-checkpoint](./turn-checkpoint.md) 拟补掉这个窗口。
