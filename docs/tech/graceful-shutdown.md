...
> 相关：产品/使用视角见 [../features/graceful-shutdown.md](../features/graceful-shutdown.md)，施工进展见 [../plans/graceful-shutdown.md](../plans/graceful-shutdown.md)。
> 依赖/延续：[停止本轮](./turn-abort.md) §3（`abortTurn` 与[起轮占位](../terms.md)，本方案整个建在它上面）· [chat webapp](./chat-webapp.md) §5.1（[轮状态快照](../terms.md)，界面层的第三道防线）· [核心 SDK](./core-sdk.md) §4.2（`TurnOptions.signal`）。
...
- **防线 3（已完成）**：[轮状态快照](../terms.md)——tail 每次连上，服务端都下发「此刻到底有没有轮在跑」。它不依赖账本，所以即便前两道都没生效，界面也不会卡在假流式态。见 [chat-webapp §5.1](./chat-webapp.md)。
...
1. **不删那些 chunk 行。** 它们是那一轮**唯一**的内容记录——没有 session 可以把它们物化成 `kind = 'message'` 行了。删掉界面就什么都看不到。所以补收尾、不 GC（沿用 [chat-webapp](./chat-webapp.md) 既有的「崩溃残渣不清理」取舍）。
