...
> 依赖：[builtin-tools](./builtin-tools.md)（内置工具本体与规格）· [sandbox](./sandbox.md)（`NimboFS & NimboExec` 的云沙盒实现，含 §4.4 两个可选[原生搜索](../terms.md)方法的适配器契约与逐家映射，拆单见 [plans/native-search](../plans/native-search.md)）
> 被延续/演进：[single-ledger](./single-ledger.md)（P13-5：[账本](../terms.md)改为 [UIMessage](../terms.md) 单账本、`session.stream()` 吐 ai [chunk](../terms.md)、[审批结果](../terms.md)三值化、内置工具名 kebab-case——本页 §4.2 事件模型、§4.5 审批链、§4.8 loop 细节均已按 P13-5 落地状态书写，机制全文见该功能文档）
...
**事件模型（P13-5 起，见 [single-ledger](./single-ledger.md)）**：`session.stream()` 产出 **NimboChunk** —— ai 官方的 UIMessage 流协议词汇表（`UIMessageChunk`）加上 nimbo 特有的 data 部件，不再有 nimbo 自有的 `SessionEvent`/`SessionItem` 联合（已退役）。账本落盘的是 **NimboUIMessage**（= `UIMessage<NimboMessageMetadata, NimboDataParts>`）；模型上下文每步经官方 `convertToModelMessages` 现场推导，不再单独存 `nimbo_state_json`：
...
- **与 VirtualFS 的一致性**（"bash 能看到 agent 刚改的文件吗"）按三种模式解决，责任划分各不相同：
  - **模式 A · 同源工作区（推荐真沙盒）**：同一对象实现 `NimboFS & NimboExec`，`workspace` 选项一次注入。**一致性在此模式下是结构性的**——文件数据只有一份（在沙盒里），NimboFS 实现只是它的 API 视图，bash 是另一个访问口；不存在同步逻辑与竞态。[sandbox](./sandbox.md) 的 E2B/Vercel/Cloudflare 三家适配器即此形态。
