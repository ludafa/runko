...
> 相关：产品文档见 [../features/composer-skill-mention.md](../features/composer-skill-mention.md)，施工进展见 [../plans/composer-skill-mention.md](../plans/composer-skill-mention.md)。
> 依赖/延续：[chat webapp 技术方案](./chat-webapp.md)（§2.2 起轮通路、§4 服务端模块划分、§8 前端）· [核心 SDK](./core-sdk.md)（§4.1 `Skill.fromFS`、§4.6 skills 注入三件事）· [内置工具](./builtin-tools.md)（§1.9 `load-skill`）。
...
本功能选定的路线是**软提示**——不把 SKILL.md 正文硬塞进上下文，仍走 [`load-skill`](./builtin-tools.md) 的[渐进式披露](../terms.md)。
...
/** 这个会话当前可选的 skill 清单（docs/tech/composer-skill-mention.md §2.1）——
