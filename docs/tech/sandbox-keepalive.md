...
> 产品文档见 [docs/features/sandbox-keepalive.md](../features/sandbox-keepalive.md)，施工进展见 [docs/plans/sandbox-keepalive.md](../plans/sandbox-keepalive.md)。
...
7. **不做自愈**。续期失败只记录，不重连不重放。理由见 [sandbox-provider](./sandbox-provider.md) §5.1。
...
- [docs/features/sandbox-keepalive.md](../features/sandbox-keepalive.md)：产品文档。
- [docs/plans/sandbox-keepalive.md](../plans/sandbox-keepalive.md)：施工进展。
- [docs/tech/sandbox.md](./sandbox.md) §4.6：沙盒生命周期与会话恢复的既有立场。
- [docs/tech/sandbox-provider.md](./sandbox-provider.md) §5.1：这个问题的第一次分析（缓存失效与保活心跳）。
- [docs/tech/turn-checkpoint.md](./turn-checkpoint.md) §5：P13-2b 保活方案，本文档取代其定位部分、保留其补足语义与单轮上限。
- [docs/tech/core-sdk.md](./core-sdk.md)：core 的接口分层。
