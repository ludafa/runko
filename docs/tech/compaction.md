...
> 相关：[产品视角](../features/compaction.md) · [施工进展](../plans/compaction.md) · 依赖 [single-ledger](../tech/single-ledger.md)（本方案的地基，P14 依赖 P13-5 落地）
> 参考：[tech/core-sdk](../tech/core-sdk.md) §4.8（上下文管理 v1 显式上限）· [tech/sandbox](../tech/sandbox.md)（账本流水与模型上下文的两层结构、责任划分）
...
账本在 server（`conversation_events`），压缩本质是「宿主如何组装 resume 输入」的存储层策略——与沙盒重建同类，归宿主（[tech/sandbox](../tech/sandbox.md) 责任划分的既有立场）。core 的 loop / session 不感知压缩：它拿到的就是一份合法的 `UIMessage[]`。
