...
> 相关：产品/使用视角见 [../features/agent-kernel.md](../features/agent-kernel.md)，施工进展见 [../plans/agent-kernel.md](../plans/agent-kernel.md)。
> 设计讨论的完整记录（含被推翻的路线）在 [issue #2](https://github.com/ludafa/nimbo/issues/2)。
> 依赖/延续：[进行中草稿放内存](./in-flight-draft.md)（附录 C 的「五样东西」是本方案的起点）· [优雅关闭](./graceful-shutdown.md) 附录 B · [排队与插话](./steer-and-queue.md) §7 · [沙盒保活](./sandbox-keepalive.md) §4.2 · [单一账本](./single-ledger.md)。
