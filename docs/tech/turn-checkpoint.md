...
`sandbox.recreated` 是 wire 事件，落账本、可回放：`{ recovery: 'wip' | 'branch' | 'fresh', checkpointTurn?, stateTurn }`。把 [chat webapp](../features/chat-webapp.md) §4.1 的「UI 如实提示」从文档承诺变成机制。web 时间线渲染为一条简单系统条目。
...
> **本节已被取代**（2026-07-26）：保活独立成功能，见 [docs/tech/sandbox-keepalive.md](./sandbox-keepalive.md)。
>
