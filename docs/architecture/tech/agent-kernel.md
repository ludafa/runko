...
| 形态 | 换掉哪几个宿主模块 | 剩下用内置 | 展开 |
|---|---|---|---|
| **⓪ 本机 CLI** | 沙盒 → 本机（`localExec` + `fromDirectory`） | 持久化、流分发、归属仲裁 | [Node 长驻](../../host/node/tech/deployment.md) |
| **① 同机多进程 cluster** | + 持久化 → SQLite，归属仲裁 → 租约 | 流分发 | 同上 |
| **②③ Docker / k8s** | + 沙盒 → 远端，持久化 → Postgres | 流分发（靠[应用层转发](../../terms.md)） | 同上 · [E2B](../../host/e2b/tech/deployment.md) |
| **④a Cloudflare DO** | 四样**全换成 DO 版** | —— | [Cloudflare](../../host/cloudflare/tech/deployment.md) |
| **④b Vercel** | 四样**全换**（流分发 → Redis 是这一档独有的） | —— | [Vercel](../../host/vercel/tech/deployment.md) |

**从 ⓪ 到 ④，就是内置实现被逐个换掉的过程。**

> 本节是**横向对照**——一眼看清各档差在哪。每一档「四样能力具体怎么配、有什么固有边界」在右边那列的展开文档里，按环境各一份。
