...
| 日期 | 阶段 | 记录 |
|---|---|---|
| 2026-07-14 | P13 | **P13-1 transcript 减量**（持久化分析后用户逐项确认，docs/08 §2.2d）：`item.updated` tick 改 ephemeral——只广播、不落库、**不占 seq**（信封 seq 变可选：有 seq ⇔ 持久可回放）。动机：tick 携带累积全文，落库量为消息长度平方级（实证 98% 行是 tick）。持久流因 ephemeral 不占号而**无空洞**，崩溃后 seq 续起无漂移；stream tail 以 replayDone 标志丢弃回放阶段到达的 ephemeral（防「旧 tick 覆盖已回放 completed」回归竞态，flush 后同步翻转无新窗口）；web 端 seq 记账（去重/lastSeq/after=）只看持久信封，envelopes 改按到达顺序追加。取舍：turn 中崩溃后半截打字机不可回放（P13-3 中断哨兵将显性化）；不做抽稀持久。server 102→113、web 112→117，双端 typecheck/lint 零告警，tester 无缺陷（含 TransformStream 背压构造的确定性时序用例）。P13-2（WIP shadow ref）/P13-3（中断哨兵）/P13-4（留存治理）待逐项讨论 |
