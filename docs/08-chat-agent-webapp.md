...
- **P13 持久化系列（2026-07-14 起，逐项确认制）**：P13-1 transcript 减量（§2.2d，✅ 已交付）；**P13-5 单一数据账本被定为最高优先级**（UIMessage 为唯一真相，验证实验方案见 [docs/10](./10-uimessage-single-ledger.md)，通过后 core 迁移正式立项）；P13-2 turn checkpoint 与 P13-2b 沙盒保活顺延（方案见 [docs/09](./09-turn-checkpoint-and-keepalive.md)，若单账本落地其对齐语义将简化）；P13-3 中断哨兵、P13-4 留存治理待讨论。
