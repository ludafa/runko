...
| **交权（handover）** | — | 节点要下线（滚动发布、缩容、`SIGTERM`）时，把手上每一份对话交给别的节点接着跑。它是轮编排那一堆动作里**唯一自上而下**的一件。**按这一轮当时所处的阶段处置**：模型正在输出 → 掐断、扔掉这半步，接手节点重新生成；正在跑工具 → 工具留在旧节点上[跑完](#十三架构分层)，对话立刻交出去；正在等人 → [挂起](#十三架构分层)（无损）；装配中或两步之间 → 直接收尾。交出去走[指定交接](#十三架构分层)。旧说法「跑完当前这一轮再放手」「一律中止」均已作废。见 docs/logic/orchestration/tech/handover.md。 |
...
| **节点登记表（node registry）** | — | 每个节点登记并定时刷新的一行：地址、[发布序号](#十三架构分层)、状态（正常 / [下线中](#十三架构分层)）、心跳时刻。**只用来挑接手候选**，接不接由被挑中的节点看自己内存里的状态决定——登记表一定有时差。 |
| **发布序号（release sequence）** | — | 每次发布都递增的一个数，**回滚也递增**。[指定交接](#十三架构分层)靠它挑「比自己新」的节点：同一批下线的节点序号相同，所以只挑更大的就不会挑中同批节点。由宿主注入，框架只比大小。 |
| **下线中（leaving）** | — | 节点收到 `SIGTERM` 之后的状态：入站已关（不再有新请求进来）、已有连接与出站保留，正在[交权](#十三架构分层)或[工具收尾](#十三架构分层)。**不要译成 draining**——那会与 [conversation-drained](#十三架构分层) 混淆。 |
