...
| **优雅关闭（graceful shutdown）** | — | 服务端进程收到 SIGTERM/SIGINT（热重载、部署、pod 迁移、Ctrl-C）后，先把进行中的[轮](#一agent-运行的基本单位)主动[停止](#一agent-运行的基本单位)并等它收尾、再退出，而不是让它无声消失。落地为 `turn-runner/shutdown.ts` 的 `shutdownTurns` + `index.ts` 的信号处理。见 docs/logic/orchestration/features/graceful-shutdown.md。 **「一律停止」已被[交权](#十三架构分层)的按阶段处置取代**（多副本下对话交给别的节点接着跑），见 docs/logic/orchestration/tech/handover.md。 |
...
| **等待窗口（finish window）** | drain、drainMs（**别用**：drain 已被 [conversation-drained](#十三架构分层) 占了） | [交权](#十三架构分层)时先不中止、让干活的[轮](#一agent-运行的基本单位)自己跑完的那段时间，上限由 `finishWindowMs` 配（缺省 0 = 不等）。窗口里等人的轮立刻[挂起](#十三架构分层)；到点还没跑完的才中止。[节点下线](#十验证与示例脚本)时配成 90 秒。 **过渡方案**：它就是「跑完当前这一轮再放手」，已被[交权](#十三架构分层)的按阶段处置否决（入站关闭后旧节点还持有对话，这段时间里这份对话的请求都会失败）；在交权落地的同一次改造里**直接删除**（不经过废弃期）。见 docs/logic/orchestration/tech/handover.md §3.3。 |
...
| **交权（handover）** | — | 节点要下线（滚动发布、缩容、`SIGTERM`）时，把手上每一份对话交给别的节点接着跑。它是轮编排那一堆动作里**唯一自上而下**的一件。**按这一轮当时所处的阶段处置**：模型正在输出 → 掐断、扔掉这半步，接手节点重新生成；正在跑工具 → 工具留在旧节点上[跑完](#十三架构分层)，对话立刻交出去；正在等人 → [挂起](#十三架构分层)（无损）；装配中或两步之间 → 直接收尾。交出去走[指定交接](#十三架构分层)。旧说法「一律中止」「跑完当前这一轮再放手」均已作废；后者目前以[等待窗口](#十验证与示例脚本)的形式留在代码里，是过渡方案。见 docs/logic/orchestration/tech/handover.md。 |
| **指定交接（directed handover）** | — | [交权](#十三架构分层)时，下线节点从[节点登记表](#十三架构分层)里挑一个接手节点，在[租约](#十三架构分层)上为它做[交接预留](#十三架构分层)，再经出站直接请它接手。**不依赖浏览器**（用户可能已经关了页面），优先级高于[定时回捞](#十三架构分层)。 |
| **节点登记表（node registry）** | — | 每个节点登记并定时刷新的一行：地址、[发布序号](#十三架构分层)、状态（正常 / [节点下线](#十验证与示例脚本)中）、心跳时刻。**只用来挑接手候选**，接不接由被挑中的节点看自己内存里的状态决定——登记表一定有时差。 |
| **发布序号（release sequence）** | — | 每次发布都递增的一个数，**回滚也递增**。[指定交接](#十三架构分层)靠它挑「比自己新」的节点：同一批下线的节点序号相同，所以只挑更大的就不会挑中同批节点。由宿主注入，框架只比大小。 |
| **交接预留（handover reservation）** | — | [租约](#十三架构分层)的一种状态：「预留给节点 X，某时刻前有效」。有效期内**只有 X 抢得到**，[定时回捞](#十三架构分层)和打到别处的请求都会被挡住；过期后退化成没人持有。它让[指定交接](#十三架构分层)的优先级写进数据，而不是靠谁先到。 |
