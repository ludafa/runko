...
| **交权（handover）** | 等待窗口、finishWindowMs（**已删除**：「先让干活的轮跑完、到点再中止」的过渡做法，交权落地时一并删掉；读 2026-09 之前的集群控制台文档时对照用） | 节点要下线（滚动发布、缩容、`SIGTERM`）时，把手上每一份对话交给别的节点接着跑。它是轮编排那一堆动作里**唯一自上而下**的一件。**按这一轮当时所处的阶段处置**：模型正在输出 → 掐断、扔掉这半步，接手节点重新生成；正在跑工具 → 工具留在旧节点上[跑完](#十三架构分层)，对话立刻交出去；正在等人 → [挂起](#十三架构分层)（无损）；装配中或两步之间 → 直接收尾。交出去走[指定交接](#十三架构分层)。旧说法「一律中止」「跑完当前这一轮再放手」均已作废；后者目前以[等待窗口](#十验证与示例脚本)的形式留在代码里，是过渡方案。见 docs/logic/orchestration/tech/handover.md。 |
