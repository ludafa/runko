...
| 主术语 | 同义词（退役） | 大白话定义 |
|---|---|---|
| **agent** | — | 能自己调用工具、多步完成任务的 AI。这里特指 runko 驱动的那个。 |
| **loop** | loop agent | agent 的「思考→调工具→看结果→再思考」这个反复循环。runko 自己实现这个循环（不外包给 AI SDK 的现成 loop）。跑这个循环的 AI 直接叫 agent。 |
...
| **UIMessage** | — | 「给界面看的消息」格式，比 ModelMessage 多带界面用的过程信息。P13-5 之后 runko 的账本只存这一种。 |
...
| **transient 部件** | 临时部件 | 一种特殊 data 部件：只在直播时推一下、**不存进账本**。打字进度就是这种。注意：它是「线上传输块」的属性，不是存进消息数组后再过滤（见 docs/logic/orchestration/tech/single-ledger.md）。 |
| **元数据（metadata）** | — | 挂在一条 UIMessage 上的附加信息（这一轮的 token 用量、成功/失败状态、是不是 steer 插话）。不参与转模型消息。 |
| **官方转换器（`convertToModelMessages`）** | — | AI SDK 的转换函数：吃一串 UIMessage，吐出对应的 ModelMessage。由 Vercel 官方维护，runko 不自己写这个转换。 |
...
| **折叠（fold）** | — | 把一段 chunk 流按消息边界（`start`/`finish` chunk）积累还原成完整 UIMessage 的动作——chunk 是消息的传输形态，折叠是反向还原。界面端由 AI SDK 流处理器边收边折叠；服务端不重新折叠——core 的 loop 产出 chunk 的同时就在账本工作态里维护着成品消息，收尾直接落盘（见 docs/logic/orchestration/tech/single-ledger.md §2.4）。 |
| **直播流** | live tail | 服务端通过 `GET .../stream` 实时把 chunk 推给浏览器的那条连接（SSE）。「直播」= 边跑边推，区别于「回放」= 从账本读历史。 |
| **回放（replay）** | — | 刷新页面或断线重连后，从账本按序号读出历史、在界面重建出来。 |
| **进行中草稿（in-flight draft）** | 临时记录、半成品 | 一轮进行中、成品消息还没落盘时，为「此刻刷新页面能重建当前画面」而留着的那串耐久 chunk。轮一收尾就被成品消息取代、随即丢弃。它的存放位置见 docs/logic/orchestration/tech/in-flight-draft.md（现状在账本表里占 `kind='chunk'` 行，方案是改放进程内存）。 |
| **seq（序号）** | — | 账本里每条落盘记录的单调递增编号，一个会话内唯一。断线重连靠它续传（`after=<seq>`）。 |
| **裁决表（decision store）** | 决策表 | 把每一次「停下来等人」落一行的那张表（接口 `DecisionStore`，实现在 `@runko/persist-*`）。一行 = 一次审批或一次 `ask-user` 提问：谁问的、入参是什么、人答了什么、什么时候答的。**`decidedAt` 为空 = 这条还悬着。** 与[人工裁决](#四审批human-in-the-loop)的区别：那个是**人给的那句答复**，这个是**记录它的地方**。今天它是**纯审计表**（人的答复走内存 promise，不读库）；[挂起](#十三架构分层)落地后它同时成为[恢复](#十三架构分层)的权威来源。 |
...
> P13-5 审批 API 重构后的定案术语（见 docs/logic/orchestration/tech/single-ledger.md）。旧的 `always`/`never`/`once` 字符串已废弃。
...
| **等人项（pending entry）** | 挂起项（2026-09-19 退役——「挂起」已是 suspend 的主术语，两个意思撞名） | 一次正在等人答的调用（一张[审批卡片](#四审批human-in-the-loop)或一个 ask-user 提问）在服务端**内存里**的那一项（`@runko/agent` 的 `PendingEntry`）。它有四种解开方式：人答了、等满[内存窗口](#十三架构分层)后[挂起](#十三架构分层)、这一轮被停止、交权时立刻挂起。挂起之后内存里就没有它了，改由[裁决表](#三数据存哪怎么传)里那一行接着等人。 |
| **会话级授权（conversation grant）** | once 记忆、session grant（2026-08-22 退役——`session` 指聊天会话时已于 2026-07-17 整体退役，这里是漏网的一处） | 用户在审批卡片上点「会话内都允许」后落下的一条放行记录：**本会话内、该用户**后续同样的调用直接放行、不再弹卡片。记账粒度按工具分两种——bash 走**分段授权**（按[命令段](#四审批human-in-the-loop)记，见 docs/logic/engine/features/approval-grant-split.md），其余工具（及拆不动的 bash）仍按**完全相同的调用**（同工具 + 同入参指纹）记。**持久化**到 `conversation_grants` 子表、按 (会话, **用户**, 工具, 入参指纹) 记账——随会话存续、跨进程重启存活、会话删除即随 conversation 级联清（`clearConversationGrants` 为显式清理入口）；`user_id` = 审批人，查时按**本轮发起者**匹配，为将来一个 conversation 多用户时「每人管自己的授权」留好数据。落在 chat 层（`apps/node-server/src/agent/conversation-grants.ts`），wire 上是 `POST .../approvals/:callId` 的 `behavior:'allow-session'` 裁决（**wire 字面量刻意不跟着改名**：它说的是「用户点了哪个按钮」这个产品动作，与框架裁决表里那个表示**范围**的 `scope:'conversation'` 分属两层，各自都对）——与 core 的 `review-once`「once 记忆」是**同一意图的两条实现路径**（core 那条按工具名、由策略驱动、纯内存；这条按具体调用、由用户在卡片上驱动、持久化到会话，chat 分类器够不着 core 的 once 记忆，故自建）。 |
| **命令段（command segment）** | 子命令 | 一条 bash 命令行按 `&&` / `\|\|` / `;` / `\|` 切开后的**一条简单命令**——含它自己的参数、重定向与工作目录。`cd /repo && rm -rf build` 是两个命令段。切分由 `apps/node-server/src/agent/split-command.ts` 做，**看不透的写法一律不切**（见 docs/logic/engine/tech/approval-grant-split.md §3.4 拒绝清单）。 |
| **分段授权（segmented grant）** | — | [会话级授权](#四审批human-in-the-loop)的记账粒度：按**命令段**记，而不是按整条调用的入参指纹。人点「会话内都允许」时，这次调用切出的每一段各记一行；后续调用**每一段都记过**才自动放行，有任何新段仍照常弹卡片。记的是段的 argv 数组 + cwd + 重定向（不是命令名，也不是段的字符串原文——`rm -rf "my dir"` 与 `rm -rf my dir` 必须是两个键）。见 docs/logic/engine/features/approval-grant-split.md。 |
...
| **工作区（workspace）** | — | 沙盒对 runko 暴露的统一对象，同时实现文件读写（RunkoFS）和命令执行（RunkoExec）。 |
| **RunkoFS / RunkoExec** | — | runko 定义的两个接口：文件系统操作 / 命令执行。谁实现了它们，谁就能当 agent 的工作区。 |
| **原生搜索（native search）** | — | 沙盒适配器在沙盒内一次命令执行完成的搜索快路径，由 RunkoFS 可选方法 `searchFiles`/`searchContent` 承载；未实现或不可用时内置 `grep`/`glob` 回退 JS 逐文件扫描。 |
...
| **保活（keepalive，实现为 `ensureLifetime`）** | touch、续期 | 主动把沙盒剩余[存活时长](#五沙盒与生命周期)补到目标值。语义是**补足**不是加时：够了就什么都不做，不够才补差额。实现下沉在[沙盒适配器](#五沙盒与生命周期)里，厂商差异封在适配器内部（见 [沙盒保活（keepalive） · 功能](./logic/orchestration/features/sandbox-keepalive.md)）。touch 是它取代的旧叫法。 |
| **活动信号（activity signal）** | — | core 在一轮产出 chunk 时通知[工作区](#五沙盒与生命周期)「这一轮还在干活」的可选回调（`RunkoActivityAware.onActivity`）。同步、不返回值、绝不抛错；工作区没实现这个方法就什么都不会发生。 |
...
| **优雅关闭（graceful shutdown）** | — | 服务端进程收到 SIGTERM/SIGINT（热重载、部署、pod 迁移、Ctrl-C）后，先把进行中的[轮](#一agent-运行的基本单位)主动[停止](#一agent-运行的基本单位)并等它收尾、再退出，而不是让它无声消失。落地为 `turn-runner/shutdown.ts` 的 `shutdownTurns` + `index.ts` 的信号处理。见 docs/logic/orchestration/features/graceful-shutdown.md。 |
| **孤儿轮（orphaned turn）** | — | 一个在[账本](#三数据存哪怎么传)里从未收尾的轮：驱动它的进程已经不在了（被强杀、OOM、断电），所以那条收尾 `message-metadata` 永远不会到来。服务端启动时扫出它们并补上「已停止」标记。**识别特征是[起轮标记](#十三架构分层)**：起轮时往库里写一行「这一轮由谁在跑」，收尾时删掉；启动扫描时库里还留着标记 = 那一轮没人管了（`runtime.recover()`）。旧的间接判据（事件行以 `kind='chunk'` 收尾）已随[进行中草稿](#三数据存哪怎么传)搬进内存而退役——账本里不再有 chunk 行，那条判据恒为假。 |
| **代码快照（checkpoint）** | — | P13-2 计划：每轮结束把工作区完整状态推到用户仓库的快照引用，防平台快照过期丢未 push 的工作。 |
| **快照引用（checkpoint ref）** | WIP ref、隐藏 ref | 存放代码快照的自定义 git 引用（如 `refs/runko/wip/<会话id>`），不在正常分支命名空间下：GitHub 界面看不到、不触发 CI。与代码快照同族——快照存进快照引用。 |
| **VirtualFS（虚拟文件系统）** | — | runko 的 RunkoFS 抽象的具体实现族——MemoryFS（纯内存）/ OverlayFS（真实目录零拷贝 overlay）/ 自定义实现；agent 视角是普通文件系统，宿主视角是可检查、可导出（diff/writeBack）、可丢弃的对象。 |
...
| **物化（materialize）** | — | `localExec({ materialize: true })` 的模式 B：执行命令前把 VirtualFS 内容写到随机临时目录、执行后按 mtime 把变更收回 overlay——runko 负责一致性的便利实现，非安全边界。 |
| **模式 A（同源工作区）** | 同源工作区 | 一个对象同时实现 RunkoFS 与 RunkoExec、文件真身只有一份（在沙盒里）的执行模式；文件工具与 bash 的一致性是结构性的、无需同步逻辑。沙盒专指这种模式（云沙盒三家适配器即此形态）。 |
| **模式 B（物化执行）** | — | 文件真身在内存虚拟 FS 里、执行命令前临时物化到真实磁盘、结束后按 mtime 收回的本机便利模式；一致性由 runko 维护，命令跑在真机上故默认逐条审批。 |
...
| **crash ref（事故留底引用）** | — | `refs/runko/crash/<sessionId>`，turn-runner catch 分支把崩溃残局尽力推到的 git 引用；恢复流程永不读取它，仅供人工打捞。 |
...
| **skill 清单（skill catalog）** | — | 一个会话当前可选的 skill 集合（`{name, description}[]`）。事实来源是沙盒 `.agents/skills/` 目录，缓存在 `conversations.available_skills_json` 里供前端列菜单（读缓存不唤醒沙盒，故最多滞后一轮）。见 docs/ingress/features/composer-skill-mention.md。 |
| **kubb 重生成** | openapi 重生成 | 服务端 API 契约改了之后，用 kubb 重新生成前端的类型/客户端代码。 |
| **BYO 实例（Bring Your Own）** | — | runko 不创建/不销毁沙盒，只接管宿主已创建好的实例；生命周期归宿主。 |
...
| **ask-user（工具）** | — | 内置工具/产品能力：agent 在一轮进行中直接向用户提问（可带快捷选项）并阻塞等回答，回答后本轮继续；超时返回一段提示文案而非报错，模型自行决定继续。与审批无关，是产品能力不是安全闸。缺省注册，宿主可用 `createAgentRuntime({ human: { askUser: false } })` 关掉。 |
| **联网搜索（web search，工具名 `web-search`）** | exa 搜索 | chat 应用注册给 agent 的查网工具（不是 [core 内置工具](#九内置工具)，只活在 `apps/node-server`）：一句自然语言查询 → 若干条「标题/网址/日期/摘录」。后端是 Exa 的 `/search`，`EXA_API_KEY` 没配就不注册、模型看不见。只读、不弹审批。与[原生搜索](#五沙盒与生命周期)（搜沙盒里的文件）是两回事。见 docs/logic/engine/features/web-search.md。 |

## 七、上下文压缩（compaction）

> 见 [上下文压缩（compaction） · 功能](./logic/engine/features/compaction.md)。
...
> 见 [core-sdk · 功能](./logic/engine/features/core-sdk.md)。
...
> 见 [内置工具（builtin tools） · 功能](./logic/engine/features/builtin-tools.md)。

| 主术语 | 同义词（退役） | 大白话定义 |
|---|---|---|
| **内置工具（builtin tools / builtinTools）** | — | runko 出厂自带、宿主不写一行代码就能用的一组 agent 工具：文件八件套（read/write/edit/delete/move/list-dir/glob/grep）+ update-plan 为可裁剪内置（由 `builtinTools` 选项控制），bash / load-skill 为条件内置。 |
| **条件内置（conditional builtin）** | — | 一类内置工具：不由 `builtinTools` 数组控制，而是当对应能力被配上时才出现在工具列表——注入 RunkoExec 时出现 bash，配置了 skills 时出现 load-skill。 |
...
> 见 [示例集 examples（实验田） · 功能](./misc/features/examples.md) · [verification · 施工进展](./misc/plans/verification.md)。

| 主术语 | 同义词（退役） | 大白话定义 |
|---|---|---|
| **示例集（examples，实验田）** | — | 仓库里的 `examples/` 目录：一块打开即用的 pnpm workspace 成员，十二个可独立运行的示例脚本各演示 runko 一块核心能力；用户根目录 `pnpm install` 后 `pnpm example <编号>` 即跑。「实验田」是它面向用户的定位——低门槛把玩各能力的地方。 |
...
> 见 [Chat 界面语言 · 功能](./ingress/features/chat-ui.md)。

| 主术语 | 同义词（退役） | 大白话定义 |
|---|---|---|
| **ai-elements** | — | Vercel 官方的 AI 界面组件库（建在 shadcn/ui 上，从 registry 取到本地、代码归你）。chat 页面的消息流、工具卡片、推理块、审批、待发队列、输入框全部建在它上面；runko 特有的部分（中文状态词、工具计时条、三值裁决、插进本轮）作为薄封装留在 `features/chat/components/`。 |
...
| **skill 提及（skill mention）** | — | 用户在 [composer](#十一界面语言chat-页面) 里打 `/` 唤出[skill 清单](#六其它)、选中后插入的一枚原子标记块，形如 `/frontend-design`——告诉 agent「这件事按这个 skill 来做」。删除是整枚一起删，不会剩半截字符。wire 上它就是消息文本里的普通字符串，不是独立字段（这样[回放](#三数据存哪怎么传)时天然跟着走）。见 docs/ingress/features/composer-skill-mention.md。 |
| **sm 档密度** | — | 本项目对 ai-elements 出厂间距统一收一档的调校（消息 gap-8→gap-4、工具卡片 p-4→p-2.5 等）。改在**组件本体**里而不是调用点，否则下次重新 `add` 组件就全丢了。改动清单见 [tech/chat-ui §4](./ingress/tech/chat-ui.md)。 |
...
> 见 [推送通知（push notification） · 功能](./ingress/features/push-notification.md)。
...
| **在场（presence）** | — | 「此刻这个用户正盯着这条会话」——页面可见 + 窗口聚焦 + 路由停在这条会话，三个条件缺一不可。只有页面自己知道，所以由前端每 20 秒心跳上报，服务端内存记 45 秒有效期。**两个用途**：① [前台抑制](#十二推送通知)（人在跟前就不发推送）；② 延长[内存窗口](#十三架构分层)（人还盯着就别急着[挂起](#十三架构分层)，`runtime.reportPresence`，⬜ 随 K3 落地）。两处都要求它基于**真实交互事件**——拿 TCP 连通性当在场，会让半夜挂着页面的浏览器把会话永远钉在内存里。 |
...
> 见 [架构总纲 · 功能](./architecture/features/agent-kernel.md)。

> 这一节的词来自 [agent 内核包](./architecture/features/agent-kernel.md) 的分层设计。一条总规律：**凡是可替换的东西，语义在 agent 逻辑层、实现在宿主层。**
...
| **执行引擎（execution engine）** | — | 逻辑层最底下那个模块：给定历史和工具，调模型 → 跑工具 → 喂回去，直到模型说完。就是 `@runko/core` 干的事。它**不知道时间、进程、存储**，也不碰[账本](#三数据存哪怎么传)——拿到的是已经读好的历史。保证的性质叫**推进**。 |
...
| **持久化（persistence）** | 存储层、数据层 | 宿主层的一个模块：把[账本](#三数据存哪怎么传)、人工裁决、[待发队列](#一agent-运行的基本单位)、租约表**存起来**的能力。**它不是逻辑层的一层**——这四样分属不同模块，但「存起来」是同一种能力，所以是宿主层的一个模块（逻辑层定义模型，宿主层负责存，形态同 better-auth）。落在 `@runko/persist-*` 三条腿上，见 [持久化（宿主层） · 技术方案](./host/contract/tech/persistence.md)。 |
| **推进（progress）** | — | [执行引擎](#十三架构分层)保证的性质：给定历史和工具，一直跑到模型说完。 |
| **连续（continuity）** | — | [轮编排](#十三架构分层)保证的性质：一[轮](#一agent-运行的基本单位)接一轮连成一段连贯的对话。 |
| **可达（reachability）** | — | [接入](#十三架构分层)保证的性质：外部能触达、能观察。它由构建者保证，不在框架里。 |
...
| **租约（lease）** | — | 租约版[归属仲裁机制](#十三架构分层)里「这份对话此刻归谁」的那一行记录（`agent_leases` 表）：持有者地址、[租期标识](#十三架构分层)、账本水位、最后一次续约的时刻。持有者要按[租约心跳](#十三架构分层)不停续；超过[接管阈值](#十三架构分层)没续，别的副本就可以抢走。**释放不删行**，只把持有者与令牌置空。 |
| **租约心跳（lease heartbeat）** | — | 持有[租约](#十三架构分层)的副本每隔 `heartbeatMs`（缺省 5 秒）往租约行写一次当前时刻，证明自己还活着。**与[心跳（keepalive heartbeat）](#五沙盒与生命周期)不是一回事**——那个是给沙盒保活的，这个是给归属续约的。 |
| **接管阈值（takeover threshold）** | 判死阈值 | [租约](#十三架构分层)多久没续就算持有者已经死了、允许别的副本抢占（`takeoverMs`，缺省 60 秒）。**必须 ≥ 3 倍[租约心跳](#十三架构分层)间隔**，否则构造时就抛——一次普通的调度延迟不该被当成死亡。 |
| **自我围栏（self-fencing）** | — | 持有者发现自己的[租约心跳](#十三架构分层)一直没打通、离[接管阈值](#十三架构分层)只剩一拍时，**不等任何人通知就主动停手**（中断正在跑的轮）。账本有[租期标识](#十三架构分层)挡着不会写坏，但沙盒挡不住，两个执行同时改一个工作区就是互相踩文件，所以要自己先停。 |
| **挂起（suspend）** | 切碎（讨论期叫法，2026-08-09 退役） | 等人等太久时，把这一轮**落盘退出、释放机器**，人回来之后在任意节点接着干。**唯一的挂起点是「正在等人」**——那是唯一的干净边界（没有命令在跑、模型没在流式输出、沙盒也没在动）。挂起是**无损**的：记录、裁决、沙盒快照都还在，代价只是唤醒沙盒的几秒。✅ **已实现**（路线图 K3，2026-09-19）：等满[内存窗口](#十三架构分层)（默认 5 分钟）就挂起；四档库跨副本恢复都验过，chat 应用已接入。见[挂起与恢复 · 施工进展](./logic/orchestration/plans/suspend-resume.md)。 |
| **恢复（resume）** | — | [挂起](#十三架构分层)的反面：人回来答了，**开新的一轮**把那次悬着的调用接上——用[账本](#三数据存哪怎么传)里**原封不动的参数**执行，可以落在任意节点。⚠️ **这个词在本表里另有三个不相干的用法，别混**：[SessionState](#三数据存哪怎么传) 的下轮 `resume`（把上一轮的会话快照读回来）、[重连令牌](#五沙盒与生命周期)（resume token，按名字唤醒沙盒）、崩溃恢复（`recover()` 扫[孤儿轮](#五沙盒与生命周期)补「已停止」，**恰恰是不恢复执行**）。✅ 已实现，与挂起同批。 |
| **推一把（advance）** | — | 「这个会话接下来该干什么」的唯一入口（`advance(conversationId)`）：本进程已有轮在跑就不管；账本末尾有悬空调用，就找一个人已经答过的，开一轮[恢复](#十三架构分层)；否则照旧出队一条普通消息。**每一轮收尾、人答完、挂起期间来消息**三处都会推一把。交汇点在数据库里（答案先写进[裁决表](#三数据存哪怎么传)，占着归属的那一轮收尾时再读），所以跨副本也不会漏。⚠️ 别跟执行引擎的性质「[推进](#十三架构分层)（progress）」混——那是「一轮跑到模型说完」。 |
| **内存窗口（memory window）** | — | [挂起](#十三架构分层)之前先在进程内存里等人的那一段（`suspend.memoryWindow`，缺省 5 分钟；配 `0` = 立刻挂起）。旧参数 `human.approvalTimeoutMs` / `human.askUserTimeoutMs` 是它的废弃别名，各管一路。**存在的理由**是绝大多数审批人在几十秒内就点了，为这部分人付「落盘退出 + 重新抢归属 + 唤醒沙盒」那几秒不划算。缺省值沿用[审批保活预算](#五沙盒与生命周期)，**但不是同一件事**——那个管沙盒还愿意续多久，这个管框架还愿意等多久；对齐只是为了不出现「沙盒已休眠、框架还在傻等」。 |
...
| **起轮标记（turn ownership mark）** | — | 起轮时往库里写、收尾时删掉的一行「这一轮由谁在跑」。它是[归属仲裁机制](#十三架构分层)那张归属表的**最小形态**——只有「谁在跑」，还没有心跳、没有[租期标识](#十三架构分层)。**它不是「为多机预留」**：[进行中草稿](#三数据存哪怎么传)一搬进内存，旧的[孤儿轮](#五沙盒与生命周期)判据就失效了，所以它是那一步的直接依赖，必须同批上线。见 [进行中草稿放内存 · 技术方案](./logic/orchestration/tech/in-flight-draft.md) §5.9。 |
| **应用层转发（application-level forwarding）** | sticky routing、会话亲和 | 请求打到的实例不是[归属](#十三架构分层)持有者时，由[接入](#十三架构分层)代码把它转给持有者（框架给不透明的 `holder`，转发由构建者写）。**刻意不依赖基础设施提供 sticky**。亲和的单位是**「一次排空」**——不是一轮，也不是会话永久绑定。 |

## 十四、可观测性

> 见 [可观测性 · 功能](./architecture/features/observability.md)。这一节的词讲的是「框架怎么告诉宿主发生了什么」；
> 与[遥测（telemetry）](#五沙盒与生命周期)不是一回事——遥测是 AI SDK 自己的回调接口，只管模型调用那一段。

| 主术语 | 同义词（退役） | 大白话定义 |
|---|---|---|
| **可观测性事件（runko event）** | 日志（框架内那 42 处 `logger.*`）、运行时钩子（`RuntimeHooks`） | 框架在关键时刻报出的一条**带类型的纯数据**：「发生了什么 + 相关字段 + 什么时刻」，比如 `turn.started`、`lease.taken-over`、`approval.pending`。框架**只报告、不决定怎么处理**——记不记日志、记什么级别、要不要发推送或建 span，全由[订阅者](#十四可观测性)决定。取代了原来的注入式 `Logger` 与 `RuntimeHooks`。 |
| **订阅者（observer）** | — | 宿主交给框架的一个对象，收[可观测性事件](#十四可观测性)。三个方法都可选：`onEvent`（收事件）、`wrap`（把一段执行包进自己的上下文，见[包裹](#十四可观测性)）、`capture`（入队时抓[调用链载体](#十四可观测性)）。**实例注入**：`createAgentRuntime` 与宿主自己构造的组件（租约仲裁等）各收一个 `observers` 数组。订阅者抛错一律吞掉，不影响这一轮。 |
| **包裹（wrap）** | — | [订阅者](#十四可观测性)可选实现的一个方法：框架把「一整轮」或「单次工具执行」这段代码交给它，由它在自己的上下文里执行。OTel 靠它把「轮」设成当前 span，AI SDK 在这一轮里建的模型调用 span 才会自动挂到下面。只在这两处有，叶子操作（租约、写账本）不包。 |
| **调用链载体（trace carrier）** | — | 一段**不透明的字符串键值**（比如 W3C 的 `traceparent`），代表「触发这件事的那次请求在调用链里的位置」。消息进[待发队列](#一agent-运行的基本单位)时由[订阅者](#十四可观测性)的 `capture` 抓出来，随输入存进队列项；出队起轮时交还给订阅者，用来把这一轮和原请求连起来（OTel 里是 span link）。框架只存、只传、不解释。 |
| **runko 通道（runko channel）** | — | 框架在有 `node:diagnostics_channel` 的运行环境里**额外**发布事件的出口，名字以 `runko:` 开头（`runko:turn`、`runko:tool` 两个 `TracingChannel`，加一个收全部点状事件的 `runko:event`）。进程级、零配置：Sentry 这类厂商直接订阅就能收到，不用改宿主代码。没人订阅时不发布；运行环境没有这个模块时整个出口不存在。 |
