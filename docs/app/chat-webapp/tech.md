...
> 相关：[产品视角](./feature.md) · [施工进展](./plan.md) · 可观测性（工具计时 + server 日志，§11）拆单见 [app/chat-observability/plan](../chat-observability/plan.md)
> 依赖：[core-sdk](../../core/core-sdk/feature.md)（`@nimbo/sdk`/`@nimbo/core`：[agent](../../terms.md)/[loop](../../terms.md)/[session](../../terms.md)/[chunk](../../terms.md)）· [sandbox](../../host/sandbox/feature.md)（`@nimbo/sandbox-vercel`：[工作区](../../terms.md) = [NimboFS / NimboExec](../../terms.md)）
> 被增强：[turn-checkpoint](../../agent/turn-checkpoint/feature.md)（[代码快照](../../terms.md)/[保活](../../terms.md)）· [single-ledger](../../agent/single-ledger/feature.md)（[UIMessage 单账本](../../terms.md)，P13-5 已落地）· [compaction](../../agent/compaction/feature.md)（[上下文压缩](../../terms.md)）
...
> **本文是「现状实现」，不是设计权威。** 下面 §2（账本表）·§3（起轮与驱动）·§5（断线可续的流）·§6（人在环上）讲的这几件事，[agent 层](../../agent/README.md)已经有了统一设计（[agent/agent-kernel](../../agent/agent-kernel/feature.md)），chat 应用是它的**第一个宿主**、也是这些机制的试验田。**两者有出入时以 agent 层为准**，本文只回答「今天的代码是怎么写的」。逐条对照与迁移阶段见 [agent/agent-kernel/plan 表 A/表 B](../../agent/agent-kernel/plan.md#从-chat-应用吸收2026-08-10-逐条对照)；正文里已知会变的地方都就地标了 ⚠️。

---

## 1. 布局与工作区（结构性决策）

- seed 的 `apps/client` 更名为 **`apps/web`**，seed 的 `apps/server` 保留（2026-07-20 更名为 **`apps/node-server`**，与 [`apps/cloudflare-worker-server`](../cloudflare-worker-server/tech.md) 形成 node/worker 两个服务端形态的对称命名）；`apps/docs` 不引入。包名 `@nimbo-chat/web`、`@nimbo-chat/node-server`。
...
**两类条目的含义（docs/agent/single-ledger/tech.md §5 单-3）**：
...
> ⚠️ **这张表有两处会按 agent 层的设计改**：
>
> - **`kind='chunk'` 整类条目会消失**——[进行中草稿放内存](../../agent/in-flight-draft/tech.md)之后账本只留完工消息，agent 的 `LEDGER_ENTRY` 也只有一种 `kind`（K1）。
> - **`seq` 不再由 `MAX(seq)+1` 取号**——改由[归属仲裁](../../terms.md)从租约表水位 + CAS 分配，并且**明文允许空洞**（`MAX(seq)+1` 是方言特性，通用持久化适配器表达不了，见 [agent/agent-kernel/tech §3](../../agent/agent-kernel/tech.md)）（K6）。
>
> 会话行上的应用字段（`title`/`repo`/`branch_name`/`sandbox_name`/`status`/属主）**不进框架**——nimbo 只认不透明的 `conversationId`/`ownerId`。
...
- **`web-search.ts`**：[联网搜索](../../terms.md)工具（`web-search`，后端 Exa `/search`）——应用级工具，**条件注册**：`EXA_API_KEY` 有值才进 `agent.tools`，没配则模型看不见它。与 `ask-user` 同构、core 零改动，详见 [app/web-search/tech](../web-search/tech.md)。
...
- **`routes/chat.ts`**（`@hono/zod-openapi`，十二个端点，全登录态强制）：见 §7 契约。本页只写其中的会话/直播流/审批那几个；[待发队列](../../terms.md)的三个见 [agent/steer-and-queue/tech §4.2](../../agent/steer-and-queue/tech.md)，`POST .../abort`（[停止](../../terms.md)本轮）见 [agent/turn-abort/tech §3.2](../../agent/turn-abort/tech.md)。
...
形状与[待发队列](../../terms.md)快照帧完全同构（见 [steer-and-queue §4.3](../../agent/steer-and-queue/tech.md)）：**没有 `seq`**、不落库、不进[账本](../../terms.md)、不参与 `after=` 续传——它是「此刻状态长这样」，重发一次即最新。前端收到后设 `turnInProgressRef`/`status`（`applyTurnState`）。
...
> 🔀 **这类帧要进 agent 层的流分发契约**：`turn-state` 与[待发队列](../../terms.md)快照都是「此刻状态长这样」——**无 `seq`、不落库、不进[账本](../../terms.md)、不参与 `after=` 续传**，每条连接发一次。agent 的 `subscribe` 目前只考虑了账本帧，缺了这一类前端就又得回去猜。记在 [agent/agent-kernel/plan 表 B 第 5 行](../../agent/agent-kernel/plan.md#表-bchat-应用有本方案还没有待补)。
...
> ⚠️ **超时那半句会被取代**：agent 层的定案是等不到就[挂起](../../terms.md)（内存窗口默认 5 分钟），裁决落库、机器放掉，人回来在任意节点恢复——`CHAT_APPROVAL_TIMEOUT_MS` 那条「到点自动 deny」是当前实现的过渡（K3）。
...
- **ask-user 工具**：`BuildSessionOptions.onAskUser` 存在时注册进 `agent.tools`（kebab-case `ask-user`）；`execute` 经 `requestUserAnswer`/`resolveUserAnswer` 同款桥挂起（`pendingQuestions`，独立于 `pendingReviews`）。可见性=`tool-ask-user` 部件自身的 `input-available`/`output-available` 状态（普通工具调用）。超时（`CHAT_ASK_USER_TIMEOUT_MS` 默认 240s）返回固定提示文案（`status:"completed"`，不抛错，模型自行继续）。**与 approvalMode 无关恒注册**（产品能力，不是安全闸）。
  > 🔀 **agent 层还没覆盖这条通道**：它的「人」目前只设计了审批裁决。但 ask-user **同样是等人**，同样该走挂起/恢复——差别只在答案的形状（一段自由文本 vs allow/deny）。见 [agent/agent-kernel/plan 表 B 第 1 行与还没定的 6](../../agent/agent-kernel/plan.md#还没定的)。
...
> ⚠️ **本节这条判据在挂起上线后会失效**。现在的推导是「轮不在跑 ⇒ 挂起项必然已结」；而[挂起](../../terms.md)恰恰打破它——那一轮**已经收尾了**（以「已挂起」结束），但它等的那条裁决**仍然有效、还在等人**。替代判据是看裁决记录本身（`DECISION.decidedAt` 为空 = 仍待定），不是看轮活没活。记在 [agent/agent-kernel/plan 表 B 第 6 行](../../agent/agent-kernel/plan.md#表-bchat-应用有本方案还没有待补)。
...
1. **恢复语义的兜底链**：快照恢复（快）→ 快照过期重 clone + checkout 已 push 分支 → 分支从未 push 则重建分支（**未提交工作丢失**，UI 如实提示）——最后一档是 Vercel 快照 TTL 的客观约束。[turn-checkpoint](../../agent/turn-checkpoint/feature.md) 拟用[代码快照](../../terms.md)推[快照引用](../../terms.md)补掉这个窗口。
...
**耗时单一来源**：`tool-output-available` 日志里的耗时**不是** turn-runner 自己另掐一个计时器，而是读 loop 已经产出的 `data-tool-timing` chunk（见 [agent/single-ledger/tech](../../agent/single-ledger/tech.md) §3.2 的三段生命周期）——避免两处计时器分叉出两个不一致的数字。`durationMs = completedAt - executionStartedAt` 是**真实执行耗时**（2026-07-16 按用户反馈修订，不再含排队/审批等待）；`queueMs = executionStartedAt - startedAt` 单列排队/审批等待，两者相加才是用户在界面上看到的全程等待。`tool-approval-response` 日志额外单独记一个「请求到响应」的等待时长（server 旁路时钟），供运维判断「慢在人审还是慢在执行」。
...
遥测已拆为独立功能文档（2026-07-17）：完整技术方案（存储/ER 图/有效期/时序图/接口/取舍）见 **[app/telemetry/tech](../telemetry/tech.md)**，产品行为与数据清单见 [app/telemetry/feature](../telemetry/feature.md)。与本页相关的落位一句话版：core loop 每次 `streamText` 恒注入 `functionId="<nimbo 会话 id>#<turn>"` 关联键并补发工具执行事件，server 经 `ChatRouteDeps.telemetry`（写）/`telemetryStore`（读）装配 SQLite 集成（`src/telemetry.ts`，独立 `telemetry.db`），读侧端点 `GET .../turns/{turn}/telemetry` 供 `TurnStatsButton`（完成轮末尾的「统计」按钮，点开「本轮统计」弹窗）的明细按需拉取——**弹窗概览值仍来自账本 metadata（产品数据），遥测只做增强**，关闭遥测产品功能零损失。

---

## 附录 A：历史设计（P13-5 之前的 wire 事件模型，保留作动机记录）

> 以下是 §2–§7 现行 chunk 帧流模型的**前身**。P13-5（[single-ledger](../../agent/single-ledger/feature.md)）已用 UIMessage 单账本 + chunk 帧流取代整套 wire 事件模型；解耦 turn 与连接、可续传 tail、durable/ephemeral 分层的**机制沿用**，只是词汇从「自定义 wire 事件」换成「ai 原生 chunk」。保留是为了不丢当时的设计/取舍/动机。
