...
> 依赖：[core-sdk](../../core/core-sdk/feature.md) · [sandbox](../../host/sandbox/feature.md)
> 被增强：[turn-checkpoint](../../agent/turn-checkpoint/feature.md) · [single-ledger](../../agent/single-ledger/feature.md) · [compaction](../../agent/compaction/feature.md)
> **会话生命周期那半边的设计权威在 [agent 层](../../agent/README.md)**（起轮驱动、账本、断线续传、等人、崩溃恢复）——本页只记 chat 应用自己的施工。将来「迁到 `@nimbo/agent`」是那边的 [K7 阶段](../../agent/agent-kernel/plan.md#阶段拆单)，不在本页拆单里。

> **立项 banner**：已立项开工——2026-07-12 用户 `/goal` 直接立项。
> 原型来源：[sandbox 端到端示例](../../host/sandbox/feature.md)（沙盒 E2E 示例 = 本应用的核心能力原型）。
...
- **P13-1 transcript 减量**（durable/ephemeral 分层，✅ 已交付）。
- **P13-5 单一数据账本**被定为**最高优先级**：UIMessage 为唯一真相，验证实验方案见 [single-ledger](../../agent/single-ledger/feature.md)，通过后 core 迁移正式立项——**已落地**（P13-5-1..6，见 [agent/single-ledger/plan](../../agent/single-ledger/plan.md) 与 [agent/single-ledger/tech](../../agent/single-ledger/tech.md)）。
- **P13-2 turn checkpoint 与 P13-2b 沙盒保活顺延**（方案见 [turn-checkpoint](../../agent/turn-checkpoint/feature.md)，若单账本落地其对齐语义将简化）。
- **P13-3 中断哨兵、P13-4 留存治理**待讨论。
...
- **2026-07-27 混合帧回放的顺序修复：崩溃轮跑到新轮后面去了（✅）**：用户实测截图——最新完成的那一轮显示在**上面**，22:14 那个被服务重启中断的旧轮显示在**下面**。根因在 `materialize.ts` 的 `MessageLedger`：渲染顺序由 `order` 数组决定，而 `order` 记的是**「upsert 到达的顺序」而不是 wire 顺序**。两种帧的物化时机差着一个微任务——`MessageFrame` 走 `upsert` **同步**落位，`ChunkEnvelope` 要经 `readUIMessageStream()` **异步**吐出消息才 upsert。于是回放一段「先是崩溃轮的 chunk 行、后是新轮的 message 行」的历史时，同步那批先把 `order` 占满，异步物化的旧轮消息只能排到末尾。**这个洞一直存在，只是没机会暴露**：崩溃的轮从前总是账本里的最后一轮（崩溃即终止），后面没有别的轮，排到末尾看不出问题；[优雅关闭](../../agent/graceful-shutdown/feature.md)让崩溃轮之后还能继续对话，它就浮出来了。**修法**：新增 `ensureOrder(id)`，在**同步**可见的 `start` chunk（它已经带着 `messageId`）那一刻就把位子占下，异步物化的 upsert 只填内容、不再改顺序；`snapshot()` 跳过「占了位但还没物化」的格子（刻意不塞空消息占坑——那会闪一个空气泡）。steer 消息那条路同样在 `start` 时占位。**验证**：web 276 全绿，新增用例同样做了**反向验证**（临时移除 `start` 处的占位 → 该用例变红 → 恢复 → 绿）。
...
- **P13-5（✅ 已落地，2026-07-15 定案项）**：UIMessage 单账本取代整套 wire 事件模型——`agent_events` 改 `kind`(message/chunk) + `nimbo` 标量 header 取代 `nimbo_state_json`；审批改走 ai 原生 `tool-approval-request`/`tool-approval-response` chunk + `onReview` 人审通道 + `tool-ask-user` 部件；人工裁决收敛为纯两值（「改参数」口子 2026-07-15 删除）；工具名一律 kebab-case。历史 wire 模型详见[技术方案 附录 A](./tech.md)。
