...
> 依赖：[core-sdk](../../core/core-sdk/feature.md)（nimbo agent SDK，驱动对话的 [agent](../../terms.md)/[loop](../../terms.md)/[turn](../../terms.md)）· [sandbox](../../host/sandbox/feature.md)（Vercel 沙盒，代码真身所在）
> 被增强：[turn-checkpoint](../../agent/turn-checkpoint/feature.md)（每轮代码快照保活）· [single-ledger](../../agent/single-ledger/feature.md)（UIMessage 单账本，P13-5 已落地）· [compaction](../../agent/compaction/feature.md)（长会话上下文压缩）· [chat-ui](../chat-ui/feature.md)（chat 页面的界面语言，2026-07-25 改版）

> **界面呈现细节看 [chat-ui](../chat-ui/feature.md)**：本文讲这个应用**做什么**（端点、生命周期、人在回路的行为语义），chat-ui 讲它**长什么样**（[轨道](../../terms.md)、[打断](../../terms.md)、[信号色](../../terms.md)、[指令块](../../terms.md)）。下文出现的界面描述若与 chat-ui 冲突，以 chat-ui 为准。

## 与 `@nimbo/agent` 的关系（读之前先看这段）

这个应用里有一半能力，本质上是[会话生命周期](../../agent/agent-kernel/feature.md)——一轮接一轮怎么跑下去、等人怎么办、崩了怎么办。这些**设计权威已经移交 [agent 层](../../agent/README.md)**，本文只记 chat 应用**现在**是怎么落地的。

| 这一节讲的事 | 设计以谁为准 |
|---|---|
| 起轮 / 一轮的驱动与收尾 / 断线续传 / 崩溃后的收敛 | [agent/agent-kernel](../../agent/agent-kernel/feature.md) |
| 账本与回放 | [agent/single-ledger](../../agent/single-ledger/feature.md) · [agent/in-flight-draft](../../agent/in-flight-draft/tech.md) |
| 排队与插话 · 停止本轮 · 优雅关闭 | [agent/steer-and-queue](../../agent/steer-and-queue/feature.md) · [agent/turn-abort](../../agent/turn-abort/feature.md) · [agent/graceful-shutdown](../../agent/graceful-shutdown/feature.md) |
| 沙盒的取用、休眠与唤醒 | [host/sandbox](../../host/sandbox/feature.md) · [host/sandbox-keepalive](../../host/sandbox-keepalive/feature.md) |
| **本文独有**：仓库/分支/PR、注册登录与属主、审批的具体危险命令清单、界面行为 | 本文（构建者侧产品决策，nimbo 明确不管） |

**现状与 agent 设计已知不一致的三处**，下文出现处都就地标了：审批超时（现在 4 分钟作废，将来改成挂起）、进程重启（现在一律中断，将来「正在等人」那档可恢复）、轮边界的呈现。完整对照见 [agent/agent-kernel/plan 表 A/表 B](../../agent/agent-kernel/plan.md#从-chat-应用吸收2026-08-10-逐条对照)。

## 一句话

一个 chat agent 网页应用：用户在对话里驱动 nimbo [agent](../../terms.md) 在 Vercel [沙盒](../../terms.md)中修改真实仓库代码、开 PR、触发 Vercel 部署。它把 [sandbox 端到端示例](../../host/sandbox/feature.md) 的「沙盒里跑完整设计任务」示例产品化成一个可注册、可登录、可持续多轮对话的 Web 应用（apps/web + apps/node-server）。
...
5. **人在回路（[human-in-the-loop](../../terms.md)）**：
   ...
   - **一直不理会怎样**：审批卡片等满 4 分钟（可配）会**自动按「拒绝」处理**——只是那一次命令不执行，agent 会接着往下做（比如换个做法或告诉你它做不下去）。**这一轮不会因此被中止。** 提问卡片同理：超时后 agent 收到一句「用户没回应」的固定提示，自己继续。
     > ⚠️ **这条会被 agent 层取代**：[agent 内核](../../agent/agent-kernel/feature.md) §7.1 的定案是「内存窗口内等不到就[挂起](../../terms.md)」——机器放掉、进度落盘，人几小时后回来点「允许」照样接着跑，**不再有「N 分钟就作废」这回事**。4 分钟自动拒绝是当前实现的过渡行为。
...
服务端是 `@hono/zod-openapi` 定义的 REST + SSE 接口，**全部要求登录态**（同一套 `requireAuth`）。契约细节见[技术方案](./tech.md)，这里只讲每个端点「给用户/前端解决什么」：
...
| `POST /api/chat/sessions/{id}/messages` | 发消息：无进行中的一轮则**起新一轮**（`mode: "started"`）；已有进行中的一轮时看 `intent`——默认 `queue` 则**排队**（`mode: "queued"`），`steer` 则**注入当前轮**（`mode: "steered"`）。返回 202——事件不在这个响应里，去 `GET .../stream` 收。详见 [agent/steer-and-queue/feature](../../agent/steer-and-queue/feature.md)。 |
...
产品级验收（P12-3 真机验收链，详见[施工进展](./plan.md)）：
...
4. **进程重启会中断进行中的一轮**：沙盒还在，但驱动这一轮的内存态停了；已落库的内容保留到中断点，重连回放发现无进行中轮即静默收尾。这是 v1 内存态 runner 的已知取舍。**界面不会因此卡住**（2026-07-27 修）：重新打开这个会话时，服务端会明确告知「没有轮在跑」，输入框回到空闲形态、发消息照常起新一轮。此前它会一直显示成「正在跑」，于是发的消息全被排进待发队列却永远发不出去，按停止也没有反应。
   > ⚠️ **部分会被 agent 层取代**：[agent 内核](../../agent/agent-kernel/feature.md) §7.1/§7.4 把「**正在等人**」这一个干净边界改成挂起——那种情况下重启不再丢那一轮，人回来在任意节点接着跑。**跑到一半被强杀仍走中断老路**（没有干净边界）。
5. **不再有「第 N 轮」分割线**：单账本是连续的消息/帧序列，界面不再按轮切分渲染出显式边界。
   > 🔀 **与 agent 层待对齐**：挂起/恢复上线后，一次等待会拆成「以已挂起收尾的那一轮」+「从结清那个调用开始的下一轮」（[agent 内核](../../agent/agent-kernel/feature.md) §7.2）。挂起态至少要有一个用户看得懂的落点，怎么呈现见 [agent/agent-kernel/plan 还没定的 8](../../agent/agent-kernel/plan.md#还没定的)。
