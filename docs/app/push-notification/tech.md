...
> 术语见 [docs/terms.md](../../terms.md)。产品视角见 [docs/app/push-notification/feature.md](./feature.md)，施工进展见 [docs/app/push-notification/plan.md](./plan.md)。
> 依赖与被影响：[chat-webapp](../chat-webapp/tech.md)（[账本](../../terms.md)、[审批链](../../terms.md)、起轮装配）·[steer-and-queue](../../agent/steer-and-queue/tech.md)（[待发队列](../../terms.md)决定"一轮算不算结束"）·[sandbox-keepalive](../../host/sandbox-keepalive/tech.md)（[审批保活预算](../../terms.md)与审批超时必须一起定，见 §8）·[telemetry](../telemetry/tech.md)（本方案的接线姿态照抄它）
> **与 [agent 层](../../agent/README.md)的边界**：本方案留在 app 层不动；但 §8「审批超时与保活预算一起定」的前提会变——agent 层把等人改成[挂起](../../terms.md)（不再有超时作废），届时「要审批」这个时刻要改成订阅框架信号。见 [agent/agent-kernel/plan 表 B](../../agent/agent-kernel/plan.md#从-chat-应用吸收2026-08-10-逐条对照)。
...
**依赖方向是单向的**：`turn-runner/` 不认识推送，也不认识[在场](../../terms.md)——它只多**一个**生命周期通知点（`onTurnSettled` 多带一个"这一轮怎么结束的"）。拼载荷、查闸门、发投递全在 `push/` 下。这与[遥测](../telemetry/tech.md)当初的接法完全同构，理由也一样：运行内核不该长出对可选外围功能的认识。
...
- **不存偏好。** 这一期没有按事件类型的开关（[产品文档附录 A.2](./feature.md)）；服务端总闸是环境变量，设备级开关就是"这一行在不在"。
...
**这张图里最要紧的一条顺序**：`requestReview` 先跑，通知后发。审批卡片抵达用户的路径（SSE 直播）绝不能被推送这条可选支路拖慢或拖挂——同[遥测](../telemetry/tech.md)那条"观测绝不插在 chunk 送达用户的前面"。
...
`CHAT_PUSH_EVENTS`，逗号分隔，默认 `approval,question,turn-done,turn-failed`（全开）。改配置重启即生效，不需要界面（[产品文档附录 A.2](./feature.md) 说明了为什么这一期不做设置界面）。
...
等人的时候，沙盒得活着。管这件事的是[沙盒保活](../../host/sandbox-keepalive/tech.md)的[审批保活预算](../../terms.md)（`KeepAliveOptions.approvalBudgetMs`）——**卡在人工审批时最多还愿意为沙盒续多久**。
...
正确的一组改动（需要拍板，见[施工计划 PN-9](./plan.md)）：
...
4. **没有重试**：投递失败就是失败（§6.6）。
5. **通知里不能直接批准**（[产品文档附录 A.1](./feature.md) 说明了为什么这是有意的）。
