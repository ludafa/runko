...
> 产品文档 [docs/app/push-notification/feature.md](./feature.md)，技术方案 [docs/app/push-notification/tech.md](./tech.md)。
...
- `docs/app/push-notification/feature.md`、`docs/app/push-notification/tech.md`、本文件。
...
- `.env.template`：六个变量（[技术方案附录 A](./tech.md)）
...
- `apps/node-server/src/db/schema.ts`：新增表（字段见[技术方案 §2](./tech.md)）
...
2. TTL / Urgency / Topic 按[技术方案 §6.2–6.3](./tech.md) 设置；topic 长度 ≤32 且字符集合法。
...
1. SW 两个事件：`push`（弹通知，字段读取带 `typeof` 守卫，读不到用兜底文案）、`notificationclick`（focus 已有窗口 + postMessage，否则 openWindow）。**不调 API**（[技术方案附录 B.2](./tech.md)）。
2. 铃铛五态：`unsupported` / `off` / `on` / `blocked` / `busy`，各有明确的中文提示。iOS 非主屏打开走 `unsupported` 且提示"分享 → 添加到主屏幕"。
3. 页面加载时的幂等重上报（[技术方案 §3.1](./tech.md)）。
...
3. 文案与截断按[技术方案 §6.1](./tech.md)：会话标题 60 / 命令 100 / 问题 80。
...
> **这一项要你点头再做。** 详细论证见[技术方案 §8](./tech.md)。要点：
>
...
| # | 计划 | 实际 | 为什么 |
|---|---|---|---|
| 1 | 做 PWA（manifest + 图标），为了 iOS 能收 | **不做**，iOS 因此收不到 | 你 2026-07-27 定的：用不上"装成独立应用"这件事。图标已生成又删掉。补回来的做法写进了[技术方案附录 B.5](./tech.md) |
| 2 | 退订走 `DELETE /api/push/subscriptions`（body 带 endpoint） | 改成 `POST /api/push/unsubscribe` | endpoint 是几百字节的 URL，放 query 要编码，放 DELETE 的 body 又会被部分客户端/代理**静默**丢掉。见[附录 B.4](./tech.md) |
...
| 8 | PN-9 的论据：240 秒的理由已被 `startHeartbeat` 作废 | **论据换了**：`startHeartbeat` 已被删除，现在挡在中间的是[审批保活预算](../../terms.md)（默认 5 分钟） | 施工期间隔壁并行落地了[沙盒保活](../../host/sandbox-keepalive/plan.md)，手写心跳整个删掉。结论不变（4 分钟太短），但要动的旋钮从一个变成两个。详见[技术方案 §8](./tech.md) |
