...
> **相关**：产品手册见 [app/examples/feature](./feature.md)；技术方案见 [app/examples/tech](./tech.md)。
> **依赖**：原 13 号迁出的去向见 [agent/single-ledger/plan](../../agent/single-ledger/plan.md)；示例验证语义承接 [core/verification/plan](../../core/verification/plan.md)。
...
| 4 | 13 挪进 `@nimbo-chat/node-server` | 离线段改写成 vitest 用例进 CI；真机段留 git 历史（用户拍板产品归属在 chat，知情其语义实现在 core 后仍选 server） |
...
- **13 迁移**：新 server 测试 `uimessage-single-ledger.test.ts` 3 用例全绿；`@nimbo-chat/node-server` 全量 **11 文件 / 222 用例**通过；server typecheck exit 0。13 已 `git rm`，examples 根只剩 `run.ts`；全库无残留旧文件名引用（唯一出现是 `loop.ts:31` 有意的迁移指引）。
...
- **2026-07-19（追加）**：把 Cloudflare 网关从「可部署模板」重新定性为 example 11 的 **BYO 参考料**。起因：用户指出我们并不想提供一个「拎走即部署」的环境，只想要一个示例、CF 环境让用户自备；且澄清包需要的是**自己的测试能力**（已自足：`test/` 用 `FakeCfSandbox`+`fetchViaGateway` 进程内驱动 `createSandboxGateway`，约 48 用例，零 wrangler/真机/网络）。据此：`examples/cloudflare-gateway/` → `examples/cloudflare-gateway-ref/`，删 `package.json`/`tsconfig.json`（连带 `file:..`）、扁平化 `src/index.ts`→`index.ts`，README 改写为自备 CF 环境的参考料。同步 [app/examples/tech §6](./tech.md) 边界说明与 [app/examples/feature §3](./feature.md) 非目标条目，及各处路径引用。> 注：本条推翻了同日更早「网关归位到 `packages/sandbox-cloudflare/gateway/` 作为包的服务端交付另一半」那次改动（未提交即被本次取代）——收回的判断与理由记于 `.lantie_history`。
- **遗留**：`docs/core/verification/plan.md` §1/§2 仍有对已删 `setup-node-modules.mjs`/`typecheck.mjs` 的操作性引用与「examples 不是 workspace 包」旧框定——属 core-sdk 验证文档的历史记录，留其下次专门修订，未在本次跨功能改动。
- **2026-07-20**：上一条（2026-07-19）把 `examples/cloudflare-gateway-ref/` 定性为 example 11 的 **BYO 参考料**，本条如实记录该定性已被取代——该目录已删除，能力整体并入独立 workspace 成员 `apps/cloudflare-worker-server`（原是一个 SPIKE 性质的探针项目，现已转正为正式示例并随之更名，去掉 SPIKE 定性）。这是一个**完整可跑可部署的 workspace 成员**，同时扮演两个角色：①服务端自己在 Worker 里跑 nimbo 会话，进程内直连驱动真实 CF Sandbox；②对外提供 **BYO 网关端点** `ALL /gateway/*`，供任意 Node 机器上的 `cloudflareWorkspace({ url, token })` 连入——即 11 号真机段所需的那个网关。顺带修掉一个真实缺陷：`AbortSignal` 跨不过 Durable Object RPC 边界，网关转发 `request.signal` 会让真机报错，新 Worker 用 `stripAbortSignal` 绕过。因此「examples 不为 Cloudflare 提供拎走即部署的网关模板」这条旧非目标不再成立——现状见 [app/examples/tech §6](./tech.md)、[app/examples/feature §3](./feature.md)，以及该 app 自己的三份文档：[app/cloudflare-worker-server/feature](../cloudflare-worker-server/feature.md) · [app/cloudflare-worker-server/tech](../cloudflare-worker-server/tech.md) · [app/cloudflare-worker-server/plan](../cloudflare-worker-server/plan.md)。
