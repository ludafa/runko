...
> 依赖/延续：[chat 聊天 webapp](../chat-webapp/feature.md)（消息发送通路、[composer](../../terms.md)所在位置）· [chat 界面语言](../chat-ui/feature.md)（composer 现在是 ai-elements 的 `PromptInput`，本功能换掉它的输入区）· [核心 SDK](../../core/core-sdk/feature.md)（[skill](../../terms.md) 与[渐进式披露](../../terms.md)的定义在这里）。
...
详细的端到端验证步骤（含多 skill、休眠恢复、回归项）见 [施工进展](./plan.md) 的「验证方案」一节。
