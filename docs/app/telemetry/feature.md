...
> 相关：[技术方案](./tech.md) · [施工进展](../chat-observability/plan.md)（遥测属「chat 可观测性」拆单）
> 依赖：[chat-webapp](../chat-webapp/feature.md)（统计弹窗与查询端点长在 chat 应用上）· [core-sdk](../../core/core-sdk/feature.md)（`SessionTelemetry` 注入口在 SDK 层）
> 术语：[遥测（telemetry）](../../terms.md)，本文一律使用术语表主术语。
> **与 [agent 层](../../agent/README.md)的边界**：**存储与查询归构建者**，本功能留在 app 层。但轮级的耗时/用量（`durationMs`/`toolDurationMs`/token 四分）是**轮编排收尾时产出**的，将来由框架写进账本 metadata，遥测退化为纯消费方。见 [agent/agent-kernel/plan 表 B](../../agent/agent-kernel/plan.md#从-chat-应用吸收2026-08-10-逐条对照)。
...
chat 的账本数据（[single-ledger](../../agent/single-ledger/feature.md)）已经提供轮级聚合：统计弹窗概览里的总耗时、工具/agent 拆分、token 总量，以及每张工具卡片的起止时间。但它们回答不了这些问题：
...
3. **直接查库**：`sqlite3 telemetry.db "SELECT * FROM telemetry_events WHERE agent_session_id = ? AND turn = ?"`——运维排障的兜底入口，表结构见[技术方案](./tech.md)。
...
- **不做**长期保留承诺与自动清理策略（当前不会自动删，但随时可以手动清，见[技术方案](./tech.md)有效期一节）。
- **不做**对话正文的采集（见上）。
