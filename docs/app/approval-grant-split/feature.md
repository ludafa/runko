...
> 依赖：[chat 应用](../chat-webapp/feature.md)（[会话级授权](../../terms.md)是本功能改造的对象） · [chat 界面](../chat-ui/feature.md)（审批卡片）
> 术语一律引用 [docs/terms.md](../../terms.md)，本文不自建术语。
> **与 [agent 层](../../agent/README.md)的边界（本功能是两件事）**：
> ① **机制**——「跨轮记住一条更宽的授权、下次命中直接放行」是[跨执行的状态](../../terms.md)，**该归 agent 层**；而框架现在只有「一次调用一条裁决」，还没有可复用的授权（缺口记在 [agent/agent-kernel/plan](../../agent/agent-kernel/plan.md#两个容易归错层的-app-文档2026-08-11-复核)）。
> ② **策略**——「宽到哪、一条复合 bash 命令怎么拆[命令段](../../terms.md)、键怎么算」框架不该懂，**留本层**。本文讲的主要是这一件。
> 迁移后 wire 上的 `allow-session` 等价于 `outcome: allow + scope: broader`。
