...
> 依赖：[chat 应用技术方案 §6](../chat-webapp/tech.md)（[会话级授权](../../terms.md)现状）
> 术语一律引用 [docs/terms.md](../../terms.md)，本文不自建术语。
> **与 [agent 层](../../agent/README.md)的边界**：**授权的存与查将上移到框架**（它是[跨执行的状态](../../terms.md)，agent 那边现在缺这张表）；**本文的 §拆分器与键算法留在本层**——框架不该懂 bash 语法，键由本层算完当不透明字符串交给框架。`behavior: 'allow-session'` 迁移后重述为 `outcome: 'allow' + scope: 'broader'`。见 [agent/agent-kernel/plan](../../agent/agent-kernel/plan.md#两个容易归错层的-app-文档2026-08-11-复核)。
