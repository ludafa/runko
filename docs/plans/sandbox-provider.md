...
- `pnpm --filter @nimbo-chat/server typecheck` 通过。
...
- 未生成 changeset：仅改 `@nimbo-chat/server`（private 应用），未触任何 public `@nimbo/*` 包。
...
- 主线程独立复核：`pnpm --filter @nimbo-chat/server typecheck` 通过；`vitest run`：**12 文件 / 242 测试全绿**（SP-3 基线 232 + 新增 10）；改动测试文件 eslint 0 error。
...
**修复**（`apps/server/src/agent/sandbox-manager.ts`）：`resume()` 对 `connect` 做有限退避重试（`E2B_RESUME_ATTEMPTS=4`、退避 `500ms×attempt`，总窗 ~3s）——瞬时 404 重试即重连回原盒（保住未 push 的 WIP），只有**持续** not-found 才 `unavailable`→重建；`isE2bSandboxGone` 加 message 兜底（`/sandbox.*not found/i`），"Invalid sandbox ID"(400) 不算 gone。
