...
> 相关：产品/使用视角见 [../features/turn-checkpoint.md](../features/turn-checkpoint.md)，技术方案见 [../tech/turn-checkpoint.md](../tech/turn-checkpoint.md)。
> 依赖/延续：[chat 聊天 webapp](../features/chat-webapp.md)（§2.2d transcript 减量 P13-1 已交付）· [UIMessage 单账本](../features/single-ledger.md)（P13-5，checkpoint `lastSeq` 对齐[账本](../terms.md) `seq`）· [tech/sandbox](../tech/sandbox.md)（BYO 原则与配方章节，施工时回填保活配方）· [core-sdk 施工计划](../plans/core-sdk.md) P13。
...
3. [tech/sandbox](../tech/sandbox.md) 配方章节回填「turn 对齐 checkpoint」（回填时与用户确认）。
