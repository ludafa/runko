...
> 产品文档 [docs/features/push-notification.md](../features/push-notification.md)，技术方案 [docs/tech/push-notification.md](../tech/push-notification.md)。
...
- `docs/features/push-notification.md`、`docs/tech/push-notification.md`、本文件。
...
| 8 | PN-9 的论据：240 秒的理由已被 `startHeartbeat` 作废 | **论据换了**：`startHeartbeat` 已被删除，现在挡在中间的是[审批保活预算](../terms.md)（默认 5 分钟） | 施工期间隔壁并行落地了[沙盒保活](./sandbox-keepalive.md)，手写心跳整个删掉。结论不变（4 分钟太短），但要动的旋钮从一个变成两个。详见[技术方案 §8](../tech/push-notification.md) |
