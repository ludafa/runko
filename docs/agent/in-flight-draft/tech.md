...
> 相关文档：[账本设计](../single-ledger/tech.md)（本文要改的就是它 §2.4／§6 描述的做法）· [chat 服务端](../../app/chat-webapp/tech.md) §2.2b（一轮怎么跑、断线怎么重连）· [产品视角](../../app/chat-webapp/feature.md)
...
| 8 | 测试：重放幂等、崩溃后行为、干活中刷新画面完整、编号只由消息消耗 | 服务端 + 前端 |
| 9 | 回头修订[账本设计](../single-ledger/tech.md) §2.4／§6，产品文档补一句行为变化 | 文档 |

**顺序**：1 → 2/3/4/5/6 → 7 → 8；第 9 项收尾（代码落地后再改账本设计，免得文档跟实现对不上）。
...
**所以多机的正解大概率是按会话做固定路由**（同一个会话的所有请求——发消息、SSE、审批、停止——永远打到同一个实例），而不是把五样东西逐个外置。代价是实例挂掉那一轮丢失，但这跟今天单进程重启的结果**完全一样**，没变差。
