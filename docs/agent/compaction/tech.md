...
> 相关：[产品视角](./feature.md) · [施工进展](./plan.md) · 依赖 [single-ledger](../single-ledger/tech.md)（本方案的地基，P14 依赖 P13-5 落地）
> 参考：[core/core-sdk/tech](../../core/core-sdk/tech.md) §4.8（上下文管理 v1 显式上限）· [host/sandbox/tech](../../host/sandbox/tech.md)（账本流水与模型上下文的两层结构、责任划分）
...
> 推导正确性由离线断言覆盖（见 [施工进展](./plan.md) 验收项 1–4：过滤、链式、转换合法性、并发防重），不依赖真机。
...
账本在 server（`conversation_events`），压缩本质是「宿主如何组装 resume 输入」的存储层策略——与沙盒重建同类，归宿主（[host/sandbox/tech](../../host/sandbox/tech.md) 责任划分的既有立场）。core 的 loop / session 不感知压缩：它拿到的就是一份合法的 `UIMessage[]`。
