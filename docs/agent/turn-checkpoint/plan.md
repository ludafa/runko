...
> 依赖/延续：[chat 聊天 webapp](../../app/chat-webapp/feature.md)（§2.2d transcript 减量 P13-1 已交付）· [UIMessage 单账本](../single-ledger/feature.md)（P13-5，checkpoint `lastSeq` 对齐[账本](../../terms.md) `seq`）· [host/sandbox/tech](../../host/sandbox/tech.md)（BYO 原则与配方章节，施工时回填保活配方）· [core-sdk 施工计划](../../core/core-sdk/plan.md) P13。
...
- 前置：P13-1（transcript 减量）已交付；P13-5（[UIMessage 单账本](../single-ledger/feature.md)）落地后 `lastSeq` 对齐简化为直接取账本 `seq`。
...
3. [host/sandbox/tech](../../host/sandbox/tech.md) 配方章节回填「turn 对齐 checkpoint」（回填时与用户确认）。
...
- **P13-5（[UIMessage 单账本](../single-ledger/feature.md)）落地后**：事件表与「模型记忆」合一为账本，`nimbo_state_json` 取消，[模型上下文](../../terms.md)从账本 UIMessage 现场推导。checkpoint 的 `lastSeq` 对齐从「跨事件表 seq / `nimbo_state_json` 轮号两本账互校」简化为直接取账本 `seq`；核心不变量随之更简（模型上下文与代码快照仍在同一收尾边界写入）。
