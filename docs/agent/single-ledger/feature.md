...
> **相关**：技术方案见 [agent/single-ledger/tech](./tech.md)；施工进展见 [agent/single-ledger/plan](./plan.md)。
> **依赖**：本功能改造 [chat-webapp](../../app/chat-webapp/feature.md) 的持久化层（把它原来的双份落盘换成单账本）。下游的 [上下文压缩（compaction）](../compaction/feature.md) 在本功能的账本上再加第三种条目。

本文面向使用者与集成方，讲清楚「单一数据账本」给用户和开发者解决了什么问题、怎么用、边界在哪、怎样算成功。实现细节（数据结构、写入时序、图）在 [agent/single-ledger/tech](./tech.md)。
...
单一数据账本是「换 SDK 地基」的决定，因此先做真机验证实验（P13-5a）再立项迁移（P13-5）。判定标准是[五个验证项](./plan.md)全部有可复现的真机答案，且其中三项**硬性通过**、两项允许「有限制但可接受」：
...
> 判定结论：**GO**（三项硬性全过，两项弹性一项通过、一项带限制条款通过）。逐项真机记录见 [agent/single-ledger/plan](./plan.md) 的实验记录。
