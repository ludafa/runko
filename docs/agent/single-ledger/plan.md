...
> **相关**：产品手册见 [agent/single-ledger/feature](./feature.md)；技术方案见 [agent/single-ledger/tech](./tech.md)。
> **依赖**：本功能改造 [chat-webapp](../../app/chat-webapp/plan.md) 的持久化层。

## 历史 banner（原始状态记录）

> 状态（2026-07-14）：实验方案待 review。「唯一数据真相」被定为 P13 系列最高优先级，先于 checkpoint 施工。
>
> 相关（原始 docs/09·docs/core/core-sdk/tech.md §4.3·docs/08，迁移后指向）：[turn-checkpoint 计划](../turn-checkpoint/plan.md)（checkpoint 与保活——本实验若通过，其「对齐」语义将基于单账本简化）· [core-sdk 的手动 loop 选型](../../core/core-sdk/tech.md)（本实验不推翻它：loop 仍是 nimbo 自己的，换的是 loop 的**工作格式**）· [chat-webapp](../../app/chat-webapp/plan.md)（chat 应用）。
>
...
- **P13-5a-1 实验实现（coder）**：按 §2.1/§2.2/[部件清单](./tech.md) 写 `examples/13-uimessage-single-ledger.e2e.test.ts` + 必要的 shared 辅助（请求体取证的 fetch 包装）。离线部分（mock 模型走结构断言）进常规测试；真机部分沿 examples 现有 e2e 门控约定（缺凭证即跳过）。验收：typecheck/lint 过、离线断言绿、真机路径可一键执行并在结束时打印五项验证的逐项结果。
- **P13-5a-2 真机执行（用户）**：跑真机路径，把五项结果回填进 §3。
- **P13-5a-3 判定（主线程）**：按 §2.3 标准给 go/no-go；go → 立项 P13-5（core 迁移，粒度：core loop/session/state → chat server → web → docs 修订）；**no-go → 硬伤记录归档，回退到 [turn-checkpoint 计划](../turn-checkpoint/plan.md)讨论过的「同账本双条目」方案**。
...
### 3.1 实验发现（P13-5 立项时必须消化，技术细节见 [agent/single-ledger/tech](./tech.md) §7/§8）
...
**定案：审批判定三值化，且在阻塞之前显式产出。** 详细方案（三值表、`ApprovalPolicy`/`ApprovalOutcome`/`HumanDecision` 类型、两层组合、两值裁决、删 `updatedInput`）见 [agent/single-ledger/tech](./tech.md) §5。
...
- **crash mid-turn（无收尾）**：本轮无 message 条目、chunk 条目残留——`finalizeTurnPersistence` 未运行、header 未写。属可接受残留，不再清理（技术细节见 [agent/single-ledger/tech](./tech.md) §6）。
