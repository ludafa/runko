...
§6 共四条成功标准，每条至少一个用例；用例 ID 前缀即标准序号。四条标准即 core-sdk 的验收基准（见开头“相关”行的 [core-sdk](../../logic/engine/features/core-sdk.md) 链接）。
...
任何改动合入前，仓库根目录按序全绿（顺序固定，理由见 §1.2 注释与 docs/logic/engine/plans/core-sdk.md P4-2）：
...
CI（`.github/workflows/`）跑 1–3 与 5（P8-2 起，examples typecheck 入 CI——docs/logic/engine/plans/core-sdk.md P8-1 裁定）；4 为本地门槛（阈值已在 vitest 配置强制）。
