...
**注释和文档不是两个抄本，是两种职责。** 分工按「**什么时候必须跟着改**」划，不按「重不重要」：

| 放哪 | 判据 | 例子 |
| --- | --- | --- |
| **注释** | **改这几行代码时，这条注释必须跟着改** | 为什么这里是 `z.any()`（没有可复用的 schema，`z.ZodType<T>` 把 `any` 关住了）；为什么 `seq` 是可选的；某个坑的实测结论 |
| **文档** | 改代码不会让它失效，**只有改方案才会** | 方案取舍、否决过的备选、验收结论、历史演进 |
| **哪都不放** | 代码自己说得出口的 | 「这个字段是字符串」 |

按这条判据，**历史一律进文档，代码里一行不留**。读代码的人要知道「现在是什么」；想知道「为什么变成这样」的人会去翻文档，而且只有文档那份有人维护。

### 三条硬规矩

1. **注释里禁止写「以前是 X」。** 这类句子描述的东西已经不在代码库里，**没有任何手段能验证它是真是假**，只会一直躺着误导人。同理禁止写工单号（`P13-5-2c` 这种）与「本工单范围」——它们的读者早就不存在了。
2. **引用单向：注释可以引文档，文档别引代码行号。** 引文件名与符号名可以，引行号必烂。
3. **注释里的文档链接会被检查。** `pnpm check:doc-links`（已挂进 `pnpm lint` 与 CI）扫源码注释里的 `docs/**.md`，指向不存在的文件就报错，并给出正确路径。

> **为什么要有第 3 条**：2026-09-18 扫过一次——`docs/` 里的链接有 `pnpm docs:build` 做死链检查，**零失效**；源码注释里的文档路径没有任何检查，于是 docs 改成分层目录之后，**807 处引用、33 个路径，全部指向不存在的文件**，没人发现。差别不在纪律，在有没有守门员。

### 写法

- **一句话只说一件事。** 一句跨 5 行、套三层括号的注释，读的人得先解析语法再理解内容。
- **单个注释块别超过 25 行。** 超了就该分小节（`## 小标题`）或挪进文档。
- **中文。** 标识符、类型名、代码片段保持原样，那是代码不是文字。
- **块注释里别写含 `*` 加 `/` 的路径通配**——那会提前闭合注释，报错还会指向几十行之外的位置。
