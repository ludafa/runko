 ...
 * header for the shared background: docs/host/contract/tech/sandbox.md
 * §8, docs/logic/engine/plans/core-sdk.md P10). Cloudflare Sandbox is architecturally
 ...
 *
 * Two entry points, two different "no runtime import" stories (docs/host/contract/tech/sandbox.md §8.1/
 * §8.2):
 ...
 *     wrangler project — see `apps/cloudflare-worker-server/src/index.ts`):
 ...
 * setup; see apps/cloudflare-worker-server/README.md to deploy the real
 * gateway).
 *
 ...
 *      into docs/misc/plans/verification.md once a user deploys the gateway and supplies the two
