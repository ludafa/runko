 ...
 * (docs/logic/engine/tech/core-sdk.md §4.4 `OverlayFS`): reads pass through to disk, every
 ...
 *
 * Demonstrates: RunkoFS.fromDirectory, session.fs.diff(), session.fs.writeBack().
 *
 ...
 *      writeBack() is called.
 *   2. If RUNKO_MODEL is set: the agent edits a real file through the
 ...
 *      If RUNKO_MODEL is unset, this section is skipped with a clean exit.
 */
...
async function makeScratchProject(seedContent: string): Promise<string> {
  const dir = await nodeFs.mkdtemp(nodePath.join(nodeOs.tmpdir(), "runko-example-02-"));
...
async function deterministicSection(): Promise<void> {
  ...
  const fs = RunkoFS.fromDirectory(projectDir);
