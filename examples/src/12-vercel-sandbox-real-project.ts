 ...
 * (docs/host/contract/tech/sandbox.md §1/§2, docs/logic/engine/plans/core-sdk.md
 ...
 * (docs/host/contract/tech/sandbox.md §8.3).
 *
 * ---- File naming / test runner note ----
 *
 * The `.e2e.test.ts` suffix is the user-specified filename (docs/host/contract/tech/sandbox.md §8.6),
 ...
 * backfilled into docs/misc/plans/verification.md once a user supplies `GITHUB_REPO`/`GITHUB_PAT`.
 *
 ...
 * chain: docs/host/contract/tech/sandbox.md §8.4 pins DeepSeek specifically (a design-quality task
 ...
 * `deepseek-v4-flash` and `deepseek-v4-pro`; the latter is docs/host/contract/tech/sandbox.md §8.4's
 ...
 *
 * ---- The Git workflow (docs/host/contract/tech/sandbox.md §1/§2, §8.1/§8.3) ----
 *
 ...
 *
 * GitHub auth is fine-grained-PAT v1 (docs/host/contract/tech/sandbox.md §8.1): exactly Contents R+W +
...
/**
 * Accepts both forms `GITHUB_REPO` might hold (docs/host/contract/tech/sandbox.md §8.3): SSH
...
/**
 * Six host-side `runCommand` calls (docs/host/contract/tech/sandbox.md §2.3/§2.4), none of them routed
  ...
  if (model === undefined) {
    console.log(
      "[runko example] DeepSeek is not configured — skipping the real-project section.\n" +
        "This example is DeepSeek-only (docs/host/contract/tech/sandbox.md §8.4 pins a stronger tier for the design task), unlike other\n" +
