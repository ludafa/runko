...
const runtime = createAgentRuntime({ agent, prepareTurn, persistence: mysqlPersistence(db) });
```
...
两件事值得先知道：`mysqlPersistence()` 与 `mysqlArbitration()` 各建一个 Kysely 实例，但共用你给的那个连接池，
