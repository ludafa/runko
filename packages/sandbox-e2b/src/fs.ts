/**
 * RunkoFS 七方法 → e2b `Filesystem` 映射（docs/host/contract/tech/sandbox.md §3.1 E2B 列 / §8.2）。
 */
...
/** `EntryInfo.type` 非 `"file"` 一律视为目录（docs/host/contract/tech/sandbox.md §8.2："FileType 非 file 视为 dir"）；`size` 只在文件类型下有意义，仿 MemoryFS 用条件展开省略而非填 0/undefined。 */
...
export function createE2bFs(sandbox: E2bSandboxLike, anchor: PathAnchor): RunkoFS {
  return {
    ...
    async rm(path: string, opts?: { recursive?: boolean }): Promise<void> {
      ...
      // 的 DirectoryNotEmptyError 语义（docs/host/contract/tech/sandbox.md §8.2）。
    ...
    async glob(pattern: string): Promise<string[]> {
      ...
      // matcher，不依赖沙盒内是否装了 find（docs/host/contract/tech/sandbox.md §4 决策点 4）。
      let entries: E2bEntryInfo[];
      try {
