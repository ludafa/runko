 ...
 * （docs/logic/engine/tech/core-sdk.md §4.5a 模式 A；docs/host/contract/tech/sandbox.md §5.2/§8.2）。
 *
 * BYO 实例是唯一入口：本函数不创建、不销毁沙盒——沙盒的生命周期（创建、
 * 暂停/恢复、销毁）完全由宿主自己管理，`e2bWorkspace()` 只是给一个已经
 * 存在的沙盒包一层 RunkoFS/RunkoExec 的视图（docs/host/contract/tech/sandbox.md §5.2 "runko 不关心沙盒
 * 长什么样"）。
 *
 * **唯一的例外是[保活](../../../docs/terms.md)**（docs/logic/orchestration/features/sandbox-keepalive.md）：
