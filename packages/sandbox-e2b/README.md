...
把一个已创建好的 [E2B](https://e2b.dev)（真实 Firecracker microVM）沙盒包成 `RunkoFS & RunkoExec`，一次注入 `createSession(agent, { workspace })`——"模式 A 同源工作区"落在真实云沙盒上的实现（对照 [core-sdk · 技术方案 §4.5a](../../docs/logic/engine/tech/core-sdk.md) / [沙盒工作区（Sandbox Workspace） · 技术方案 §8](../../docs/host/contract/tech/sandbox.md)）。

## 安装

```sh
pnpm add @runko/sandbox-e2b e2b
```

`e2b` 需要单独装——本包运行时**不 import** `e2b`（见下方"结构化接口"），但宿主要创建真实沙盒实例（`Sandbox.create()`）离不开它。本包不随 `@runko/sdk` 一起装，属于按需显式安装的可选集成。
...
完整可跑示例（含零凭证的 fake 沙盒演示，证明"BYO 实例 + 结构化接口"不需要真实网络）见 [examples/09-sandbox-e2b.ts](../../examples/src/09-sandbox-e2b.ts)。
...
- 其余可调项（`maxTurnMs` 单轮上限、`approvalBudgetMs` 审批预算、`onRenew` 观测回调）见 `@runko/core` 的 `KeepAliveOptions` 与 [沙盒保活（keepalive） · 功能](../../docs/logic/orchestration/features/sandbox-keepalive.md)。
...
## 已知限制（docs/host/contract/tech/sandbox.md §8.2 / docs/logic/engine/plans/core-sdk.md P10-1 实际改动，如实照抄不发明）
...
- **每次 RunkoFS 文件工具调用都是一次网络往返**（几十到几百毫秒）——扫描类操作（大量 glob、grep-like 搜索）优先用一条 bash 命令，而不是逐个 `glob`/`read_file` 调用。
