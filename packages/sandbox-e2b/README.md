...
把一个已创建好的 [E2B](https://e2b.dev)（真实 Firecracker microVM）沙盒包成 `NimboFS & NimboExec`，一次注入 `createSession(agent, { workspace })`——"模式 A 同源工作区"落在真实云沙盒上的实现（对照 [docs/02-tech-spec.md §4.5a](../../docs/02-tech-spec.md) / [docs/06-sandbox-workspace-research.md §8](../../docs/06-sandbox-workspace-research.md)）。
...
## 已知限制（docs/06 §8.2 / docs/03-construction-plan.md P10-1 实际改动，如实照抄不发明）
