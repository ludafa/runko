/**
 * End-to-end wiring (tech-spec §4.5a "端到端两件套" 验收点，与
 ...
 *      `readState` 失效（§4.5a 模式 A 规则 2），后续 `edit_file` 被拒绝直到
 *      重新 `read_file`。
 */
...
describe("a) mock model drives bash through e2bWorkspace() against a fake sandbox", () => {
  it("bash runs the command via e2bWorkspace and the tool_call completes with the expected output", async () => {
    ...
    const events = await drainStream(session.stream("run a command in the sandbox"));
...
describe("b) bypass proof: a bash-side write invalidates readState (§4.5a mode A rule 2)", () => {
  ...
  it("read_file, then a bash-side write, then edit_file is rejected until the path is re-read — and re-reading unblocks it", async () => {
    const workspace = e2bWorkspace(createFakeE2bSandbox());
    ...
    expect(toolCalls[1]?.status).toBe("completed"); // bash bypass write, real e2bWorkspace exec, real exit 0
