 ...
 * 由被挑中的节点看自己内存里的状态决定（技术方案 docs/logic/orchestration/tech/handover.md §7）。
 *
 * 它是一样可选的宿主能力：不给它，交权时挑不到接手节点，对话一律标成[待接手](../../../docs/terms.md)。
 */
...
export interface NodeRegistry {
  ...
  /** 刷新心跳。只动心跳，不动状态——下线中的节点照样要续，好让别人知道它还活着。 */
  heartbeat(node: string): Promise<void>;
  /** 标成下线中。收到 SIGTERM 之后的**第一件事**。 */
  markLeaving(node: string): Promise<void>;
  /** 删掉自己那一行（进程退出前）。 */
  remove(node: string): Promise<void>;
  /**
   * 按技术方案 §7.2 的顺序排好的候选，**不含自己**，只要 `ready` 且心跳在 `freshMs` 之内的：
   *
