...
> **租约版归属仲裁不在这张单子上了**——2026-09-01 已交付，四档库现在都有。多进程 / 多副本
> 共享一个库时，用 `@runko/persist-kysely` 的 `leaseArbitration()`，或各个包的
> `sqliteArbitration()` / `postgresArbitration()` / `mysqlArbitration()` / `mongoArbitration()`，
> **轮编排一行不改**。怎么配见[多副本部署](../../docs/host/node/tech/multi-replica.md)。
