/**
 * `@runko/persist-postgres`——[持久化](../../../docs/host/contract/features/persistence.md)
 * 的 PostgreSQL（pg） 实现。
 *
 ...
 * createAgentRuntime({ agent, prepareTurn, persistence: postgresPersistence(db) });
 * ```
 *
...
export const RUNKO_PERSIST_POSTGRES_VERSION = "0.0.0" as const;
...
/** 驱动 → Kysely。本包唯一做的事。 */
function toKysely(pool: PgPool): Kysely<RunkoDatabase> {
  return new Kysely<RunkoDatabase>({ dialect: new PostgresDialect({ pool: pool }) });
}
...
/**
 * `postgresArbitration()` 的入参——就是 `@runko/persist-kysely` 的那份，**去掉 `flavor`**：
 * 本包只接一家，方言不该再让调用方填一遍。
 */
export type PostgresArbitrationOptions = Omit<LeaseArbitrationOptions, "flavor">;
 ...
 * 两件事值得先知道：
 *
 * 1. `postgresPersistence()` 与本函数会各建一个 Kysely 实例。Kysely 是查询构建器、
 ...
 * 是**真保证**，租约版只是**尽力 + 可检测**。
 */
export function postgresArbitration(pool: PgPool, opts: PostgresArbitrationOptions): Arbitration {
  return leaseArbitration(toKysely(pool), { ...opts, flavor: "postgres" });
}

/**
 * `postgresNodeRegistry()` 的入参——就是 `@runko/persist-kysely` 的那份，**去掉 `flavor`**：
 * 本包只接一家，方言不该再让调用方填一遍。
 */
export type PostgresNodeRegistryOptions = Omit<NodeRegistryOptions, "flavor">;
 ...
 */
export function postgresNodeRegistry(pool: PgPool, opts: PostgresNodeRegistryOptions = {}): NodeRegistry {
  return nodeRegistry(toKysely(pool), { ...opts, flavor: "postgres" });
}
