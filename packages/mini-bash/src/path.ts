/**
 * 自包含的极简 POSIX 虚拟路径解析。刻意不依赖 @runko/virtual-fs 的
 * `normalizePath`（语义几乎相同）——src 侧按工单并行边界只允许依赖
 * @runko/core，且 mini-bash 的路径解析只是"把相对参数变成绝对路径字符串
 * 交给注入的 RunkoFS"，不承担安全边界（docs/logic/engine/tech/core-sdk.md §4.4："安全边界在 FS
