...
import type { ExecOptions, ExecOutputChunk, ExecResult, FileStat, NimboFS } from "@nimbo/core";
...
export function run(fs: NimboFS, command: string, opts: RunOpts = {}): Promise<ExecResult> {
...
/**
 * 一个 `stat()` 立即成功、`readFile()` 永不 resolve 的假 NimboFS——专门用来
 ...
 */
export function makeHangingFs(): NimboFS {
 ...
 * 用于模拟"stat() 说是可读文件，但 readFile() 仍然失败"这类第三方 NimboFS
 * 实现可能出现的不一致（例如某些 reference 变体），验证 readFileForCommand
 * 的兜底 catch 分支。
 */
export function withFailingReadFile(fs: NimboFS, path: string, error: Error): NimboFS {
  return {
...
export function withFailingReaddir(fs: NimboFS, path: string, error: Error): NimboFS {
  return {
