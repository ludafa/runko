 ...
 * 兼容子集）上的直译（docs/host/contract/tech/sandbox.md §3.1 / §8.2 Vercel 列）。
 *
 * ---- 一个实测推翻工单研究原文的发现：`fs.rm()` 无法承担"非递归删非空目录
 * 拒绝"的语义 ----
 *
 * docs/host/contract/tech/sandbox.md §8.2 原文说 Vercel 的 `fs.rm(p, {recursive})` "原生对齐"——本地实测
 ...
 * 目录多一次 `stat` 判断类型，可接受（docs/host/contract/tech/sandbox.md §4 第 6 点本就预期扫描类操作
 ...
 *
 * ---- 原生搜索快路径（docs/host/contract/tech/sandbox.md §4）：`searchFiles`/`searchContent` + `glob` 重写 ----
 *
