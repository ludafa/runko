...
把一个已创建好的 [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox)（真实 Amazon Linux 2023 Firecracker microVM）实例包成 `RunkoFS & RunkoExec`，一次注入 `createSession(agent, { workspace })`——"模式 A 同源工作区"落在真实云沙盒上的实现（对照 [core-sdk · 技术方案 §4.5a](../../docs/logic/engine/tech/core-sdk.md) / [沙盒工作区（Sandbox Workspace） · 技术方案 §8](../../docs/host/contract/tech/sandbox.md)）。

## 安装

```sh
pnpm add @runko/sandbox-vercel @vercel/sandbox
```

`@vercel/sandbox` 需要单独装——本包运行时**不 import** `@vercel/sandbox`（见下方"结构化接口"，甚至连 `import type` 都不出现），但宿主要创建真实沙盒实例（`Sandbox.create()`）离不开它。本包不随 `@runko/sdk` 一起装，属于按需显式安装的可选集成。
...
```ts
function vercelWorkspace(sandbox: VercelSandboxLike, opts?: { root?: string }): RunkoFS & RunkoExec;
```
...
## 已知限制（docs/host/contract/tech/sandbox.md §8.2 / docs/logic/engine/plans/core-sdk.md P10-2 实际改动，如实照抄不发明）
...
- **每次 RunkoFS 文件工具调用都是一次网络往返**——扫描类操作（大量 glob、grep-like 搜索）优先用一条 bash 命令，而不是逐个 `glob`/`read_file` 调用。
