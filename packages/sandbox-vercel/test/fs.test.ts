/**
 * RunkoFS 七方法契约（docs/host/contract/tech/sandbox.md §3.1 / §8.2 Vercel 列）：ENOENT → NotFoundError，
...
/**
 * `glob()` is now a single `node -e`-script round-trip (docs/host/contract/tech/sandbox.md §4 native search fast
...
describe("root anchoring", () => {
  ...
  it("rejects '..' escaping past the virtual root for the fs tools (PathEscapesRootError), same boundary as MemoryFS/DirFS — src/path.ts's documented interpretation of docs/host/contract/tech/sandbox.md §3.1", async () => {
    const sandbox = new FakeVercelSandbox({ files: { "/a.txt": "hi" } });
    const ws = vercelWorkspace(sandbox);
