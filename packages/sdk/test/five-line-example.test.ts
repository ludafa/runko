/**
 * P7-1 acceptance: docs/logic/engine/features/core-sdk.md §4.1's five-line quickstart, ported to this
 * package with the two deviations the ticket allows — the import source (`@runko/sdk` via
 ...
 *
 *   import { defineAgent, createSession } from "runko";
...
import { defineAgent, createSession, RunkoFS } from "../src/index.js";
import { mockModel, stopChunk, toolCallChunk } from "./helpers.js";

describe("docs/logic/engine/features/core-sdk.md §4.1 five-line quickstart (import source + model are the only deviations)", () => {
  let projectDir: string;

  beforeEach(async () => {
    projectDir = await nodeFs.mkdtemp(nodePath.join(os.tmpdir(), "runko-sdk-quickstart-"));
  ...
  it("runs the quickstart end to end: agent reads, edits, and the diff is visible on session.fs", async () => {
    ...
    const session = createSession(agent, { fs: RunkoFS.fromDirectory(projectDir) });
    const result = await session.send("把 src/index.ts 里的 var 全部改成 const");

    expect(result.finalResponse).toBe("已将 src/index.ts 里的 var 改成 const。");
    // `session.fs.diff()` must compile without a cast — RunkoFS.fromDirectory(...) returns an
