...
**bash 分档**：`@nimbo/mini-bash` 随本包 re-export，开箱可用，定位安全默认/测试演示；`@nimbo/just-bash`（vercel-labs/just-bash 适配器）因依赖树含 wasm 大件（sql.js、quickjs-emscripten 等）**不进本包依赖**，强制打包违背门面轻量默认——需要全语法档的宿主显式安装该包后按上表用法注入（见 [core-sdk · 技术方案 §4.5b](../../docs/logic/engine/tech/core-sdk.md)）。
