...
把一个 [Cloudflare Sandbox](https://developers.cloudflare.com/sandbox/)（真实 Linux 容器）通过一个自部署的 HTTP 网关包成 `RunkoFS & RunkoExec`，一次注入 `createSession(agent, { workspace })`——"模式 A 同源工作区"落在真实云沙盒上的实现（对照 [core-sdk · 技术方案 §4.5a](../../docs/logic/engine/tech/core-sdk.md) / [沙盒工作区（Sandbox Workspace） · 技术方案 §8](../../docs/host/contract/tech/sandbox.md)）。
...
```sh
pnpm add @runko/sandbox-cloudflare
```

客户端侧零 provider SDK——本包不随 `@runko/sdk` 一起装，属于按需显式安装的可选集成。网关侧 `@cloudflare/sandbox` 是本包的 `peerDependency`（版本跟宿主的 wrangler 项目走），装在部署网关的那个 wrangler 项目里，不装在跑 agent 的 Node 进程里。

## 网关部署

先把网关部署到你自己的 Cloudflare 账号（需要 Workers Paid 计划，无免费层）。一个完整可跑可部署的参考项目见 [apps/cloudflare-worker-server/](../../apps/cloudflare-worker-server/README.md)——自备 CF 账号后 `pnpm --filter @runko-chat/cloudflare-worker-server deploy` 即可部署；本包自身只发布 `createSandboxGateway` 这层纯函数，网关的部署形态维护在该项目里。核心装配只有几行：
...
export default {
  async fetch(request: Request, env: { Sandbox: DurableObjectNamespace; RUNKO_GATEWAY_TOKEN: string }) {
    const gateway = createSandboxGateway({
      token: env.RUNKO_GATEWAY_TOKEN,
...
const workspace = cloudflareWorkspace({
  url: process.env.RUNKO_CF_GATEWAY_URL!,     // 部署好的网关 URL
  token: process.env.RUNKO_CF_GATEWAY_TOKEN!, // 与 `wrangler secret put RUNKO_GATEWAY_TOKEN` 相同的值
});
...
```ts
// 客户端（"."）
function cloudflareWorkspace(opts: { url: string; token: string; sandboxId?: string; fetch?: typeof fetch }): RunkoFS & RunkoExec;
...
- 全部 `POST` + JSON body，`Authorization: Bearer <token>` 鉴权，`x-runko-sandbox` 头选择沙盒（缺省 `"default"`），二进制经 base64，`/exec` 走 NDJSON 流式响应——完整协议见 [沙盒工作区（Sandbox Workspace） · 技术方案 §8.3](../../docs/host/contract/tech/sandbox.md)。

## 已知限制（docs/host/contract/tech/sandbox.md §8.2–§8.3 / docs/logic/engine/plans/core-sdk.md P10-3 实际改动，如实照抄不发明）
...
- **每次 RunkoFS/RunkoExec 调用都是一次 HTTP 往返**（经 Durable Object，几十到几百毫秒）——扫描类操作（globbing 大目录树、grep-like 搜索）优先用一条 bash 命令（`find`/`grep`），而不是逐个文件工具调用。
