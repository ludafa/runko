...
把一个 [Cloudflare Sandbox](https://developers.cloudflare.com/sandbox/)（真实 Linux 容器）通过一个自部署的 HTTP 网关包成 `NimboFS & NimboExec`，一次注入 `createSession(agent, { workspace })`——"模式 A 同源工作区"落在真实云沙盒上的实现（对照 [core-sdk · 技术方案 §4.5a](../../docs/logic/engine/tech/core-sdk.md) / [沙盒工作区（Sandbox Workspace） · 技术方案 §8](../../docs/host/contract/tech/sandbox.md)）。
...
- 全部 `POST` + JSON body，`Authorization: Bearer <token>` 鉴权，`x-nimbo-sandbox` 头选择沙盒（缺省 `"default"`），二进制经 base64，`/exec` 走 NDJSON 流式响应——完整协议见 [沙盒工作区（Sandbox Workspace） · 技术方案 §8.3](../../docs/host/contract/tech/sandbox.md)。
