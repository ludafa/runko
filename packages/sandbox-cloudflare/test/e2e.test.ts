/**
 * 端到端接线（docs/06 §8.4 第 1 点"与 core 文件工具/bash 工具的 e2e"）：mock model
 ...
 *   a) mock model 用 `bash` 工具跑一条 echo 命令，tool_call 完整回填预期输出；
 ...
 *      使该路径的 `readState` 失效（§4.5a 模式 A 规则 2），之后 `edit_file` 被拒
 *      直到重新 `read_file`。
 */
...
describe("a) mock model drives bash through the full client → gateway → FakeCfSandbox chain", () => {
  it("bash runs an echo command and the tool_call completes with the expected output", async () => {
    ...
    const events = await drainStream(session.stream("run echo through the cloudflare sandbox gateway"));
...
describe("b) bypass proof: bash's redirect write invalidates readState (§4.5a mode A rule 2)", () => {
  ...
  it("read_file, then a bash redirect write, then edit_file is rejected until the path is re-read — and re-reading unblocks it", async () => {
    ...
    expect(toolCalls[1]?.status).toBe("completed"); // bash redirect write, real exec through the gateway, real exit 0
