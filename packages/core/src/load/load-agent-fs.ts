...
/**
 * L3 目录约定层，虚拟 FS 版本（docs/logic/engine/tech/core-sdk.md §4.7）。只读 `instructions.md` 与
  ...
  if (model === undefined) {
    throw new Error(
      `loadAgentFromFS(fs, "${dir}"): no model configured — this loader never evaluates agent.ts/agent.json ` +
        "(docs/logic/engine/tech/core-sdk.md §4.7: no arbitrary code execution surface over a virtual FS), so pass " +
