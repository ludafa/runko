/**
 * `bash`（04-builtin-tools.md §1.10 / tech-spec §4.5a）：条件内置——只在
 * `createSession(agent, { exec })`（或 `{ workspace }` 语法糖）注入了
 * `NimboExec` 实现时才出现在工具列表（与 `load_skill` 同款条件内置机制，接线
 * 在 `session.ts` 的 `assembleTools`，控制条件各自独立）。
 *
 * ---- 实现契约对齐（tech-spec §4.5a"实现契约"，P6-1 施工回填） ----
 *
...
/** 04-builtin-tools.md §1.10 原文：`{ command, cwd?, timeout_ms? }`。 */
...
/** 04 §0.5 横切规则的一次性错误结构，索引签名理由同 `load-skill.ts` 的 `ToolErrorResult`（不从 virtual-fs 导入，core 不依赖它）。 */
...
const OUTPUT_BYTE_LIMIT = 64 * 1024; // 04 §1.10：stdout/stderr 各自 64KB 上限
...
/** stdout/stderr 合并文本 + 退出码（04 §1.10 输出规格）。 */
...
const BASE_DESCRIPTION =
  ...
  "workspace, files bash writes are visible to read_file immediately, but bash-made changes do not produce " +
 ...
 * "always"、沙盒实现通常 "never"），未点名"完全没声明"这个第三种情况该怎么
 * 办。按"审批是安全机制，未知实现的安全性不该被乐观假设"的原则选择保守默认
 * "always"（要求宿主接入审批链才能放行），而不是静默直通—— 一个没有声明
 * `defaultApproval` 的第三方 `NimboExec` 更可能是简单包装、未必经过安全考量。
 */
const CONSERVATIVE_DEFAULT_APPROVAL: ApprovalPolicy = "always";
