/**
 * `bash`（docs/logic/engine/tech/builtin-tools.md §1.10 / docs/logic/engine/tech/core-sdk.md §4.5a）：条件内置——只在
 ...
 *
 * ---- 实现契约对齐（docs/logic/engine/tech/core-sdk.md §4.5a"实现契约"，P6-1 施工回填） ----
 *
 ...
 * "completed"（docs/logic/engine/tech/builtin-tools.md §1.10 验收点："超时/非零退出码作为正常 tool 结果（非
 * isError 崩溃）回填模型"）。但 `RunkoExec` 是宿主可自行实现的接口
 * （§4.5a 模式 C"完全解耦"），第三方实现可能不遵守这个契约——因此仍对
 * `exec()` 的 reject 做兜底：转成一个 `{ isError: true, content }` 结构化
 * 结果（docs/logic/engine/tech/builtin-tools.md §0.5"错误即指导"），而不是让它以裸 throw 冒泡到
...
/** docs/logic/engine/tech/builtin-tools.md §1.10 原文：`{ command, cwd?, timeout_ms? }`。 */
...
/** docs/logic/engine/tech/builtin-tools.md §0.5 横切规则的一次性错误结构，索引签名理由同 `load-skill.ts` 的 `ToolErrorResult`（不从 virtual-fs 导入，core 不依赖它）。 */
...
const OUTPUT_BYTE_LIMIT = 64 * 1024; // docs/logic/engine/tech/builtin-tools.md §1.10：stdout/stderr 各自 64KB 上限
...
/** stdout/stderr 合并文本 + 退出码（docs/logic/engine/tech/builtin-tools.md §1.10 输出规格）。 */
 ...
 * "review"（旧 "always"）、沙盒实现通常 "allow"（旧 "never"），docs/logic/orchestration/tech/single-ledger.md §6.1
