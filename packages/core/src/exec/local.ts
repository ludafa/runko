/**
 * `localExec(opts?)`：`RunkoExec` 的本机实现（docs/logic/engine/tech/core-sdk.md §4.5a / docs/logic/engine/tech/builtin-tools.md
 * §1.10；P7-3 工单任务 1）。用 `node:child_process` 起一个真实 OS shell 子进程
 * 执行 `command`——与 `@runko/mini-bash` 的纯 TS 解释器互为对偶：mini-bash 不
 * fork 子进程、只读、`defaultApproval: "allow"`（docs/logic/orchestration/tech/single-ledger.md §6.1 三值重构后的
 * 映射，原 "never"）；`localExec` fork 真实进程、无只读限制、
 * `defaultApproval: "review"`（同一映射，原 "always"；出厂值，docs/logic/engine/tech/builtin-tools.md §1.10——
 * 本机执行没有天然隔离，审批是唯一的把关点）。
 *
 * ---- `exec()` 契约（docs/logic/engine/tech/core-sdk.md §4.5a"实现契约"，同 `tools/builtin/bash.ts` 头注释） ----
 *
...
const MATERIALIZE_REQUIRES_FS_MESSAGE =
  ...
  "so bash and the file tools stay consistent, docs/logic/engine/tech/core-sdk.md §4.5a mode B).";

/** ≤150 token 规格（docs/logic/engine/tech/builtin-tools.md §1.10）：OS/架构/node 版本 + 网络可达假设 + 模式 B/C 的 fs 映射说明 + 默认 cwd。 */
...
/** `RunkoExec` 的本机实现（docs/logic/engine/tech/core-sdk.md §4.5a / docs/logic/engine/tech/builtin-tools.md §1.10）。`defaultApproval: "review"` 出厂值（docs/logic/orchestration/tech/single-ledger.md §6.1 三值重构后的映射，原 "always"）——本机执行没有天然隔离。 */
