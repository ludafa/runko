 ...
 * （tech-spec §4.1/§4.2/§4.4/§4.5a/§4.8），P1-2 追加 L1 定义层
 * defineAgent/defineTool/defineSkill（tech-spec §4.1）。P4-1 追加审批链 +
 * ToolRuntime + update_plan；P4-2 追加 L2 运行层 `createSession`/`runTurn`
 * （tech-spec §4.2 全节），P4 收尾。P5 追加 skills：`Skill.fromDirectory/
 * fromFS/fromMarkdown` 三加载器（`skill.js`）、`<available_skills>` 注入 +
 * `load_skill` 工具 + 附属文件挂载 + 真实 `getSkill`（`skills/loader.js`、
 ...
 * `toJSON`/`SessionOptions.resume`（`session.js`）。
 */
