/**
 * Skill 三加载器（docs/logic/engine/tech/core-sdk.md §4.1 `Skill.fromDirectory`/`fromFS`/`fromMarkdown`；
 * frontmatter 规则同 §4.1 末段 + docs/logic/engine/tech/builtin-tools.md §1.9）。纯加载逻辑，不含
...
/** packaged skill：读 `<path>/SKILL.md` + 附属文件（docs/logic/engine/tech/core-sdk.md §4.1）；`name` 取目录 basename。 */
...
async function collectFSFiles(fs: RunkoFS, rootPath: string, exclude: string): Promise<Record<string, Uint8Array>> {
  ...
  async function walk(dirPath: string, relDir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      // "reference" 条目跳过：没有可读取的本地字节内容（docs/logic/engine/tech/core-sdk.md §4.4），skill
...
/** packaged skill，同语义跑在 `RunkoFS` 上（docs/logic/engine/tech/core-sdk.md §4.1）。 */
