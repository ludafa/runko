/**
 * L1 定义层：Skill 数据类型、`defineSkill`、三个加载器（docs/logic/engine/tech/core-sdk.md §4.1）。
 ...
 * `RunkoFS` 的先例（P2-1 `fromMemory`/`fromDirectory` 改成独立导出函数）是
 * 不同的情形：那里 `RunkoFS` 类型在 `@runko/core`、构造函数要放在
 * `@runko/virtual-fs`，是**跨包**的 interface+namespace 合并，TS 不支持跨模块
...
/**
 * 三个加载器（docs/logic/engine/tech/core-sdk.md §4.1，规则见 `./skills/loader.js` 头注释）：
 * - `fromDirectory(path)`：packaged skill，真实磁盘上的 `<path>/SKILL.md` + 附属文件；
 * - `fromFS(fs, path)`：同语义，跑在任意 `RunkoFS` 上；
...
export const Skill: {
  ...
  fromFS(fs: RunkoFS, path: string): Promise<Skill>;
