/**
 * L2 运行层：审批链求值（docs/logic/orchestration/tech/single-ledger.md §6，P13-5-2c 三值
 * 重构；两层组合语义沿自 docs/logic/engine/tech/core-sdk.md §4.5"审批链（求值顺序）"，逐字保留，只是
 ...
 *
 * ---- once 记忆的标记时机（P13-5-2c 新语义，docs/logic/orchestration/tech/single-ledger.md §6.1 "review-once：…
 ...
 * 拒绝理由：无仲裁者指导文案，或策略/分类器 `deny` 的默认文案，docs/logic/orchestration/tech/single-ledger.md §6.4
 ...
 * 注入 `onReview`（人审通道）时——docs/logic/orchestration/tech/single-ledger.md §6.4"未注入时：review 无人可裁 →
