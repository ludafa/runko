 ...
 * (docs/logic/engine/tech/builtin-tools.md §4 bash 验收项 / docs/logic/engine/tech/core-sdk.md §4.5a). `@runko/mini-bash`
 ...
 * formalize.
 *
 * docs/logic/engine/tech/builtin-tools.md §4 bash 验收项逐条对应：
 ...
 *
 * P13-5-2（docs/logic/orchestration/tech/single-ledger.md）迁移：断言从 `SessionEvent`/
...
describe("bash builtin + exec/workspace wiring (docs/logic/engine/tech/builtin-tools.md §4 / docs/logic/engine/tech/core-sdk.md §4.5a)", () => {
  ...
  describe("approval default comes from exec.defaultApproval (§1.10; docs/logic/orchestration/tech/single-ledger.md §6.1 三值重构 always→review / never→allow)", () => {
  ...
  describe("timeout/non-zero exit backfill as a normal settled tool part (docs/logic/engine/tech/builtin-tools.md §4, not output-error)", () => {
...
describe("mode A: same-source workspace, fs + exec: miniBash(fs) (docs/logic/engine/tech/core-sdk.md §4.5a)", () => {
  ...
  describe("readState invalidation on bash-bypass writes (docs/logic/engine/tech/builtin-tools.md §0.4 / §4.5a 模式 A 衍生规则 2)", () => {
    /**
    ...
    it("a bash-made write invalidates readState — a subsequent edit-file on that path is rejected until re-read", async () => {
      ...
      // (docs/logic/engine/tech/builtin-tools.md §0.5) — state stays "output-available", the rejection shows up in output (same
