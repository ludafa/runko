...
describe("evaluateApproval", () => {
  describe("per-tool 'allow' / unconfigured — default allow (§4.5, docs/logic/orchestration/tech/single-ledger.md §6.1)", () => {
  ...
  describe("per-tool callback — decides directly from its ApprovalOutcome, never escalates to the session classifier", () => {
    ...
    it("denies via a per-tool callback returning 'deny' — capability-narrowed to the default message (docs/logic/orchestration/tech/single-ledger.md §6.4: a custom deny text is no longer expressible here, only via review + HumanDecision.deny.message)", async () => {
  ...
  describe("'review' — every call escalates to the session classifier (docs/logic/orchestration/tech/single-ledger.md §6.1)", () => {
  ...
  describe("'review-once' — first call escalates, an approved call is remembered by tool name (docs/logic/orchestration/tech/single-ledger.md §6.1 review-once 语义)", () => {
  ...
  describe("spec gap: 'review'/'review-once' with no session classifier configured — deny with guidance (docs/logic/orchestration/tech/single-ledger.md §6.4 无仲裁者)", () => {
