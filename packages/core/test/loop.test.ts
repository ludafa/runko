/**
 * P13-5-2（docs/logic/orchestration/tech/single-ledger.md）迁移：`runTurn` 的工作态/产出
...
describe("runTurn", () => {
  it("multi-step tool-use: step 1 tool-calls (update-plan) executed & settles in place, step 2 stops with the final text", async () => {
    ...
    // docs/logic/orchestration/tech/single-ledger.md §4.1 实现教训：同一 toolCallId 在账本里只记结算态，不会残留
  ...
  it("data-plan-update 同 id 覆盖 vs data-file-change 逐条追加 (docs/logic/orchestration/tech/single-ledger.md §2.2b / 关键语义 3)", async () => {
    const store = createPlanStore();
    const derivedData = createDerivedDataCollector();
  ...
  /**
   * P13-5-2c 返工（docs/logic/orchestration/tech/single-ledger.md §6.1/§6.4）：审批三值化后 "denied tool call" 不再是
   * 单一场景——`deny` 结果（策略/分类器直接判定）与 `review` 结果（先产出
   * 请求 chunk、再等真人裁决）走完全不同的 chunk 序列，因此分成下面几个独立
   * 用例，而不是原来那一个混在一起的 "denied tool call" 测试。
   */
  describe("deny outcome (evaluateApproval resolves 'deny' directly — not 'review'): straight to output-denied, no approval-request/response chunks at all (docs/logic/orchestration/tech/single-ledger.md §6.1)", () => {
    it("per-tool 'deny' policy: no execute(), reason backfilled via both tool-approval-response.reason and the tool-result error-text (docs/logic/orchestration/tech/single-ledger.md §5 关键语义 10)", async () => {
  ...
  describe("review resolved but no human reviewer is wired up (SessionOptions.onReview not configured): no-arbiter deny, and no tool-approval-request chunk is ever produced (docs/logic/orchestration/tech/single-ledger.md §6.4)", () => {
  ...
  describe("review resolved AND a human reviewer (onReview) is wired up: approval-request chunk first, then await onReview, then approval-response (docs/logic/orchestration/tech/single-ledger.md §6.1/§6.4 P13-5-2c '先产出后阻塞')", () => {
    ...
    it("onReview denies with a message: tool-approval-request → tool-approval-response(approved:false, reason) → output-denied, no execute(); reason backfilled to the model (docs/logic/orchestration/tech/single-ledger.md §5 关键语义 10)", async () => {
      const execute = vi.fn(() => "should not run");
      const tool: Tool = { description: "d", inputSchema: z.object({}), approval: "review", execute };
    ...
    it("onReview denies without a message: falls back to the default deny text (docs/logic/orchestration/tech/single-ledger.md §6.3 message is optional)", async () => {
  ...
  describe("review-once via onReview (docs/logic/orchestration/tech/single-ledger.md §6.1 review-once + P13-5-2c once-memory 标记时机): the human's decision — not the classifier's — controls whether once-memory gets marked", () => {
  ...
  it("'review-once' approval memory persists across steps within the same turn — a session classifier that itself resolves synchronously to 'allow' asks only once, and never touches onReview at all (docs/logic/orchestration/tech/single-ledger.md §6.1 once-memory timing)", async () => {
    const execute = vi.fn(() => "ok");
    const tool: Tool = { description: "d", inputSchema: z.object({}), approval: "review-once", execute };
  ...
  /**
   * 两条失败路径（docs/logic/orchestration/tech/single-ledger.md §5 关键语义 8）——实测确认（见工单回报"发现的 src
  ...
  it("abort during a tool execution: the turn stops at the next step boundary without another model call (docs/logic/orchestration/tech/turn-abort.md §2)", async () => {
    const controller = new AbortController();
  ...
  // 宿主给的中止理由要透传进收尾 message（docs/logic/orchestration/tech/graceful-shutdown.md §2）——
  ...
  it("ctx.update() progress ticks replay as accumulating transient data-tool-progress chunks — they reach the stream but never land in the ledger", async () => {
    ...
    // docs/logic/orchestration/tech/single-ledger.md §4.1 发现 A：transient 只出流，绝不进 UIMessage.parts/序列化存档——
