...
// Chat agent (docs/ingress/tech/chat-webapp.md §2.2, docs/logic/orchestration/tech/single-ledger.md §5 单-3 "UIMessage 单账本"): one row per
...
export const conversations = sqliteTable('conversations', {
  ...
  /** 沙盒 provider（docs/host/contract/tech/sandbox-provider.md）：这次会话跑在哪家云沙盒上，建会话时选定、1:1 绑定、运行中不切换。存量行迁移回填 'vercel'。 */
  ...
  /**
   * 待发队列（[排队](../../../../docs/terms.md)，docs/logic/orchestration/tech/steer-and-queue.md §2）：
  ...
  /**
   * [skill 清单](../../../../docs/terms.md)缓存（docs/ingress/tech/composer-skill-mention.md
...
// docs/logic/orchestration/tech/single-ledger.md §5 单-3: the single ledger for a
...
// 会话级授权（session grant，docs/ingress/tech/chat-webapp.md §6 / docs/terms.md §四）持久化：
...
// [推送订阅](docs/terms.md)（docs/ingress/tech/push-notification.md §2）：一台设备的一个
...
//   见 docs/ingress/tech/push-notification.md §3.1）天然是 upsert，不需要先查再插。
// - **`user_id` 可以被覆盖**。同一台设备换个账号登录，浏览器给的还是同一个
//   endpoint——upsert 时直接把归属改成新的人。这是对的：通知该跟着「这台设备
//   现在是谁在用」走，而不是跟着第一个用它登录过的人。
// - **不存偏好**。这一期没有按事件类型的用户开关（docs/ingress/features/push-notification.md
 ...
 * 补上结局。
 *
 * **两个用途**：留底（谁在什么时候批了什么），以及[挂起](../../../../docs/terms.md)之后的恢复——
 * 这一轮等满内存窗口就落盘放手，人隔几小时回来答，答案写进这一行，框架再从这里读回来接着跑。
 * 内存窗口之内，人的答复仍走框架内存里那条 promise 路由，不读这张表。
 *
 * `decided_at` 为空 = 还在等人：会话列表的「在等你」就是数这个（`countPendingDecisions`）。
 *
