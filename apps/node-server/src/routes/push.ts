/**
 * 推送订阅的四个接口（docs/ingress/tech/push-notification.md §3.1）——全部要登录，与
...
export function createPushApp(deps: PushRouteDeps): OpenAPIHono<PushEnv> {
  ...
  const configRoute = createRoute({
    ...
    tags: ['Push'],
    summary:
      '推送是否可用 + VAPID 公钥（docs/ingress/tech/push-notification.md §3.1）：前端据此决定渲不渲染铃铛',
    responses: {
      200: {
  ...
  const testRoute = createRoute({
    ...
    tags: ['Push'],
    summary:
      '给自己的全部设备发一条测试通知——端到端验证推送通道是否打通（docs/ingress/plans/push-notification.md PN-10 用例 4）',
    responses: {
      200: {
        content: { 'application/json': { schema: PushAckSchema } },
        description:
  ...
  app.openapi(testRoute, async (c) => {
    ...
    const payload: PushPayload = {
      ...
      // 收到响应」这条链路通不通（docs/ingress/tech/push-notification.md §6.5）。
      //
