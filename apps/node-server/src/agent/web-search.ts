/**
 * `web-search`（docs/logic/engine/tech/web-search.md）：chat 应用注册给 agent 的[联网搜索]
 * 工具，后端是 Exa 的 `POST /search`。
 *
 * 为什么在 apps 而不在 `@runko/core` 的内置工具里（docs/logic/engine/tech/web-search.md §1）：
...
/** 模型没指定条数时取几条——够用又不撑爆上下文（docs/logic/engine/features/web-search.md §6 成功标准 4）。 */
...
/** Exa 的内容类别过滤（docs/logic/engine/tech/web-search.md §3）。 */
...
/**
 * 请求体（docs/logic/engine/tech/web-search.md §2）。三个已知的过时写法刻意不用（Exa 官方
...
/**
 * `EXA_API_KEY` 的懒解析（docs/logic/engine/tech/web-search.md §5）：在 `buildSession()` 里
...
export function createWebSearchTool(opts: CreateWebSearchToolOptions): Tool {
  return defineTool({
    ...
    // `git push`/`rm -rf` 那类外发动作不是一个量级（docs/logic/engine/tech/web-search.md §3）。
...
/**
 * 非 2xx 的分档文案（docs/logic/engine/tech/web-search.md §6）——全部收敛成 `Error`，因为
