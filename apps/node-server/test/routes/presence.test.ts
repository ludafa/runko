...
import { isPresent } from '../../src/push/presence.js';
...
describe('routes/chat —— 在场心跳', () => {
  ...
  it('focused=true 记在场，focused=false 立即销掉', async () => {
    expect(await isPresent(db, 'user-1', CONVERSATION_ID)).toBe(false);

    const on = await post('user-1', CONVERSATION_ID, true);
    expect(on.status).toBe(200);
    expect(await on.json()).toEqual({ ok: true });
    expect(await isPresent(db, 'user-1', CONVERSATION_ID)).toBe(true);
  ...
  it('在场记在上报者名下，不影响别人', async () => {
    ...
    expect(await isPresent(db, 'user-2', CONVERSATION_ID)).toBe(false);
  });
