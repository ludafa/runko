...
describe("createLogDir", () => {
  it("连续跑两次：各建一个目录，`lab-latest` 指向后一个（第二次曾因删不掉旧软链接而失败）", () => {
    ...
    try {
      ...
      expect(readlinkSync(join(root, "lab-latest"))).toBe("lab-2026-09-13T10-05-00");
    } finally {
...
describe("mergeTimeline", () => {
  it("几份日志按时间交错合并；同一毫秒保持来源顺序", () => {
    const a = ["2026-09-13T06:30:52.100Z  replica-a  INFO   lease  lease acquired", "2026-09-13T06:30:52.300Z  replica-a  INFO   http  POST → 202"].join("\n");
    const b = ["2026-09-13T06:30:52.200Z  replica-b  INFO   forward  forwarding to holder", "2026-09-13T06:30:52.300Z  replica-b  INFO   forward  holder answered"].join("\n");
  ...
  it("没有时间戳的行跟着上一条（缩进），开头就没有时间戳的行丢弃", () => {
    const text = ["npm warn something", "2026-09-13T06:30:52.100Z  replica-a  ERROR  agent  boom", "Error: boom", "    at x (y.ts:1:1)"].join("\n");
