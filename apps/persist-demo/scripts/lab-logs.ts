...
export const DEFAULT_COMPOSE_FILE = fileURLToPath(new URL("../docker/compose.yml", import.meta.url));
export const DEFAULT_PROJECT = "runko-lab";
...
export function createLogDir(root: string = LOGS_ROOT, now: Date = new Date()): string {
  ...
  if (existing?.isSymbolicLink() === true) {unlinkSync(latest);}
  if (existing === undefined || existing.isSymbolicLink()) {symlinkSync(basename(dir), latest);}
  return dir;
}
...
export async function collectLabLogs(opts: CollectOptions): Promise<void> {
  const compose = async (...args: string[]): Promise<string> => {
    const { stdout } = await run(
      "docker",
      ["compose", "-p", opts.project ?? DEFAULT_PROJECT, "-f", opts.composeFile ?? DEFAULT_COMPOSE_FILE, ...args],
      { env: opts.env ?? process.env, maxBuffer: 256 * 1024 * 1024 },
    );
    return stdout;
  };

  const services = (await compose("config", "--services")).split("\n").filter((name) => name !== "");
  const texts = new Map<string, string>();
  for (const service of services) {
    // 已经删掉的容器没有日志可取，跳过它而不是让整次收集失败。
    const text = await compose("logs", "--no-color", "--no-log-prefix", service).catch(() => undefined);
  ...
  if (existsSync(testLog)) {sources.push(readFileSync(testLog, "utf8"));}
...
export function mergeTimeline(sources: readonly string[]): string {
  ...
  entries.sort((a, b) => (a.stamp === b.stamp ? a.order - b.order : a.stamp < b.stamp ? -1 : 1));
...
if (invokedDirectly) {
  ...
  console.log(`日志已存到 ${dir}（时间线：${join(dirname(dir), "lab-latest", "timeline.log")}）`);
}
