/**
 * 会话页的页眉：**分支是这一页的标题**（docs/ingress/features/chat-ui.md「分支是这一页的标题」）。
 *
 ...
 * 打它），但这里的分支名是 `runko/chat-<uuid>`：没人读得出、记得住，只会被复制。
 * 用版面最大的位置放一串没人读的字符，等于把主位给了噪音；而唯一能一眼认出这
 * 是哪个会话的东西——用户自己起的标题——反被压成行尾最小的灰字。
 *
 * 所以现在：状态点 + 会话标题居左当主位，分支名**中间省略**（`runko/chat-8430…5fb1`）
...
/**
 * `runko/chat-84302082-770a-4dd3-aa7f-edcf60125fb1` → `runko/chat-8430…5fb1`。
 *
 * 中间省略而不是 CSS `truncate`（那是截尾）：尾号是这串里唯一能区分两条会话
 * 分支的部分，截掉它等于让所有分支名长得一模一样。前缀留着是为了一眼看出这是
 * 条 runko 会话分支。复制的始终是完整名字，不是这里显示的缩写。
 */
