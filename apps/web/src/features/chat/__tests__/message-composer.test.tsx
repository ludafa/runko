...
/**
 * composer 的分流键位（docs/logic/orchestration/features/steer-and-queue.md §2.1/§2.2）：一轮进行中
 * Enter = [排队](../../../../../../docs/terms.md)、Alt+Enter / 插话按钮 =
 * [steer](../../../../../../docs/terms.md)；空闲时两者无差别（服务端都会起新一轮），
 * 所以插话按钮只在流式中出现。
 *
 * 另一半是流式态那颗按钮的语义（docs/logic/orchestration/features/turn-abort.md §2.1）：它是
...
describe('MessageComposer', () => {
  ...
  it('发送后清空输入框', async () => {
    ...
    // （docs/ingress/tech/composer-skill-mention.md §2.4）。
...
/**
 * [skill 提及](../../../../../../docs/terms.md)（docs/ingress/features/composer-skill-mention.md）：
 * 打 `/` 唤出[skill 清单](../../../../../../docs/terms.md)、选中后插入一枚原子标记块。
 *
 * 最要紧的一条是**菜单开着时 Enter 归菜单**——composer 的 Enter 平时是排队发送，
 * 绝不能因为菜单开着就把半截消息发出去（docs/ingress/tech/composer-skill-mention.md §6.1）。
 */
describe('MessageComposer · skill 提及', () => {
  ...
  it('选中后再按 Enter 才发送，发出去的文本带 /<skill 名>', async () => {
    ...
    // （docs/ingress/tech/composer-skill-mention.md §5.1）。
